...
class VirtTestOptionsProcess(object):
    ...
    def __init__(self, config):
        ...
        set_opt_from_settings(
            self.config, "vt.setup", "keep_guest_running", key_type=bool, default=False
        )
        # common section
        set_opt_from_settings(self.config, "vt.common", "data_dir", default=None)
        set_opt_from_settings(self.config, "vt.common", "tmp_dir", default="")
        set_opt_from_settings(
            self.config, "vt.common", "type_specific", key_type=bool, default=False
        )
        set_opt_from_settings(self.config, "vt.common", "mem", default=None)
        set_opt_from_settings(self.config, "vt.common", "nettype", default=None)
        set_opt_from_settings(self.config, "vt.common", "netdst", default="virbr0")
        # qemu section
        set_opt_from_settings(self.config, "vt.qemu", "accel", default="kvm")
        set_opt_from_settings(self.config, "vt.qemu", "vhost", default="off")
        set_opt_from_settings(self.config, "vt.qemu", "monitor", default=None)
        set_opt_from_settings(self.config, "vt.qemu", "smp", default="2")
        set_opt_from_settings(
            self.config, "vt.qemu", "image_type", default=SUPPORTED_IMAGE_TYPES[0]
        )
        set_opt_from_settings(
            self.config, "vt.qemu", "nic_model", default=SUPPORTED_NIC_MODELS[0]
        )
        set_opt_from_settings(
            self.config, "vt.qemu", "disk_bus", default=SUPPORTED_DISK_BUSES[0]
        )
        set_opt_from_settings(self.config, "vt.qemu", "sandbox", default="on")
        set_opt_from_settings(self.config, "vt.qemu", "defconfig", default="yes")
        set_opt_from_settings(self.config, "vt.qemu", "malloc_perturb", default="yes")

        # debug section
        set_opt_from_settings(
            self.config, "vt.debug", "no_cleanup", key_type=bool, default=False
        )
    ...
    def _process_qemu_bin(self):
        ...
        """
        qemu_bin_setting = "option --vt-qemu-bin or " "config vt.qemu.qemu_bin"
        if (
            get_opt(self.config, "vt.config")
            ...
            (
                ...
                qemu_io_path,
                qemu_dst_bin_path,
            ) = standalone_test.find_default_qemu_paths(
            ...
            if qemu_dst_bin_path is not None:
                self.cartesian_parser.assign("qemu_dst_binary", qemu_dst_bin_path)
            ...
            if get_opt(self.config, "vt.common.nettype") == "bridge":
                if os.getuid() != 0:
                    raise ValueError(
                        ...
                        "need to be root"
                        % (nettype_setting, get_opt(self.config, "vt.common.nettype"))
                    )
    ...
    def _process_machine_type(self):
        machine_type_setting = (
            "option --vt-machine-type or config " "vt.common.machine_type"
        )
        if not get_opt(self.config, "vt.config"):
            if get_opt(self.config, "vt.common.machine_type") is None:
                ...
                if (get_opt(self.config, "vt.common.arch") is None) and (
                    ...
                    self.cartesian_parser.only_filter(defaults.DEFAULT_MACHINE_TYPE)
            else:
    ...
    def _process_image_type(self):
        ...
        if not get_opt(self.config, "vt.config"):
            if get_opt(self.config, "vt.qemu.image_type") in SUPPORTED_IMAGE_TYPES:
                ...
                self.cartesian_parser.assign(
                    "image_format", get_opt(self.config, "vt.qemu.image_type")
                )
        else:
    ...
    def _process_nic_model(self):
        ...
        if not get_opt(self.config, "vt.config"):
            if get_opt(self.config, "vt.qemu.nic_model") in SUPPORTED_NIC_MODELS:
                self.cartesian_parser.only_filter(
                    get_opt(self.config, "vt.qemu.nic_model")
                )
            else:
    ...
    def _process_libvirt_specific_options(self):
        ...
        if get_opt(self.config, "vt.libvirt.connect_uri"):
            ...
            if not driver_found:
                raise ValueError(
                    "Unsupported %s '%s'"
                    % (uri_setting, get_opt(self.config, "vt.libvbirt.connect_uri"))
                )
        else:
    ...
    def _process_restore_image(self):
        if not get_opt(self.config, "vt.config"):
            if get_opt(self.config, "vt.setup.backup_image_before_test"):
                self.cartesian_parser.assign("backup_image_before_testing", "yes")
            if get_opt(self.config, "vt.setup.restore_image_after_test"):
                self.cartesian_parser.assign("restore_image_after_testing", "yes")
    ...
    def _process_options(self):
        ...
        if (not vt_type) and (not vt_config):
            raise ValueError(
                "No %s or %s specified" % (vt_type_setting, vt_config_setting)
            )
        ...
        keep_at_variants = get_opt(self.config, "vt.keep_at_variants_in_shortnames")
        if keep_at_variants is None:
            keep_at_variants = False
        self.cartesian_parser = cartesian_config.Parser(
            debug=False, keep_at_variants_in_shortnames=keep_at_variants
        )
        ...
        if vt_config:
            ...
            if "image_backend" not in get_opt(self.config, "vt.filter.default_filters"):
                self.cartesian_parser.only_filter("(image_backend=" "filesystem)")
