...
def add_basic_vt_options(parser):
    ...
    help_msg = (
        "Explicitly choose a cartesian config. When choosing this, "
    ...
    add_option(parser, dest="vt.save_config", arg="--vt-save-config", help=help_msg)
    ...
    add_option(
        parser, dest="vt.common.arch", arg="--vt-arch", default=arch, help=help_msg
    )

    machine = get_settings_value(
        "vt.common", "machine_type", default=defaults.DEFAULT_MACHINE_TYPE
    )
    ...
    help_msg = (
        "Select the guest OS to be used. If --vt-config is provided, "
    ...
    help_msg = (
        "List of space separated 'no' filters to be passed to the "
        "config parser.  Default: '%(default)s'"
    )
    add_option(
        parser, dest="vt.no_filter", arg="--vt-no-filter", default="", help=help_msg
    )

    help_msg = (
        "List of space separated 'only' filters to be passed to the "
        "config parser.  Default: '%(default)s'"
    )
    add_option(
        parser, dest="vt.only_filter", arg="--vt-only-filter", default="", help=help_msg
    )
...
def add_qemu_bin_vt_option(parser):
    ...
    try:
        ...
        qemu_bin_path = None
    qemu_bin = get_settings_value("vt.qemu", "qemu_bin", default=None)
    ...
    help_msg = (
        "Path to a custom qemu binary to be tested. If --vt-config is "
        "provided and this flag is omitted, no attempt to set the "
        "qemu binaries will be made. Current: %s" % _str_or_none(qemu_bin)
    )
    add_option(
        parser,
    ...
    qemu_dst = get_settings_value("vt.qemu", "qemu_dst_bin", default=qemu_bin_path)
    help_msg = (
        "Path to a custom qemu binary to be tested for the destination"
...
class VTRun(CLI):
    ...
    def configure(self, parser):
        ...
        help_msg = (
            "Choose test connect uri for libvirt (E.g: %s). Current: "
            "%%(default)s" % supported_uris
        )
        uri_current = get_settings_value("vt.libvirt", "connect_uri", default=None)
        add_option(
            parser=vt_compat_group_libvirt,
