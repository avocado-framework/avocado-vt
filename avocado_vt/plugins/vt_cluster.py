...
class VTCluster(Pre, Post):
    ...
    def _setup_node():
        ...
        :raise ClusterSetupError: If starting an agent fails.
        """
    ...
    def _setup_mgr():
        ...
        :raise ClusterManagerSetupError: If the manager setup fails.
        """
        try:
    ...
    def _cleanup_mgr():
        ...
        :raise ClusterManagerCleanupError: If the manager cleanup fails.
        """
        try:
    ...
    def _cleanup_node(self, job):
        """
        Finalizes node cleanup by collecting logs and stopping agents in parallel.
        ...
        :raises ClusterCleanupError: If stopping an agent server fails (logged as warning).
        """
        ...
        def __cleanup_agent_node(node):
            """Helper function to cleanup agent on a single node."""
            ...
            try:
                ...
                stop_error = ClusterCleanupError(e)

            return node, upload_error, stop_error

        with ThreadPoolExecutor(max_workers=len(nodes)) as executor:
            future_to_node = {
                executor.submit(__cleanup_agent_node, node): node for node in nodes
            }
