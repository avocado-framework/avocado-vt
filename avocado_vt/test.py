...
class VirtTest(test.Test, utils.TestUtils):
    ...
    def _cleanup_partition(self):
        ...
        Clears partition pools, fetches logs from each node to the results
        directory, and releases the partition back to the cluster. This
        ensures proper cleanup of resources after test completion.
        """
        ...
        if self._cluster_partition.nodes:
            for node in self._cluster_partition.nodes:
                try:
                    ...
                    node.proxy.api.start_logger_client(host, port)
                except Exception:
                    self.log.warning(
