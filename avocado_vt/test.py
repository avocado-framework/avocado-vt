...
class VirtTest(test.Test, utils.TestUtils):
    ...
    def _setup_partition(self):
        ...
        for pool_tag in self.params.objects("pools"):
            ...
            pools = set(resmgr.pools.keys()) - set(cluster.partitions[0].pools.values())
    ...
    def _fetch_node_logs(self, node):
        ...
        try:
            ...
            self.log.error("Failed to create log directory %s: %s", node_dir,
                           e)
            return

        log_sources = [
            # ("service", lambda: node.proxy.api.get_service_log_filename()),
            # ("console", lambda: node.proxy.api.get_console_log_dir()),
            # ("daemon", lambda: node.proxy.api.get_daemon_log_dir()),
            # ("ip_sniffer", lambda: node.proxy.api.get_ip_sniffer_log_dir())
        ...
        for log_type, get_path_func in log_sources:
            try:
                ...
                if not log_path:
                    self.log.debug("No %s log path available for node '%s'",
                                   log_type, node.tag)
                    continue
                ...
                self.log.warning("Failed to fetch %s logs from node '%s': %s",
                                 log_type, node.tag, e)
        ...
        if self._cluster_partition.nodes:
            for node in self._cluster_partition.nodes:
                try:
                    ...
                    node.proxy.core.start_log_redirection(
                        host, port
                    )
                except Exception:
                    self.log.warning(
