...
from avocado_vt.vt_agent.core import data_dir, rpc
...
def _signal_handler(signum, frame):
    ...
    LOG.info("Received %s, initiating graceful shutdown...", signal_name)
    sys.exit(0)
...
def _write_pid_file(pid_file):
    ...
    try:
        with open(pid_file, "w") as f:
            f.write(pid + "\n")
        LOG.debug("PID file written: %s (PID: %s)", pid_file, pid)
    except OSError as e:
...
def _cleanup_pid_file(pid_file):
    ...
    if os.path.exists(pid_file):
        try:
            os.remove(pid_file)
            LOG.debug("PID file removed: %s", pid_file)
        except OSError as e:
...
def _cleanup_tmp_dirs():
    ...
    if not os.path.isdir(data_dir_path):
        LOG.debug("Data directory does not exist, no temp cleanup needed")
        return
    ...
    if not tmp_dirs:
        LOG.debug("No temporary directories found to clean")
        return
...
def run(host, port, pid_file):
    ...
    try:
        ...
        LOG.info("Starting Avocado VT Agent server on %s:%s", host, port)
        pid = str(os.getpid())
        LOG.info("Agent daemon starting with PID %s", pid)

        LOG.info("Loading agent services...")
        ...
        LOG.info("Agent server ready and waiting for connections")
        LOG.info("Server endpoints available at http://%s:%s", host, port)
        ...
        _cleanup_tmp_dirs()
        LOG.info("Agent server shutdown complete")
