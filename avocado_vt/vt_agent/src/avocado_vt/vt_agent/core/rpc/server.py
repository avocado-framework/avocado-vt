...
from avocado_vt.vt_agent.services import api
...
class RPCServer(object):
    ...
    def _load_server_api(self):
        ...
        Functions are sourced from the `avocado_vt.vt_agent.services.api` module.
        """
        api_functions_registered = []
        ...
        for member_name, member_obj in inspect.getmembers(api):
            if inspect.isfunction(member_obj) and not member_name.startswith("_"):
                ...
                try:
                    ...
                    api_functions_registered.append(rpc_name)
                except Exception as e:
                    LOG.error("Failed to register API function %s: %s", rpc_name, e)

        if api_functions_registered:
            LOG.info(
                "API functions registered: %s", ", ".join(api_functions_registered)
            )
        else:
            LOG.warning("No API functions were registered")
