...
RPC endpoint that the controller node calls to coordinate resource operations across
the cluster.
...
invoked remotely via the cluster proxy system. This enables the controller node's
...
Service Architecture:
    Controller Node (ResourceManager) → Cluster Proxy → Worker Node (Resource Service) → ResourceBackingManager
...
def create_pool_connection(pool_config):
    ...
    :return: Succeeded: 0, {confs to be updated on controller node}
