...
def get_qemu_image_repr(image_config, output=None):
    ...
    mapping = {
        "uri": lambda i: volume_config["spec"]["uri"],
...
def _get_backing_name(image_name, image_topology_config):
    backing = None
    if image_topology_config["type"] == "chain":
        image_chain = image_topology_config["value"]
        i = image_chain.index(image_name)
        backing = image_chain[i - 1] if i > 0 else None
    return backing


def _clone(source_logical_image_config, clone_logical_image_config):
    src_meta = source_logical_image_config["meta"]
    clone_meta = clone_logical_image_config["meta"]

    # Clone each qemu layer image
    for idx in range(len(src_meta["topology"]["value"])):
        _copy_data(
            source_logical_image_config,
            src_meta["topology"]["value"][idx],
            clone_logical_image_config,
            clone_meta["topology"]["value"][idx],
        )

    # Construct the backing chain
    if clone_meta["topology"]["type"] == "chain":
        image_names = clone_meta["topology"]["value"]
        for i in range(1, len(image_names)):
            arguments = {
                "source": image_names[i - 1],
                "target": image_names[i],
            }
            _rebase(clone_logical_image_config, arguments)
...
def _create(logical_image_config, arguments):
    def _dd(image_tag):
        ...
        if image_spec["format"] == "raw":
            count = normalize_data_size(image_spec["size"], order_magnitude="M")
            qemu_img_cmd = "dd if=/dev/zero of=%s count=%s bs=1M" % (
                volume_config["spec"]["path"],
                count,
            )
        LOG.debug(f"Create image with command: {qemu_img_cmd}")
    ...
    def _qemu_img_create(image_tag):
        ...
        secret_objects = list()
        base_tag = _get_backing_name(image_tag, logical_image_meta["topology"])
...
def _dd(
    ...
    :return: process.CmdResult object containing the result of the
             command
    """
    source_logical_image_meta = source_logical_image_config["meta"]
    source_logical_image_spec = source_logical_image_config["spec"]
    target_logical_image_spec = target_logical_image_config["spec"]
    source_image_config = source_logical_image_spec["images"][source_image_name]
    target_image_config = target_logical_image_spec["images"][target_image_name]
    ...
    base_tag = _get_backing_name(
        source_image_name, source_logical_image_meta["topology"]
    )
    if base_tag is not None:
        # Add all backings' secret objects
        for tag in source_logical_image_meta["topology"]["value"]:
            config = source_logical_image_spec["images"][tag]
            objs_repr, _, _ = get_qemu_image_repr(config)
            if objs_repr:
                secret_objects.append(objs_repr)
            if tag == base_tag:
    ...
    dd_cmd = QEMU_IMG_BINARY + " " + cmd_formatter.format(dd_cmd, **cmd_dict)
...
def _copy_data(
    ...
    source_images = source_logical_image_config["spec"]["images"]
    target_images = target_logical_image_config["spec"]["images"]
    source_image_config = source_images[source_image_name]
    target_image_config = target_images[target_image_name]
    source_volume_config = source_image_config["spec"]["volume"]
    target_volume_config = target_image_config["spec"]["volume"]
    source_volume_type = source_volume_config["meta"]["volume-type"]
    target_volume_type = source_volume_config["meta"]["volume-type"]

    if source_volume_type == "file" and target_volume_type == "file":
        # Copy the files for the file-based volumes
        shutil.copyfile(
            source_volume_config["spec"]["uri"], target_volume_config["spec"]["uri"]
        )
    else:
        # Use qemu-img dd to copy data for other types of volumes
        _dd(
            source_logical_image_config,
            source_image_name,
            target_logical_image_config,
...
def _backup(logical_image_config, arguments):
    ...
    for image_name in image_names:
        ...
        _copy_data(
            logical_image_config, image_name, logical_image_config, backup_image_name
        )
...
def _restore(logical_image_config, arguments):
    ...
    for image_name in image_names:
        ...
        backup_image_name = image_config["spec"]["backup"]
        _copy_data(
            logical_image_config, backup_image_name, logical_image_config, image_name
        )


def _rebase(logical_image_config, arguments):
    qemu_image_binary = arguments.pop("qemu_img_binary", QEMU_IMG_BINARY)
    image_repr_format = arguments.pop("repr", None)
    backing = arguments.pop("source")
    backing_config = logical_image_config["spec"]["images"][backing]
    target = arguments.pop("target")
    target_config = logical_image_config["spec"]["images"][target]
    ...
    # Add all backings' secret and access auth objects
    for tag in logical_image_config["meta"]["topology"]["value"]:
        if tag == backing:
            break
        config = logical_image_config["spec"]["images"][tag]
        objs_repr, _, _ = get_qemu_image_repr(config)
        if objs_repr:
            secret_objects.append(objs_repr)
    ...
    LOG.debug(f"Rebase {target} onto {backing} by command: {qemu_img_cmd}")
    process.run(qemu_img_cmd, shell=True, verbose=False, ignore_status=False)
...
def _check(image_config, arguments):
    ...
    LOG.debug(f"Check {target} with command: {qemu_img_cmd}")
...
def _info(image_config, arguments):
    ...
    LOG.debug(f"Query info for {target} with command: {qemu_img_cmd}")
    cmd_result = process.run(
        qemu_img_cmd, shell=True, verbose=True, ignore_status=False
    )
    return cmd_result.stdout_text
