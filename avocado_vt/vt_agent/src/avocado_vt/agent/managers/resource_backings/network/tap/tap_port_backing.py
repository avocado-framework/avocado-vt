...
class TapPortBacking(NicPortBacking):
    ...
    def allocate_resource(self, network_connection, arguments=None):
        ...
        return self.get_all_resource_info(network_connection)
    ...
    def get_all_resource_info(self, network_connection=None, arguments=None):
        config = super().get_all_resource_info(network_connection)
        config["meta"]["port-type"] = self.PORT_TYPE
        if self.switch and self.tap_fd and self.tap_ifname:
            config["meta"]["allocated"] = (
                True
                if self.switch in (bridge.find_bridge_name(self.tap_ifname),)
                else False
            )
        else:
            config["meta"]["allocated"] = False

        config["spec"]["switch"] = self.switch
        config["spec"]["fds"] = self.tap_fd
        config["spec"]["ifname"] = self.tap_ifname
        return config

    def is_resource_allocated(self, network_connection=None):
        return self.get_all_resource_info(network_connection)["meta"]["allocated"]
