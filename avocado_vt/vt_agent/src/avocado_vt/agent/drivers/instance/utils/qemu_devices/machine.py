...
def create_machine_q35(
    machine_props, cpu, controller, has_option_device, has_option_global
):
    """
    Q35 + ICH9
    """
    ...
    if has_option_device and has_option_global:
        devices.append(
            qdevices.QStringDevice("fdc", child_bus=qdevices.QFloppyBus("floppy"))
        )
    else:
        devices.append(
            qdevices.QStringDevice("fdc", child_bus=qdevices.QOldFloppyBus("floppy"))
        )
...
def create_machine_s390_virtio(machine_props, cpu):
    ...
    devices.append(
        qdevices.QMachine(params=machine_props, child_bus=bus, aobject="virtio-blk-ccw")
    )
    return devices
...
def create_machine_devices(
    machine, cpu, controller, has_option_device, has_option_global
):
    ...
    if avocado_machine == "invalid_machine":
        devices = create_machine_i440fx(
            {"type": machine_type}, cpu, has_option_device, has_option_global
        )
    elif machine_type == "pc" or "i440fx" in machine_type:
        devices = create_machine_i440fx(
            machine_params, cpu, has_option_device, has_option_global
        )
    elif "q35" in machine_type:
        devices = create_machine_q35(
            machine_params, cpu, controller, has_option_device, has_option_global
        )
    elif machine_type.startswith("pseries"):
        devices = create_machine_pseries(
            machine_params, cpu, has_option_device, has_option_global
        )
    ...
    # FIXME: support the arm architecture by avocado_machine
        ...
        LOG.warn(
            "Machine type '%s' is not supported " "by avocado-vt, errors might occur",
            machine_type,
        )
