...
def create_serial_devices(
    ...
    def __get_serial_console_filename(name):
        if name:
            return os.path.join(
                data_dir.get_tmp_dir(), "serial-%s-%s" % (name, driver_id)
            )
        return os.path.join(data_dir.get_tmp_dir(), "serial-%s" % driver_id)
    ...
    if arm_serial or not has_option_chardev or not has_device_serial:
    ...
    for k, v in backend_props.items():
        if k == "port" and isinstance(v, (list, tuple)):
            ...
            free_ports = utils_misc.find_free_ports(v[0], v[1], serial_count, host)
    ...
    if serial_type.startswith("virt"):
        ...
        if not bus or bus == "<new>":
            ...
            if bus is None or bus == "<new>" or serial_type == "virtconsole":
                ...
                bus = dev_container.list_missing_named_buses(_hba, "SERIAL", bus + 1)[
                    -1
                ]
                ...
                bus = bus.busid
        dev = qdevices.QDevice(serial_type, serial_props, parent_bus={"busid": bus})
        ...
        if "ppc" in os_arch and serial_type == "spapr-vty":
