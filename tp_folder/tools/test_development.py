...
def develop(config, tag=""):
    ...
    LOG_UI.info(
        "Developing on virtual machines %s (%s)",
        ", ".join(selected_vms),
    ...
    for test_worker in graph.workers.values():
        ...
        nodes = graph.parse_composite_nodes(
            "all..manual..develop.%s" % mode, test_worker.net, tag, params=setup_dict
        )
        if len(nodes) == 0:
            ...
            raise RuntimeError(
                f"There must be exactly one {mode} develop test variant "
