...
def deploy_avocado(vm):
    ...
    log.debug(
        f"Deploying utilities from {source_avocado_path} on host to "
...
def run(test, params, env):
    ...
    additional_deployment_path = params.get(
        "additional_deployment_dir", "/mnt/local/packages"
    )
    destination_packages_path = params.get("deployed_packages_path", "/tmp/packages")
    if additional_deployment_path is not None and os.path.isdir(
        additional_deployment_path
    ):
        log.info(
            f"Deploying additional packages and data to {tmp_dir} on {params['main_vm']}"
        )
        ...
        deploy_data(
            vm,
            os.path.basename(additional_deployment_path),
            "packages",
            os.path.dirname(additional_deployment_path),
            60,
        )
    else:
        raise exceptions.TestError(
            "Additional deployment path %s does not exist (current dir: "
            "%s)" % (additional_deployment_path, os.getcwd())
        )
    if params.get("extra_rpms", None) is not None:
        if os_type != "linux":
            raise NotImplementedError(
                "RPM updates are only available on some linux distros and not %s"
                % os_type
            )
        for rpm in params.objects("extra_rpms"):
            session.cmd(
                "rpm -Uv --force %s" % os.path.join(destination_packages_path, rpm)
            )
