...
class LiveMigrationTaskQemu(LiveMigrationTask):
    ...
    def _prepare_migrate_destination(self):
        ...
        spec = self._instance.format_specs(
            format_type="json"
        )  # FIXME: convert it to json
        return self._destination.proxy.virt.vmm.migrate_instance_prepare(
            self._instance.uuid, driver_kind, spec, mig_params
        )
    ...
    def _perform_migrate_source(self, migrate_incoming_uri):
        ...
        mig_ret, mig_info = self._source.proxy.virt.vmm.migrate_instance_perform(
    ...
    def _finish_migrate_destination(self, mig_ret, timeout):
        ...
        mig_params = dict()
        mig_params["flags"] = self._mig_config.get("flags")
        ret, mig_info = self._destination.proxy.virt.vmm.migrate_instance_finish(
    ...
    def _confirm_migrate_source(self, inmig_ret):
        ...
        mig_params = dict()
        mig_params["flags"] = self._mig_config.get("flags")
        self._source.proxy.virt.vmm.migrate_instance_confirm(
    ...
    def _cancel_migrate(self):
        return self._source.proxy.virt.vmm.migrate_instance_cancel(self._instance.uuid)

    def _resume_migrate(self):
        self._source.proxy.virt.vmm.migrate_instance_resume(self._instance.uuid)
