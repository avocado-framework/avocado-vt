...
from .objects.migration_task import LiveMigrationTask, LiveMigrationTaskQemu
...
class _VirtualMachinesManager(object):
    ...
    def start_instance(self, instance_id, timeout=120):
        """
        Start an instance.
    ...
    def undefine_instance(self, instance_id, free_mac_addresses=True):
        ...
        instance = self._get_instance(instance_id)
        # if instance.state in States.STOPPED:
        self._instance_api.undefine_instance(instance, free_mac_addresses)
        del self._instances[instance_id]
        self._save()
        # else:
        #     raise VMMError("Failed to delete the instance")
    ...
    # def prepare_migrate_instance(self, instance_id):
    #     mig_task = self._get_migration_task(instance_id)
    #     self._migration_taskapi.prepare_migrate_instance(mig_task)
    #
    # def perform_migrate_instance(self, instance_id):
    #     mig_task = self._get_migration_task(instance_id)
    #     self._migration_taskapi.live_migrate_instance(mig_task)
    #
    # def post_migrate_instance(self, instance_id):
    #     mig_task = self._get_migration_task(instance_id)
    #     self._migration_taskapi.post_migrate_instance(mig_task)
...
def _build_spec(name, kind, vm_params):
    ...
    return specs.get(kind)(name, vm_params)
...
def define_instance_config(name, vm_params):
    ...
    spec = _build_spec(name, kind, vm_params)
    ...
    # config_repr = json.dumps(config, indent=4, separators=(",", ": "))
    # LOG.debug(f"The config of the instance {name}: {config_repr}")
    # LOG.debug(f"The spec of the instance {name}:\n{spec}")
    return config
