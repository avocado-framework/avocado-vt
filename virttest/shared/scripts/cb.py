...
def main(argv):
    ...
    parser = OptionParser()
    parser.add_option(
        "-c", "--clear", dest="clear", action="store_true", help="clear the clipboard"
    )
    parser.add_option(
        "-s", "--set", dest="setcb", default="", help="Sets the local clip board"
    )
    parser.add_option(
        "-p", "--print", dest="setcb", default="", help="Prints the text that was set"
    )
    parser.add_option(
        "-i",
        "--set_image",
        dest="setcbimage",
        default="",
        help="Sets an image to local clip board",
    )
    parser.add_option(
        "-m", help="Saves an image to the /tmp directory", dest="setfilename"
    )
    parser.add_option(
        "-r",
        "--file_copy",
        dest="setcb_file",
        default="",
        help="Creates a file, Reads the text from "
        + "a file and puts it onto the clipboard",
    )
    parser.add_option(
        "-f",
        "--file_paste",
        dest="pastecb_filename",
        default="",
        help="Given a filename, the clipboard " + "contents will be pasted",
    )
    parser.add_option(
        "-n",
        "--create_file",
        dest="setcb_lrgstr_file",
        default="",
        help="Given a integer value a file will "
    ...
    if options.setcb:
        ...
        print(
            "Starting to write the clipboard text to the file"
        ...
        file_contents.close()
        print(
            "Non random string put into file: /tmp/StringLengthTest. "
            + " The string has also been placed in the clipboard. "
            + "String of size "
            + options.setcb_lrgstr_file
        )
        clipboard.store()
    else:
