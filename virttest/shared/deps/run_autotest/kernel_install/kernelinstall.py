...
class kernelinstall(test.test):
    ...
    def _kernel_install_koji(
        self, kernel_koji_spec, kernel_deps_koji_spec, need_reboot=True
    ):
        ...
        if len(rpm_file_name_list) == 0:
            raise error.TestError(
                "No packages on brew/koji match spec %s" % kernel_deps_koji_spec
            )
        ...
        if len(rpm_file_name_list) == 0:
            raise error.TestError(
                "No packages on brew/koji match spec %s" % kernel_koji_spec
            )
    ...
    def _kernel_install_git(
        ...
        self._kernel_install_src(repodir, config, config_list, patch_list, need_reboot)

    def execute(self, install_type="koji", params=None):
        need_reboot = params.get("need_reboot") == "yes"

        LOG.info("Chose to install kernel through '%s', proceeding", install_type)
        ...
        if install_type == "rpm":
            ...
            self._kernel_install_koji(
                kernel_koji_spec, kernel_deps_koji_spec, need_reboot
            )
            ...
            self._kernel_install_src(src_pkg, config, None, patch_list, need_reboot)
        else:
            LOG.error(
                "Could not find '%s' method, " "keep the current kernel.", install_type
            )
