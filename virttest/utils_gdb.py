...
class GDB(aexpect.Expect):
    ...
    def _callback(self, callback_type, info):
        ...
        thread = threading.Thread(
            target=callback_func, args=(self, info, params), name=callback_type
        )
        thread.start()
    ...
    def cmd(self, command, cont=True):
        ...
        try:
            ...
            if result["command"] != command:
                raise GDBCmdError(
                    "Mismatch command result line got %s but "
                    "expecting %s" % (result["command"], command)
                )
    ...
    def wait_for_start(self, timeout=60):
        ...
        """
        LOG.debug("Waiting for gdb inferior %s to start" % self.inferior_command)
    ...
    def wait_for_stop(self, timeout=60):
        ...
        :param timeout: Max time to wait
        """
        LOG.debug("Waiting for gdb inferior %s to stop" % self.inferior_command)
