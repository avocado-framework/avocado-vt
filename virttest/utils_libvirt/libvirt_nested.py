...
def install_virt_pkgs(vm_session):
    ...
    if not pkg_mgr.install():
        raise exceptions.TestError("Package '%s' installation " "fails" % pkg_names)
...
def enable_nested_virt_on_host():
    ...
    if not cpu.get_cpu_vendor().count("Intel"):
        raise exceptions.TestCancel(
            "Only Intel machine is supported in " "this function so far."
        )
    ...
    ret = process.run(
        cmd, verbose=True, ignore_status=False, shell=True
    ).stdout_text.strip()
    if ret not in ["Y", "1"]:
        raise exceptions.TestError(
            "Checking nested virt environment "
...
def update_vm_cpu(guest_xml, cpu_mode=None):
    ...
    if cpu_mode:
        LOG.info(
            "Update cpu mode from '{}' to '{}'".format(cur_vmcpuxml.mode, cpu_mode)
        )
        cur_vmcpuxml.mode = (
            cpu_mode if cur_vmcpuxml.mode != cpu_mode else cur_vmcpuxml.mode
        )
