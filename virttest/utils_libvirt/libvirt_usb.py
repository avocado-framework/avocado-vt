...
def get_usbs_lists_on_host():
    ...
    if lsusb_output.exit_status != 0:
        raise exceptions.TestError("Failed to execute lsusb on host with output:{}".format(lsusb_output))
...
def check_usb_disk_type_in_vm(session, partition):
    ...
    try:
        ...
        LOG.error("Error happens when check if new disk is usb device: %s",
                  str(err))
        return False
