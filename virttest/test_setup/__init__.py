...
class HugePageConfig(object):
        ...
        if self.session:
            if self.session.cmd_status("ll %s" % self.kernel_hp_file):
                raise exceptions.TestSkipError(
                    "Hugepage config file on remote is not found: " "%s" % self.kernel_hp_file
                )
        else:
        ...
        if self.kernel_hp_file == "/proc/sys/vm/nr_hugepages":
            ...
            try:
                self.hugepage_size = int(
                    re.search(r"(\d+)kB", self.kernel_hp_file).groups()[0]
                )
            except:
                raise ValueError(
                    "Could not get huge page size setting "
    ...
    def set_node_num_huge_pages(self, num, node, pagesize, ignore_error=False):
        ...
        if obj.sys_fs_value < int(num):
            msg = (
                "%s (expecting %s) hugepages is set on the node %s, "
    ...
    def cleanup(self):
        ...
        if self.deallocate:
            ...
            self.ext_hugepages_surp = utils_memory.get_num_huge_pages_surp(session=self.session)
    ...
    def cleanup(self, env):
        ...
        if ksmtuned_pid != 0:
            ...
            if not wait.wait_for(
                lambda: not self.ksmctler.is_ksm_running(),
                timeout=120,
                step=5,
                text="Waiting for ksm to be disabled",
            ):
                LOG.warning(
                    "KSM is still running, subsequent test may not " "work as expected"
                )
            return
