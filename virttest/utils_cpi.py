...
class CPIChecker(object):
    ...
    def check_system_type(self):
        ...
        """
        if "system_type" not in self.cpi_data:
            raise RuntimeError("system_type not available in CPI data")

        system_type = self.cpi_data["system_type"]

        try:
            config = self.get_cpi_config()
            expected_system_type = config.get("CPI_SYSTEM_TYPE", "")
        except Exception as e:
            logging.warning(f"Could not read CPI config, using empty string: {e}")
            expected_system_type = ""

        if system_type == "" and expected_system_type != "":
            raise AssertionError(
                f"system_type is empty (Secure Execution guest without CPI_PERMIT_ON_PVGUEST='1'?). "
                f"Expected: {expected_system_type}"
            )

        if system_type != expected_system_type:
            raise AssertionError(
                f"system_type mismatch: expected '{expected_system_type}', got '{system_type}'"
            )

        logging.info(f"system_type check passed: {system_type}")
        return True
    ...
    def check_system_name(self):
        ...
        :raises: AssertionError if check fails
        """
        if "system_name" not in self.cpi_data:
            raise RuntimeError("system_name not available in CPI data")

        system_name = self.cpi_data["system_name"]

        try:
            config = self.get_cpi_config()
            expected_system_name = config.get("CPI_SYSTEM_NAME", "")
        except Exception as e:
            logging.warning(f"Could not read CPI config, using empty string: {e}")
            expected_system_name = ""

        if system_name == "" and expected_system_name != "":
            raise AssertionError(
                f"system_name is empty (Secure Execution guest without CPI_PERMIT_ON_PVGUEST='1'?). "
                f"Expected: {expected_system_name}"
            )

        if system_name != expected_system_name:
            raise AssertionError(
                f"system_name mismatch: expected '{expected_system_name}', got '{system_name}'"
            )

        logging.info(f"system_name check passed: {system_name}")
        return True
    ...
    def check_sysplex_name(self):
        ...
        :raises: AssertionError if check fails
        """
        if "sysplex_name" not in self.cpi_data:
            raise RuntimeError("sysplex_name not available in CPI data")

        sysplex_name = self.cpi_data["sysplex_name"]

        try:
            config = self.get_cpi_config()
            expected_sysplex_name = config.get("CPI_SYSPLEX_NAME", "")
        except Exception as e:
            logging.warning(f"Could not read CPI config, using empty string: {e}")
            expected_sysplex_name = ""

        if sysplex_name == "" and expected_sysplex_name != "":
            raise AssertionError(
                f"sysplex_name is empty (Secure Execution guest without CPI_PERMIT_ON_PVGUEST='1'?). "
                f"Expected: {expected_sysplex_name}"
            )

        if sysplex_name != expected_sysplex_name:
            raise AssertionError(
                f"sysplex_name mismatch: expected '{expected_sysplex_name}', got '{sysplex_name}'"
            )

        logging.info(f"sysplex_name check passed: {sysplex_name}")
        return True
...
def check_cpi_fields(vm, serial=False, debug=False, max_age_delta=60):
    """
...
def get_human_readable_summary(vm, serial=False, debug=False):
    """
