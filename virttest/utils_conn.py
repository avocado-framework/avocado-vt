...
class ConnCopyError(ConnectionError):
    ...
    def __str__(self):
        return "Copy file from %s to %s failed." % (self.src_path, self.dest_path)
...
class ListenUNIXSocketError(ConnectionError):
    ...
    def __str__(self):
        return (
            "Failed to start a proxy that listens on UNIX socket. "
...
class ConnUNIXSocketError(ConnectionError):
    ...
    def __str__(self):
        return (
            "Failed to start a proxy that connects to UNIX socket. "
            "cmd: '%s',\n output: %s" % (self.cmd, self.output)
        )
...
class ConnectionBase(propcan.PropCanBase):
    ...
    def _new_client_session(self):
        ...
        prompt = r"[\#\$]\s*$"
        try:
            client_session = remote.wait_for_login(
                transport, host, port, username, password, prompt
            )
...
class SSHConnection(ConnectionBase):
    ...
    def __init__(self, *args, **dargs):
        ...
        """
        init_dict = dict(*args, **dargs)
        init_dict["ssh_rsa_pub_path"] = init_dict.get(
            "ssh_rsa_pub_path", "/root/.ssh/id_rsa.pub"
        )
        init_dict["ssh_id_rsa_path"] = init_dict.get(
            "ssh_id_rsa_path", "/root/.ssh/id_rsa"
        )
        ...
        for key in tool_dict:
            ...
            try:
                ...
                LOG.debug(
                    "%s executable not set or found on path,"
        ...
        self.server_authorized_keys = remote_old.RemoteFile(
    ...
    def conn_check(self):
        ...
        ssh = self.SSH
        if ssh == "/bin/true":
            raise ConnToolNotFoundError("ssh", "executable not set or found on path, ")
    ...
    def conn_setup(self, timeout=10):
        ...
        cmd = "%s -i %s %s@%s" % (ssh_copy_id, ssh_rsa_pub_path, server_user, server_ip)
...
class TCPConnection(ConnectionBase):
    ...
    def conn_setup(self):
        ...
        if not libvirt_version.version_compare(5, 6, 0, server_session):
            ...
            pattern_to_repl = {
                r".*LIBVIRTD_ARGS\s*=\s*\"\s*--listen\s*\".*": 'LIBVIRTD_ARGS="--listen"'
            }
        ...
        if tcp_min_ssf:
            pattern_to_repl[r".*tcp_min_ssf\s*=.*"] = "tcp_min_ssf=%s" % (tcp_min_ssf)
        ...
        if self.sasl_type == "gssapi":
            ...
            if listen_addr:
                ...
                hostname = server_runner.run(
                    "hostname", ignore_status=True
                ).stdout_text.strip()
                pattern_to_repl = {
                    r".*%s.*" % listen_addr: "%s %s" % (listen_addr, hostname)
                }
                self.client_hosts.sub_else_add(pattern_to_repl)
        else:
            keytab = ""
        pattern_to_repl = {
            r".*mech_list\s*:\s*.*": "mech_list: %s" % self.sasl_type,
            r".*keytab\s*:\s*.*": keytab,
        }
        self.remote_saslconf.sub_else_add(pattern_to_repl)

        if tcp_port != "16509" and libvirt_version.version_compare(
            5, 6, 0, server_session
        ):
...
class TLSConnection(ConnectionBase):
    ...
    __slots__ = (
        ...
        "server_qemuconf",
        "client_qemuconf",
        "server_libvirtd_tls_socket",
        "client_libvirtd_tls_socket",
        "special_cn",
        "server_setup_local",
        "server_info_ip",
        "daemon_conf",
    ...
    def __init__(self, *args, **dargs):
        ...
        init_dict["scp_list_server"] = init_dict.get(
            "scp_list_server", "cacert,cakey," "servercert,serverkey"
        )
        init_dict["scp_list_client"] = init_dict.get(
            "scp_list_client", "cacert,cakey," "clientcert,clientkey"
        )
    ...
    def conn_setup(self, server_setup=True, client_setup=True):
        ...
        if self.CERTTOOL == "/bin/true":
            raise ConnToolNotFoundError(
                "certtool", "certtool executable not set or found on path."
            )

        # support build multiple CAs with different CA CN
        build_CA(self.tmp_dir, self.ca_cn, self.CERTTOOL, self.credential_dict)
    ...
    def server_setup(self, on_local=False):
        ...
        if self.ca_cakey_path:
            cacert_path = os.path.join(
                self.ca_cakey_path, self.credential_dict["cacert"]
            )
        ...
        # build a server key.
        build_server_key(
            tmp_dir, server_cn, ip_addr, self.CERTTOOL, self.credential_dict, on_local
        )
        ...
        if scp_new_cacert == "no":
            params_scp = {"serverkey": serverkey_path, "servercert": servercert_path}
        else:
        ...
        for key in scp_dict:
            ...
            try:
                ...
                raise ConnSCPError(
                    "AdminHost", local_path, server_ip, remote_path, detail
                )
        ...
        if self.qemu_chardev_tls:
            pattern2repl = {r".*chardev_tls\s*=\s*.*": "chardev_tls = 1"}
            operate_qemuconf.sub_else_add(pattern2repl)
            pattern2repl = {
                r".*chardev_tls_x509_cert_dir\s*="
                r"\s*\"\/etc\/pki\/libvirt-chardev\s*\".*": 'chardev_tls_x509_cert_dir = "/etc/pki/libvirt-chardev"'
            }
            operate_qemuconf.sub_else_add(pattern2repl)
        ...
        if tls_port != "16514":
            if libvirt_version.version_compare(5, 6, 0, server_session):
                pattern_to_repl = {
                    r".*ListenStream\s*=\s*.*": "ListenStream=%s" % tls_port
                }
        ...
        if isinstance(tls_allowed_dn_list, list):
            pattern_to_repl = {
                r".*tls_allowed_dn_list\s*=\s*.*": "tls_allowed_dn_list=%s"
                % tls_allowed_dn_list
            }
            operate_libvirtdconf.sub_else_add(pattern_to_repl)
        ...
        if pki_path:
            ...
            for cert_name in cert_path_dict:
                ...
                pattern_to_repl[r".*%s\s*=.*" % (cert_name)] = '%s="%s"' % (
                    cert_name,
                    abs_cert_file,
                )
            operate_libvirtdconf.sub_else_add(pattern_to_repl)
        ...
        if tls_verify_cert == "no":
            pattern_to_repl = {
                r".*tls_no_verify_certificate\s*=\s*.*": "tls_no_verify_certificate=1"
            }
            operate_libvirtdconf.sub_else_add(pattern_to_repl)
        ...
        if tls_sanity_cert == "no":
            pattern_to_repl = {
                r".*tls_no_sanity_certificate\s*=\s*.*": "tls_no_sanity_certificate=1"
            }
            operate_libvirtdconf.sub_else_add(pattern_to_repl)
        ...
        if not on_local:
            ...
            hostname = client_runner.run(
                "hostname", ignore_status=True
            ).stdout_text.strip()
            pattern_to_repl = {
                r".*%s.*" % self.client_ip: "%s %s" % (self.client_ip, hostname)
            }
    ...
    def client_setup(self):
        ...
        # build a client key.
        build_client_key(tmp_dir, self.client_cn, self.CERTTOOL, self.credential_dict)
        ...
        if scp_new_cacert == "no":
            params_scp = {"clientkey": clientkey_path, "clientcert": clientcert_path}
        else:
            params_scp = {
                "cacert": cacert_path,
                "cakey": cakey_path,
        ...
        for key in scp_dict:
            ...
            try:
                ...
                raise ConnSCPError(
                    "AdminHost", local_path, client_ip, remote_path, detail
                )
        ...
        if self.special_cn == "yes":
            pattern_to_repl = {
                r".*%s.*" % self.server_ip: "%s %s" % (self.server_ip, self.server_cn)
            }
        else:
            pattern_to_repl = {
                r".*%s.*" % self.server_ip: "%s %s" % (self.server_ip, hostname)
            }
        self.client_hosts.sub_else_add(pattern_to_repl)


def build_client_key(
    tmp_dir, client_cn="TLSClient", certtool="certtool", credential_dict=None
):
    """
    (1).initialization for variables.
    ...
    if private_key_password:
        cmd = "%s --generate-privkey --password %s > %s" % (
            certtool,
    ...
    cmd = (
        "%s --generate-certificate --load-privkey %s \
...
def build_CA(tmp_dir, cn="AUTOTEST.VIRT", certtool="certtool", credential_dict=None):
    """
    ...
    cmd = (
        "%s --generate-self-signed --load-privkey %s\
           --template %s --outfile %s"
...
class UNIXConnection(ConnectionBase):
    ...
    def __init__(self, *args, **dargs):
        ...
        init_dict["unix_sock_ro_perms"] = init_dict.get("unix_sock_ro_perms", "")
        init_dict["unix_sock_rw_perms"] = init_dict.get("unix_sock_rw_perms", "")
        init_dict["restart_libvirtd"] = init_dict.get("restart_libvirtd", "yes")
        init_dict["sasl_allowed_username_list"] = init_dict.get(
            "sasl_allowed_username_list"
        )
        init_dict["traditional_mode"] = init_dict.get("traditional_mode", "no")
        ...
        if init_dict["run_on_remote"] == "yes":
            ...
            LOG.debug(
                "Unix Connection will be setup on remote host: {}.".format(session_ip)
            )
        else:
            ...
            LOG.debug(
                "Unix Connection will be setup on local host: {}.".format(session_ip)
            )
    ...
    def conn_setup(self):
        ...
        if auth_unix_ro:
            pattern_to_repl = {
                r".*auth_unix_ro\s*=.*": 'auth_unix_ro="%s"' % auth_unix_ro
            }
        ...
        if auth_unix_rw:
            pattern_to_repl = {
                r".*auth_unix_rw\s*=.*": 'auth_unix_rw="%s"' % auth_unix_rw
            }
            self.client_libvirtdconf.sub_else_add(pattern_to_repl)
        ...
        if unix_sock_group != "libvirt":
            pattern_to_repl = {
                r".*unix_sock_group\s*=.*": 'unix_sock_group="%s"' % unix_sock_group
            }
            self.client_libvirtdconf.sub_else_add(pattern_to_repl)
        ...
        if unix_sock_dir != "/var/run/libvirt":
            if self.libvirt_ver:
                pattern_to_repl = {
                    r".*ListenStream\s*=.*": "ListenStream=%s/libvirt-sock"
                    % unix_sock_dir
                }
                self.client_libvirtd_socket.sub_else_add(pattern_to_repl)
            else:
                pattern_to_repl = {
                    r".*unix_sock_dir\s*=.*": 'unix_sock_dir="%s"' % unix_sock_dir
                }
                self.client_libvirtdconf.sub_else_add(pattern_to_repl)
        ...
        if access_drivers != ["polkit"]:
            pattern_to_repl = {
                r".*access_drivers\s*=.*": 'access_drivers="%s"' % access_drivers
            }
            self.client_libvirtdconf.sub_else_add(pattern_to_repl)
        ...
        if unix_sock_ro_perms:
            pattern_to_repl = {
                r".*unix_sock_ro_perms\s*=.*": 'unix_sock_ro_perms="%s"'
                % unix_sock_ro_perms
            }
            self.client_libvirtdconf.sub_else_add(pattern_to_repl)
        ...
        if unix_sock_rw_perms:
            if self.libvirt_ver and traditional_mode == "no":
                pattern_to_repl = {
                    r".*SocketMode\s*=.*": "SocketMode=%s" % unix_sock_rw_perms
                }
                self.client_libvirtd_socket.sub_else_add(pattern_to_repl)
            else:
                pattern_to_repl = {
                    r".*unix_sock_rw_perms\s*=.*": 'unix_sock_rw_perms="%s"'
                    % unix_sock_rw_perms
                }
                self.client_libvirtdconf.sub_else_add(pattern_to_repl)
        ...
        if sasl_allowed_username_list:
            pattern_to_repl = {
                r".*sasl_allowed_username_list\s*=.*": "sasl_allowed_username_list=%s"
                % sasl_allowed_username_list
            }
            self.client_libvirtdconf.sub_else_add(pattern_to_repl)
        sessions = self.session_creator()
        ...
        for current_session in sessions:
            if sasl_type in pkg_dict:
                if not utils_package.package_install(
                    pkg_dict[sasl_type], session=current_session
                ):
                    raise ConnToolNotFoundError(
                        pkg_dict[sasl_type], "Failed to install package on " "host."
                    )
        ...
        if self.sasl_type == "gssapi" and auth_unix_rw == "sasl":
            ...
            hostname = remote_runner.run(
                "hostname", ignore_status=True
            ).stdout_text.strip()
            pattern_to_repl = {
                r".*127.0.0.1\s*.*": "127.0.0.1    %s localhost localhost.localdomain "
                "localhost4 localhost4.localdomain6" % hostname,
                r".*::1\s*.*": "::1    %s localhost localhost.localdomain "
        ...
        if self.sasl_type == "plain":
            pattern_to_repl = {r"^mech_list\s*:\s*.*": "mech_list: %s" % self.sasl_type}
        else:
            pattern_to_repl = {
                r".*mech_list\s*:\s*.*": "mech_list: %s" % self.sasl_type,
                r".*keytab\s*:\s*.*": keytab,
...
class UNIXSocketConnection(ConnectionBase):
    ...
    def __init__(self, *args, **dargs):
        ...
        init_dict["desturi_sock_listen"] = init_dict.get(
            "desturi_sock_listen", "/tmp/desturi-socket"
        )
        ...
        self.pmsocat_path = os.path.join(
            data_dir.get_shared_dir(), "scripts/pmsocat", "pmsocat36.py"
        )
        self.remote_pmsocat_path = os.path.join("/tmp", "pmsocat36.py")
        remote_old.scp_to_remote(
            self.server_ip,
            "22",
            self.server_user,
            self.server_pwd,
        ...
        self.qemu_kvm_pp_path = os.path.join(
            data_dir.get_shared_dir(), "scripts/pmsocat", "qemu-kvm.pp"
        )
    ...
    def listen_on_unix_socket(self, session, pmsocat_path, destination_ip):
        ...
        """
        for socks in [
            [self.desturi_sock_listen, self.desturi_port],
            [self.migrateuri_sock, self.migrateuri_port],
            [self.disks_uri_sock, self.disks_uri_port],
        ]:
            cmd = (
                "{} unix2tcp -c system_u:system_r:svirt_socket_t:s0 {} {} "
                "{} &".format(pmsocat_path, socks[0], destination_ip, socks[1])
            )
            status, output = session.cmd_status_output(cmd)
            if status:
    ...
    def install_qemu_kvm_pp(self, session, qemu_kvm_pp_path, timeout=240):
        ...
        if status:
            ...
            if status:
                LOG.error(
                    "Unable to active SELinux policy module - "
    ...
    def conn_setup(self):
        ...
        self.listen_on_unix_socket(unix2tcp_session, src_pmsocat_path, destination_ip)
...
def update_crypto_policy(value="DEFAULT", verbose=True, ignore_status=True, shell=True):
    ...
    cmd_res = process.run(
        cmd, verbose=verbose, ignore_status=ignore_status, shell=shell
    )

    cmd_get_policy = "update-crypto-policies --show"
    val = process.run(
        cmd_get_policy, verbose=False, ignore_status=True, shell=True
    ).stdout_text
