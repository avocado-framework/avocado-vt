...
class VMStressEvents:
    def __init__(self, params, env):
        ...
        """
        self.host_cpu_list = (
            cpu.online_list() if hasattr(cpu, "online_list") else cpu.cpu_online_list()
        )
    ...
    def run_threads(self):
        ...
        for event in self.host_events:
            self.threads.append(
                threading.Thread(target=self.host_stress_event, args=(event,))
            )
        ...
        for itr in range(self.iterations):
            if "vcpupin" in event:
                for vcpu in range(current_vcpu):
                    result = virsh.vcpupin(
                        vm.name, vcpu, random.choice(self.host_cpu_list), **dargs
                    )
                ...
                if not self.ignore_status:
                    ...
                    cpuutil.check_vcpu_value(vm, exp_vcpu, option="--live")
                time.sleep(self.event_sleep_time)
                result = virsh.setvcpus(vm.name, current_vcpu, "--live", **dargs)
                if not self.ignore_status:
                    libvirt.check_exit_status(result)
                    exp_vcpu = {
                        "max_config": max_vcpu,
                ...
                for iface_num in range(int(iface_num)):
                    ...
                    LOG.debug(
                        "VM name: %s , Options for Network attach: %s", vm.name, options
                    )
                    ret = virsh.attach_interface(vm.name, options, ignore_status=True)
                    time.sleep(self.event_sleep_time)
                    if not self.ignore_status:
                    ...
                    if detach_option:
                        ...
                        ret = virsh.detach_interface(
                            vm.name, options, ignore_status=True
                        )
                        if not self.ignore_status:
                            libvirt.check_exit_status(ret)
                ...
                for disk_num in range(len(device_source_names)):
                    ...
                    disk_name = os.path.join(
                        path, vm.name, device_source_names[disk_num]
                    )
                    device_source = libvirt.create_local_disk(
                        disk_type, disk_name, disk_size, disk_format=disk_format
                    )
                    ...
                    if detach_option:
                        ret = virsh.detach_disk(
                            vm.name, device_target[disk_num], extra=detach_option
                        )
                        if not self.ignore_status:
                            libvirt.check_exit_status(ret)
    ...
    def host_stress_event(self, event):
        ...
        for itr in range(self.host_iterations):
            if "cpu_freq_governor" in event:
                ...
                online_count = (
                    cpu.online_count()
                    if hasattr(cpu, "online_count")
                    else cpu.online_cpus_count()
                )
...
def install_stressapptest(vm):
    ...
    if not utils_package.package_install(name, session, timeout=300):
        raise exceptions.TestError(
            "Installation of packages %s in guest " "failed" % name
        )

    app_repo = "git clone https://github.com/stressapptest/" "stressapptest.git"
    stressapptest_install_cmd = (
        "rm -rf stressapptest "
        "&& %s"
        " && cd stressapptest "
        "&& ./configure "
