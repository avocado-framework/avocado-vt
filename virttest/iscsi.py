...
class Fileio(Backstore):
    ...
    def delete_backstore(self):
        ...
        if "No such path" not in output:
            ...
            LOG.info("No backstore %s found. Nothing to do!" % self.device)
    ...
    def export_target(self):
        ...
        if not re.findall("%s$" % self.target, output, re.M):
            ...
            for name, obj in self.devices.items():
                ...
                cmd += "create /backstores/%s/%s" % (obj.iscsi_backend, obj.device)
                output = process.run(cmd).stdout_text
                luns = re.findall(r"Created LUN (\d+).", output)
                if not luns:
                    raise exceptions.TestFail("Failed to create lun. (%s)" % output)
                self.luns_in_target.update({luns[0]: obj.device})
...
class MultiPathsIscsiLIOManager(object):
    ...
    def __init__(self, params, root_dir):
        ...
        self._iscsi_targets = {}
        for target in targets:
            val = params.object_params(target)["iscsi_target"]
            self._iscsi_targets[params.get(target)] = ast.literal_eval(val)
        bss = params.objects("iscsi_target_backstores")
        self._iscsi_target_bss = {}
        for bs in bss:
            val = params.object_params(bs)["iscsi_target_backstore"]
        ...
        for name, cfg in self._iscsi_targets.items():
            ...
            for bs_name in self._iscsi_targets[name]["binding_basckstores"]:
    ...
    def export_target(self, targets=None):
        ...
        for target in targets:
            for name in self._iscsi_targets[target]["binding_basckstores"]:
                self._bss_object[name].create_backstore()
            self._targets_object[target].export_target()
            process.run("targetcli / saveconfig")

    def query_targets(self, emulated_image=None):
        """
        ...
        for target in targets:
            cmd = "targetcli ls /iscsi/%s/tpg1/luns" % target
            luns_info = process.run(cmd).stdout_text
            if (not emulated_image) or (
                emulated_image and emulated_image + ")" in luns_info
            ):
    ...
    def cleanup(self, targets=None, confirmed=False):
        """
        Clean up env after iscsi used.
        ...
        for target in targets:
            for output in outputs:
                ...
                for i in range(len(m)):
                    ...
                    output = process.run(cmd).stdout_text
                    emu_img = re.search(r"\([^)]*/([^/)\s]+/[^/)\s]+)\)", output).group(
                        1
                    )
                    device_name.append(
                        {
