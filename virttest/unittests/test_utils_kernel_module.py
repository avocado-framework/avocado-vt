...
some_module_val = "N"
some_module_config = {some_module_param: some_module_val, "halt_poll_ns_shrink": "0"}
...
class TestReloadModule(unittest.TestCase):
    ...
    def test_tc6_partial(self, *mocks):
        ...
        self.handler.reload_module(
            force=False, params=some_module_params + " key=value"
        )
        self.assertLoadedWith("key=value")
...
@mock.patch.object(
    utils_kernel_module.KernelModuleHandler, "__init__", return_value=None
)
@mock.patch.object(
...
class TestReload(unittest.TestCase):
    ...
    def test_empty_params_1(self, *mocks):
        self.assertIsNone(
            utils_kernel_module.reload(some_module_name, force=False, params="")
        )

    def test_empty_params_2(self, *mocks):
        self.assertIsNotNone(
            utils_kernel_module.reload(some_module_name, force=True, params="")
        )

    def test_non_empty_params_1(self, *mocks):
        self.assertIsNotNone(
            utils_kernel_module.reload(
                some_module_name, force=False, params=some_module_params
            )
        )
    ...
    def test_non_empty_params_2(self, *mocks):
        self.assertIsNotNone(
            utils_kernel_module.reload(
                some_module_name, force=True, params=some_module_params
            )
        )
