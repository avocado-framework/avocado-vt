...
def is_mount(src, dst=None, fstype=None, options=None, verbose=False, session=None):
    ...
    command_parts = [
        part
        for part in [
            f"-S {src}" if src is not None else None,
            f"-T {dst}" if dst is not None else None,
            f"-t {fstype}" if fstype is not None else None,
        ]
        if part is not None
    ]
    mount_str = " ".join(command_parts)
    mount_check_cmd = "findmnt -J %s" % mount_str
    ...
    if mount_result:
        mount_result_dict = json.loads(mount_result)
        if options:
            options = options.split(",")
            options_result = mount_result_dict["filesystems"][0]["options"].split(",")
            for op in options:
                if op not in options_result:
                    if verbose:
                        LOG.info("%s is not mounted with given" " option %s", src, op)
                    return False
        if verbose:
