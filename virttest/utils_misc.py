...
def verify_secure_host(params):
    ...
    module_path = "%s_module_path" % secure_guest_type
    secure_module_path = params[module_path]
    LOG.info(f"Verifying cvm {secure_guest_type} capability enablement on host")
    try:
        if os.path.exists(secure_module_path):
        ...
        raise exceptions.TestCancel(
            f"Failed to read {secure_module_path}: {str(e)}"
        ) from e


def verify_secure_guest(session, params, vm):
    """
    ...
    guest_check_cmd = params["%s_guest_check" % secure_guest_type]
    try:
