...
def format_windows_disk(session, dids, mountpoint=[], size=None,
    ...
    set_rw_cmd = wrap_windows_cmd(set_rw_cmd)
    online_cmd = ' echo online disk >> disk '
    online_cmd = wrap_windows_cmd(online_cmd)
    if size:
    ...
    mkpart_cmd = wrap_windows_cmd(mkpart_cmd)
    ...
    format_cmd = wrap_windows_cmd(format_cmd)
    mountpoint_list = []
    for did in dids:
...
def format_linux_disk(session, did, all_disks_did=None,
                      partition=False, n_partitions=1,
    ...
    :param all_disks_did: all disk did.
                          e.g. [kname, size, type, serial, wwn]
                          get_linux_disks(session, partition).values()
    ...
    for disk_did in all_disks_did:
        ...
        return mountpoint or mountpoint_list
    logging.error("Not found disk '%s' in for loop." % did)
    return []
...
def format_guest_disk(session, dids, ostype, partition=False, n_partitions=1,
    ...
    value = []
    all_disks_did = sorted(get_linux_disks(session, partition).values(),
                           key=lambda x: x[0])
    for did in dids:
        p = format_linux_disk(session, did, all_disks_did,
...
def delete_linux_partition(session, kname):
    ...
    """
    session.cmd_output_safe("partprobe /dev/%s" % kname, timeout=600)
    list_disk_cmd = "lsblk -o KNAME,MOUNTPOINT"
    output = session.cmd_output_safe(list_disk_cmd, timeout=600)
    ...
    for line in output:
        ...
        if partition:
            if "/" in line.split()[-1]:
                session.cmd_output_safe("umount %s" % line.split()[-1], timeout=600)
            logging.info("remove partition %s on %s" % (partition[0], kname))
            session.cmd_output_safe(rm_cmd % (kname, partition[0]), timeout=600)
            session.cmd_output_safe("partprobe /dev/%s" % kname, timeout=600)
...
def delete_windows_partition(session, did):
    ...
    clean_cmd = wrap_windows_cmd(clean_cmd)
    session.cmd_output_safe(clean_cmd % did, timeout=600)
