...
def get_free_disk(session, mount):
    ...
    if re.match(r"[a-zA-Z]:", mount):
        cmd = ('powershell -command "Get-CimInstance Win32_LogicalDisk | Where-Object {$_.DeviceID -eq \'%s\'}'
               ' | Select-Object FreeSpace"') % mount
...
def get_win_disk_vol(session, condition="VolumeName='WIN_UTILS'"):
    ...
    cmd = (
        ...
        ' | Select-Object DeviceID"') % (c_name, c_value)
