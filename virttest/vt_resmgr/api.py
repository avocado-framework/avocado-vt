...
volume_config = define_resource_config("volume", volume_params, node_tags)
...
def register_resouce_pool(pool_config):
    ...
    :rtype: string
    """
    pool_id = vt_resmgr.register_pool(pool_config)
    if pool_id is None:
...
def define_resource_config(resource_type, resource_params, node_tags):
    """
    ...
    :param node_tags: The worker node tags, defined with cartesian param
                      "nodes". It means this resource should be accessed
                      from these worker nodes
    :type node_tags: list
    ...
    :rtype: dict
    """
    pool_id = vt_resmgr.select_pool(resource_type, resource_params, node_tags)
    if pool_id is None:
        raise ResourceNotAvailable()
    pool = vt_resmgr.get_pool_by_id(pool_id)
    return pool.define_resource_config(resource_type,
                                       resource_params,
                                       node_tags)
...
def query_resource(resource_id, request=None):
    ...
    :param request: The query content, format:
                    ...
                    Examples:
                      ...
                      spec.size
    :type request: string
    :return: The resource's configuration, it can be either the whole
             one or a snippet
    :rtype: dict
    """
    pool = vt_resmgr.get_pool_by_resource(resource_id)
...
def update_resource(resource_id, config):
    ...
    Supported commands:
      ...
      'sync': Sync up the resource configuration. Some items of the
              ...
              nodes, e.g. capacity, use sync command to sync up these items
