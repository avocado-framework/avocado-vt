...
class _TapPort(_PortResource):
    ...
    def bind(self, arguments):
        ...
        for node_name in nodes:
            if not self.resource_bindings.get(node_name):
                LOG.info(
                    f"Bind the tap port {self.resource_id} to node {node_name}")
    ...
    def unbind(self, arguments):
        ...
        for node_name in nodes:
            ...
            if backing_id:
                ...
                LOG.info(
                    f"The tap port {self.resource_id} has already unbound from {node_name}")

    def sync(self, arguments):
        LOG.debug(
            f"Sync up the configuration of the tap port {self.resource_id}")
        ...
        self.resource_spec["fds"] = config["spec"]["fds"]
        self.resource_spec["ifname"] = config["spec"]["ifname"]

    def allocate(self, arguments):
        node_name, backing_id = list(self.resource_bindings.items())[0]
        node = cluster.get_node(node_name)
        LOG.debug(
            f"Allocate the tap port {self.resource_id} from {node_name}.")
        node = cluster.get_node(node_name)
        r, o = node.proxy.resource.update_resource_by_backing(
    ...
    def release(self, arguments):
        ...
        self.resource_spec["fds"] = None
        self.resource_spec["ifname"] = None
