...
class _ResourcePool(ABC):
    ...
    def get_binding_nodes(self, vm_node_tag=None):
        workers = list()

        if vm_node_tag:
            # The resource is owned by a vm and the vm is to be running on the
            # vm_node, then bind the resource to a backing on that node
            vm_node = cluster.get_node_by_tag(vm_node_tag)
            if not vm_node:
                raise ValueError(f"Unknown vm node tag {vm_node_tag}")
            elif vm_node.name in self.attaching_nodes:
                workers.append(vm_node.name)
        else:
            # The resource is not owned by any vm, e.g. qemu image testing
            # without a vm, then bind the resource to the nodes both in the
            # pool access nodes and partition nodes
            node_names = [node.name for node in cluster.partition.nodes]
            workers.extend(list(set(self.attaching_nodes).intersection(set(node_names))))
    ...
    def define_resource_config(self, resource_name, resource_type, resource_params):
        ...
        # Update the resource bindings, the node names can never be empty
        # because this pool was selected to create the resource
        vm_node_tag = resource_params.get("vm_node")
        bindings = {n: None for n in self.get_binding_nodes(vm_node_tag)}
    ...
    def create_resource_object_from(self, resource):
        ...
        if resource_class is None:
            raise RuntimeError(
                f"Unsupported {resource.resource_type} resource for pool {self.pool_id}"
            )
