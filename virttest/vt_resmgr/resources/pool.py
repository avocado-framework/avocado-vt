...
class _ResourcePool(ABC):
    ...
    def create_resource_object_by(self, source_resource):
        ...
        res_cls = self.get_resource_class(source_resource.type)
        if not res_cls:
            raise ValueError(
        ...
        resource_config["meta"]["pool"] = self.uuid
        new_resource = res_cls(resource_config)
        new_resource.create_object()
    ...
    def clone_resource(self, resource_id, arguments, node):
        """
        ...
        if node:
            self.check_nodes_accessible([node])
    ...
    def update_resource(self, resource_id, command, arguments, node):
        resource = self.resources.get(resource_id)
