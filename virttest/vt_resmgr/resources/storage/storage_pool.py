...
class _StoragePool(_ResourcePool):
    ...
    def _check_nodes_access(self, resource_params):
        ...
        if vm_node_tag:
            ...
            if vm_node_name not in self.accessing_nodes:
                return False
        else:
            # Check if the pool can be accessed by one of the partition nodes
            node_names = [node.name for node in cluster.get_partition().nodes]
            if not set(self.accessing_nodes).intersection(set(node_names)):
                return False
    ...
    def meet_resource_request(self, resource_type, resource_params):
        ...
        if pool_tag:
            pool_id = cluster.get_partition().pools.get(pool_tag)
