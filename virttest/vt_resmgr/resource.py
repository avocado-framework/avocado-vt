...
class _ResourceBinding(object):
    ...
    def create_backing(self, resource_config, need_allocate=False):
        ...
        self._backing_id = node.proxy.create_backing(resource_config, need_allocate)
...
class _Resource(ABC):
    ...
    def resource_info(self):
        """
        Static resource configurations as well as the dynamic ones,
        the former comes from users' settings while the latter comes
        from the allocation accessed via the worker nodes
        """
        raise NotImplemented

    @abstractmethod
    def info_bindings(self):
        raise NotImplemented
