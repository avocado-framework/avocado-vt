...
class _VTResourceManager(object):
    ...
    def _dump_data(self, data):
        ...
        self.default_pools = data.get("default_pools", list())
    ...
    def default_pools(self, pools):
        self._default_pools = pools
    ...
    def setup(self, resource_pools_params):
        ...
        # e.g. if no filesystem pool is defined we need to register
        # a default one
        default_pools_nodes = {
            "filesystem": set(),
            # "switch": set(),
        ...
        for category, params in resource_pools_params.items():
            for pool_name, pool_params in params.items():
                ...
                # This pool is used as the default one, when the test case
                # doesn't specifiy a pool, this one will be used
                if pool_params.get("default") == "yes":
                    self.default_pools.append(pool_id)
    ...
    def create_pool_object(self, pool_config):
        ...
        Note: Currently the users need to setup the pool before testing
    ...
    def destroy_pool_object(self, pool_id):
        """
        The pool should be detached from all worker nodes before destroying it
    ...
    def attach_pool(self, pool_id):
        """
        Attach the resource pool to the worker nodes, where the pool can be
        accessed. The accessiable worker nodes are defined by the pool's meta
    ...
    def detach_pool(self, pool_id):
        """
        Detach the pool from its accessiable worker nodes
    ...
    def get_pool_info(self, pool_id, request=None):
        ...
        """
        pool = self.get_pool_by_id(pool_id)
        return pool.get_info(request)

    def get_all_pools(self):
        """
        Get all resource pools' uuid list

        :return: The pools' uuid list
        :rtype: list
        """
