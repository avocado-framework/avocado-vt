...
class _VTResourceManager(object):
    ...
    def create_resource_object_from(self, resource_id, pool_id=None):
        ...
        Take a resource as the input, create a new one from the specified pool
        The pool needs not be the same one where the resource comes from

        :param resource_id: The resource uuid used to base the new resource on
        :type resource_id: string
        :param poo_id: The resource pool uuid
        :type resource_id: string
        ...
        new_resource_id = target_pool.create_resource_object_from(resource)
