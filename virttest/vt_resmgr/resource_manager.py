...
class _VTResourceManager(object):
    ...
    def create_resource_object_by(self, source_resource_id, target_pool_id=None):
        """
    ...
    def bind_resource_object(self, resource_id, node_names=None):
        ...
        Note bind the resource to the backings on the worker nodes that can
        not only access the resource pool but also are in the partition nodes
        if node_names is not set.

        :param resource_id: The resource uuid
        :type resource_id: string
        :param node_names: The node names defined in the param 'nodes', if it's
                           not set, use all the partition nodes
    ...
    def clone_resource(self, resource_id, arguments=None):
        ...
        :rtype: string
        """
        node_names = arguments.pop("nodes", list()) if arguments else list()
        nodes = self._get_nodes(node_names)
        pool = self._get_pool_by_resource(resource_id)
        return pool.clone_resource(resource_id, arguments, nodes)
    ...
    def update_resource(self, resource_id, command, arguments=None):
        ...
        :param command: The command name.
              ...
              allocate: Allocate a resource.
               release: Release a resource. A resource cannot be released until
                ...
                resize: Resize the volume
        :type command: string
        :param arguments: The command's arguments.
                          The supported arguments for all commands:
                          nodes: List of node names defined in the param nodes,
                                 i.e. the command will be run on these nodes.
        :type arguments: dict
        :return:
        :rtype:
        Examples: nodes = 'node1 node2'
                  ...
                  Release a resource on node1
                    command = 'release', arguments = {'nodes': ['node1']}
        """
        node_names = arguments.pop("nodes", list()) if arguments else list()
        nodes = self._get_nodes(node_names)
        pool = self._get_pool_by_resource(resource_id)
        return pool.update_resource(resource_id, command, arguments, nodes)
