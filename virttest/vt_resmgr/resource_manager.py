...
class _VTResourceManager(object):
    ...
    def select_pool(self, resource_type, resource_params):
        ...
        1. Specify the xxx_pool_selectors params explicitly (Recommended), e.g.
           ...
           in which 'access.nodes' is the worker nodes where image1 can be
           accessed, all partition nodes will be used if it's not set
        2. If it's not set, then the other params, e.g. storage_type,  will be
           used to generate the selectors, besides, all the nodes in the
           partition will be considered as the access worker nodes

        :param resource_type: The resource type, it can be implied, e.g. the
                              image's storage resource type is the "volume",
                              Supported types: volume, port
    ...
    def create_resource_object_by(self, resource_id, pool_id=None, access_nodes=None):
        """
        Create a new resource object by an existing resource object.
        The pool needs NOT be the same one where the resource comes from

        :param resource_id: The source resource uuid
        :type resource_id: string
        :param poo_id: The new resource's pool uuid, use the source resource's
                       pool id if it is not set
        :type resource_id: string
        :param access_nodes: The worker node names defined by 'nodes' where
                             the resource is accessed, use the source
                             resource's access nodes if it is not set
        ...
        resource_type = resource.resource_meta["type"]
        if not target_pool.get_resource_class(resource_type):
            raise ValueError("The pool {pool_id} doesn't support a {resource_type} resource")
        ...
        if access_nodes:
            ...
            if not set(node_names).issubset(set(target_pool.attaching_nodes)):
                raise ValueError(
                    f"Not all nodes({node_names}) can access the pool"
                )
    ...
    def update_resource(self, resource_id, config):
        ...
        Supported commands:
          ...
          'move': Move the resource from one pool to another, it only change
                  the resource state without create a new resource object
        ...
        if access_nodes:
            node_names = [cluster.get_node_by_tag(t).name for t in access_nodes]
            if not set(node_names).issubset(set(pool.attaching_nodes)):
                raise ValueError(
                    f"Not all nodes({node_names}) can access the pool"
                )
