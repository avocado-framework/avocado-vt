...
def get_public_key(client_user=None):
    ...
    if client_user:
        if os.environ.get("USER") not in ("root", client_user):
            raise RuntimeError(
                "Can not set ssh-key for OTHER user using"
    ...
    has_dsa_keypair = os.path.isfile(dsa_public_key_path) and os.path.isfile(
        dsa_private_key_path
    )
    has_rsa_keypair = os.path.isfile(rsa_public_key_path) and os.path.isfile(
...
def get_remote_public_key(session, public_key="rsa"):
    ...
    if has_dsa_keypair and public_key == "dsa":
        ...
        LOG.info(
            "Neither RSA nor DSA keypair found, "
        ...
        if public_key == "dsa":
            ...
            public_key_path = dsa_public_key_path
        session.cmd('ssh-keygen -t %s -q -N "" -f %s' % (public_key, key_path))
...
def setup_ssh_key(hostname, user, password, port=22, client_user=None):
    ...
    :type client_user: str
    """
    LOG.debug("Performing SSH key setup on %s:%d as %s." % (hostname, port, user))
...
def setup_remote_ssh_key(
    ...
    """
    LOG.debug("Performing SSH key setup on %s:%d as %s." % (hostname1, port, user1))

    try:
        session1 = remote.remote_login(
            client="ssh",
            host=hostname1,
            port=port,
            username=user1,
        ...
        session2.cmd_output("echo '%s' >> ~/.ssh/authorized_keys; " % public_key)
...
def setup_remote_known_hosts_file(client_ip, server_ip, server_user, server_pwd):
    """
    ...
    """
    LOG.debug(
        "Performing known_hosts file setup on %s from %s." % (server_ip, client_ip)
    )
    abs_path = ""
    try:
