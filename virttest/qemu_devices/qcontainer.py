    ...
    def images_define_by_variables(
        ...
        for dev in devices:
            if self.is_dev_iothread_vq_supported(dev):
                if isinstance(dev.parent_bus, dict):
                    dev.parent_bus = (dev.parent_bus,)
                if num_queues:
                    if dev.params.get("driver") in ("virtio-blk-pci",):
                        dev.set_param("num-queues", num_queues)
                    elif dev.params.get("driver") in ("virtio-scsi-pci",):
                        dev.set_param("num_queues", num_queues)
