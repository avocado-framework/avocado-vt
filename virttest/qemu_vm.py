...
class VM(virt_vm.BaseVM):
    ...
    def define(self, name, params, root_dir, mac_source):
        ...
        if node:
            vm_config = vmm_api.define_instance_config(name, params)
    ...
    def start(self, timeout=120):
        ...
        if self._instance_id:
            vmm_api.vmm.start_instance(self._instance_id, timeout)
            self.process = _ProcessProxy(self._instance_id)
            # self.serial_console = _SerialConsoleProxy(self._instance_id)
    ...
    def undefine(self, free_mac_addresses=True):
        if hasattr(self, "_instance_id") and self._instance_id:
            ...
            # FIXME: close the process proxy, do nothing acutally.
            if self.process:
                self.process.close()
        else:
    ...
    def migrate(
        ...
        if not local:
            ...
            if protocol in ("tcp", "rdma", "x-rdma"):
                ...
                mig_uri["address"] = migration_file
            elif protocol == "exec":
                # FIXME:
                mig_uri["address"] = migration_exec_cmd_src
            elif protocol == "fd":
                # FIXME:
                mig_uri["address"] = migration_fd
