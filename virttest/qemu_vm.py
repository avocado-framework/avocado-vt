...
class VM(virt_vm.BaseVM):
        ...
        if vcpu_maxcpus != 0:
            ...
            if smp_values.count(0) == 1:
                ...
                if vcpu_maxcpus == 1 and max(smp_values) < 2:
                    vcpu_sockets = (
                        vcpu_dies
                    ) = vcpu_clusters = vcpu_cores = vcpu_threads = 1
            ...
            if smp == 0:
                ...
                if vcpu_prefer_sockets:
                    if vcpu_sockets == 0:
                        ...
                        vcpu_sockets = (
                            smp
                            // (vcpu_cores * vcpu_threads * vcpu_clusters * vcpu_dies)
                            or 1
                        )
                    elif vcpu_cores == 0:
                        vcpu_threads = vcpu_threads or 1
                        vcpu_cores = (
                            smp
                            // (vcpu_sockets * vcpu_threads * vcpu_clusters * vcpu_dies)
                            or 1
                        )
                else:
                ...
                if vcpu_threads == 0:
                    ...
                    vcpu_threads = (
                        smp // (vcpu_cores * vcpu_sockets * vcpu_clusters * vcpu_dies)
                        or 1
                    )

            smp = (
                smp
                or vcpu_sockets * vcpu_dies * vcpu_clusters * vcpu_cores * vcpu_threads
            )
    ...
    def define(self, name=None, params=None, root_dir=None, mac_source=None):
        ...
        # FIXME: Raise the VMDefinedError when all use cases no longer use
        #        "stop" to destroy the VM and then define a new one.
        if self.state is "stopped":
            LOG.warning(
                "Stopping the VM and then redefine it will be deprecated, "
                "Please consider undefined the VM before defining it to "
                "ensure proper cleanup."
            )
        elif self.state not in ["", "undefined"] and not reuse:
            raise virt_vm.VMDefineError(self.name, "VM has already been defined")
        ...
        try:
            ...
            raise virt_vm.VMDefineError(
                self.name,
                "Error occurred while executing executing "
                "make_create_command(). Check the log for traceback.",
            )
    ...
    def start(self, timeout=120):
        ...
        if self.state is "undefined" or self.devices is None:
            raise virt_vm.VMStartError(
                self.name, "The VM does not defined, " "please define the VM first"
            )
        elif self.state in ["running", "paused"]:
            LOG.warning(f'The VM "{self.name}" has been launched')
            return
