...
class ThSendCheck(Thread):
    ...
    def run(self):
        ...
        _err_msg_exception = (
            "ThSendCheck " + str(self.getName()) + ": Got " "exception %s, continuing"
        )
        _err_msg_disconnect = (
            "ThSendCheck " + str(self.getName()) + ": Port "
            "disconnected, waiting for new port."
        )
        _err_msg_reconnect = (
            "ThSendCheck " + str(self.getName()) + ": Port " "reconnected, continuing."
        )
        too_much_data = False
        if self.reduced_set:
            rand_a = 65
            ...
            if ret[1]:
                ...
                while not self.exitevent.isSet() and self.idx < target:
                    try:
                        ...
                        LOG.debug(
                            "ThSendCheck %s: Broken pipe " ", reconnecting. ",
                            self.getName(),
                        )
                        attempt = 10
                        while attempt > 1 and not self.exitevent.isSet():
                            ...
                            while not (
                                self.exitevent.isSet() or self.migrate_event.wait(1)
                            ):
                                pass
                            if self.exitevent.isSet():
                                break
...
class ThRecvCheck(Thread):
    ...
    def run_normal(self):
        ...
        _err_msg_exception = (
            "ThRecvCheck " + str(self.getName()) + ": Got " "exception %s, continuing"
        )
        _err_msg_disconnect = (
            "ThRecvCheck " + str(self.getName()) + ": Port "
            "disconnected, waiting for new port."
        )
        _err_msg_reconnect = (
            "ThRecvCheck " + str(self.getName()) + ": Port " "reconnected, continuing."
        )
        attempt = 10
        while not self.exitevent.isSet():
            try:
            ...
            if ret[0] and (not self.exitevent.isSet()):
                ...
                if buf:
                    ...
                    if attempt > 0:
                        ...
                        LOG.debug(
                            "ThRecvCheck %s: Broken pipe " ", reconnecting. ",
                            self.getName(),
                        )
                    ...
                    if attempt > 0:
                        ...
                        if self.migrate_event is None:
                            ...
                            raise exceptions.TestFail(
                                ...
                                "_event to support reconnection." % self.getName()
                            )
                        LOG.debug(
                            "ThRecvCheck %s: Broken pipe " ", reconnecting. ",
                            self.getName(),
