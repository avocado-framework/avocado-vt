...
class QtreeNode(object):
    ...
    def replace_child(self, oldchild, newchild):
        if oldchild not in self.children:
            raise ValueError("child %s not in children %s" % (oldchild, self.children))
    ...
    def set_qtree_prop(self, prop, value):
        if prop in self.qtree:
            raise ValueError(
                "Property %s = %s, not rewriting with %s"
...
class QtreeDisk(QtreeDev):
    ...
    def generate_params(self):
        ...
        if self.block.get("backing_file"):
            ...
            raise ValueError(
                "Missing 'file' or 'backing_file' information " "in self.block."
            )
...
class QtreeContainer(object):
    ...
    def parse_info_qtree(self, info):
        ...
        def _hook_usb2_disk(node):
            ...
            if not (
                node.get_parent().get_parent().get_qtree().get("type") == "usb-storage"
            ):
            ...
            if _offset >= offset:
                ...
                if line.startswith("bus: "):
                    ...
                    if line[0] == "mmio":
                        ...
                        line = None
                    # old version: gpio-in 24, gpio-out 1
                    # new version: gpio-in "" 1, gpio-out "a20" 1
                    elif line[0] == "gpio-in" or line[0] == "gpio-out":
                        if '"' in line[1]:
                            gpio_name, line[1] = line[1].split(" ", 1)
                            line[0] += " %s" % gpio_name
                else:
                    # Corrupted qtree
...
class QtreeDisksContainer(object):
    ...
    def check_guests_proc_scsi(self, info):
        ...
        for disk in self.disks:
            if disk.get_qtree()["type"].startswith("scsi") or disk.get_qtree()[
                "type"
            ].startswith("usb2"):
    ...
    def check_disk_params(self, params):
        ...
        def check_drive_format(node, params):
            ...
            if expected not in actual:
                return "drive format in qemu is %s, in autotest %s" % (actual, expected)
            ...
            for prop in six.iterkeys(current):
                ...
                if prop == "drive_format":
                    ...
                    if out:
                        ...
                        err += 1
                    handled = True
                elif image_params.get(prop) and image_params.get(prop) == current.get(
                    prop
                ):
                    handled = True
                if not handled:
