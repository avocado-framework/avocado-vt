...
class _NfsPoolConnection(_ResourcePoolConnection):
    ...
    def open(self):
        ...
        LOG.info(f"Mounting NFS at {src} to {dst} with options: {options}")
        if not utils_misc.mount(src, dst, "nfs", options, verbose=True):
            raise RuntimeError(f"Failed to mount NFS at {src} to {dst}")

    def close(self):
        src = f"{self._nfs_server}:{self._export_dir}"
        dst = self._mnt
        LOG.info(f"Unmounting NFS at {src} from {dst}")
