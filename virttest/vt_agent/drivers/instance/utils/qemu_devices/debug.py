...
def create_debug_devices(dev_container, debug, parent_bus, machine_type,
                         driver_id, uuid, instance_id):
    devs = []
    ...
    if debug_type == "isa-debugcon":
        if not dev_container.has_device(debug_type):
            ...
            filename = os.path.join(
                data_dir.get_tmp_dir(), "seabios-%s" % driver_id
            )
        ...
        filename = os.path.join(
            data_dir.get_tmp_dir(), "anaconda-%s" % instance_id
        )
