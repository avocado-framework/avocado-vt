...
def create_net_devices(dev_container, net, parent_bus, format_cfg, sock_name, pass_fds=[]):
    devs = []
    ...
    for backing_id, res_backing in resbacking_mgr.backings.items():
        if res_backing.binding_resource_id == port_resource_id:
            backing = resbacking_mgr.get_resource_info_by_backing(
                backing_id)[1]
        ...
        raise ValueError(
            f"No found the binding resource for {port_resource_id}")
