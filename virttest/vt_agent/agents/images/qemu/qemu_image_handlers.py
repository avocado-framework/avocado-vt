...
def get_qemu_virt_image_repr(virt_image_config, output=None):
    ...
    if sec_opts:
        ...
        if virt_image_format == "luks":
            ...
            raise ValueError(f"Encryption of a {virt_image_format} image is not supported")
...
def _create(image_config, arguments):
    ...
    def _qemu_img_create(virt_image_tag):
        ...
        if base_tag is not None:
            ...
            objs_repr, _, cmd_dict["backing_file"] = get_qemu_virt_image_repr(base_virt_image_config, image_repr_format)
            if objs_repr:
        ...
        objs_repr, options_repr, cmd_dict["image_filename"] = get_qemu_virt_image_repr(virt_image_config, "uri")
        if objs_repr:
            secret_objects.append(objs_repr)
        if options_repr:
        ...
        o["out"] = cmd_result.stderr_text if r != 0 else cmd_result.stdout_text
...
def _rebase(image_config, arguments):
    qemu_image_binary = arguments.pop("qemu_img_binary", QEMU_IMG_BINARY)
    image_repr_format = arguments.pop("repr", None)
    rebase_mode = arguments.pop("rebase_mode", None)
    ...
    secret_objects = list()
    obj_repr, options_repr, cmd_dict["image_filename"] = get_qemu_virt_image_repr(target_config, image_repr_format)
    ...
    obj_repr, _, cmd_dict["backing_file"] = get_qemu_virt_image_repr(backing_config, None)
    if obj_repr:
        secret_objects.append(obj_repr)
    ...
    qemu_img_cmd = (
        qemu_image_binary
        + " "
        + cmd_formatter.format(rebase_cmd, **cmd_dict)
    )
...
def _info(image_config, arguments):
    ...
    secret_objects = list()
    obj_repr, _, cmd_dict["image_filename"] = get_qemu_virt_image_repr(target_config, image_repr_format)
    if obj_repr:
        secret_objects.append(obj_repr)
    ...
    qemu_img_cmd = (
        qemu_image_binary
        + " "
        + cmd_formatter.format(info_cmd, **cmd_dict)
    )
