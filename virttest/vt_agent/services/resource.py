...
def connect_pool(pool_id, pool_config):
    ...
    :rtype: tuple
    """
    LOG.info(f"Open the connection to pool: uuid={pool_id}")
...
def create_backing_object(backing_config):
    ...
    """
    LOG.info(
        f"Create the backing object of the resource: {backing_config['meta']['uuid']}"
    )
...
def clone_resource_by_backing(backing_id, arguments=None):
    ...
    :rtype: tuple
    """
    LOG.info(f"Clone a new resource by its backing: uuid={backing_id}, args={arguments}")
