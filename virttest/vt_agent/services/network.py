...
def setup_tap_bridge(net_type, ifname, net_dst, queues=1):
    ...
    if net_type == "macvtap":
        ...
        LOG.debug(
            "Adding NIC ifname %s to bridge %s", ifname, net_dst)
        if net_type == "bridge":
...
def cleanup_tap_bridge(net_type, ifname=None, tapfds=None,
                       vhostfds=None, net_dst=None, queues=1):
    ...
    try:
        if net_type == "macvtap":
            ...
            LOG.debug(
                "Removing NIC ifname %s from bridge %s", ifname, net_dst)
            if tapfds:
            ...
            if ifname:
                ...
                if utils_misc.wait_for(
                        lambda: ifname not in utils_net.get_net_if(),
                        deletion_time
                ):
...
def verify_ip_address_ownership(ip, macs, timeout=60.0):
    ...
    return utils_net.verify_ip_address_ownership(ip, macs, timeout)