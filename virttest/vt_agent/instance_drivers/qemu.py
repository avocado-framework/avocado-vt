...
class QemuInstanceDriver(InstanceDriver):
    ...
    def _create_sandbox_device(self, sandbox):
        ...
        if not self._devices.has_option("sandbox"):
    ...
    def _create_machine_devices(self, machine):
        ...
        def create_machine_q35(machine_props):
            ...
            bus = (
                qdevices.QPCIEBus(
                    ...
                    "pcie.0", "PCIE", root_port_type, "pci.0", None
                ),
                qdevices.QStrictCustomBus(
                    None, [["chassis"], [256]], "_PCI_CHASSIS", first_port=[1]
                ),
            ...
            root_port = create_pcic(port_name, port_params)
        ...
        machines_info = utils_qemu.get_machines_info(self._qemu_binary)
        ...
        if not machine_type:
            for m_name, m_desc in machines_info.items():
            ...
            elif machine_type == "invalid":
                # For negative testing pretend the unsupported machine is
                # similar to i440fx one (1 PCI bus, ..)
                invalid_machine = create_machine_i440fx
            elif machine_type not in machines_info:
                raise ValueError(
    ...
    def _create_nodefault_device(self):
        if self._devices.has_option("nodefaults"):
            return qdevices.QStringDevice("nodefaults", cmdline=" -nodefaults")

    def _create_iommu_device(self, iommu):
        if iommu.get("type") == "intel_iommu" and self._devices.has_device(
            "intel-iommu"
        ):
            dev = qdevices.QDevice("intel_iommu", iommu.get("props"))
        elif iommu.get("type") == "virtio_iommu" and self._devices.has_device(
            "virtio-iommu-pci"
        ):
            dev = qdevices.QDevice(
                "virtio-iommu-pci",
                iommu.get("props"),
                parent_bus={"aobject": iommu.get("bus")},
            )
        else:
            raise ValueError
    ...
    def _create_vga_device(self, vga):
        ...
        machine_type = self._params.get("machine")["type"]
        vga_dev = vga_dev_map.get(vga.get("type"), None)
        if machine_type.startswith("arm64-pci:"):
            if vga == "virtio" and not self._devices.has_device(vga_dev):
        ...
        return qdevices.QDevice(vga_dev, parent_bus=parent_bus)
    ...
    def _create_watchdog(self, watchdog):
        ...
        if watchdog_type == "itco":
            ...
            devs.append(dev)
        elif watchdog_type and self._devices.has_device(watchdog_type):
            if self._devices.is_pci_device(watchdog_type):
        ...
        if devices:
            for device in devices:
                if device.get("type") == "dimm":
                    dev = qdevices.Dimm(device["props"], dimm_type=device.get("type"))
                    dev.set_param("id", device["id"])
                    dev.set_param("memdev", device["props"]["backend"])
                    devs.append(dev)
                elif device.get("type") == "virtio-mem":
                    vmem_model = "virtio-mem-pci"
                    parent_bus = self._get_pci_parent_bus(device["bus"])
                    if "-mmio:" in self._params.get("machine")["type"]:
                        vmem_model = "virtio-mem-device"
                        parent_bus = None
                    if not self._devices.has_device(vmem_model):
                    ...
                    raise ValueError("Unsupported memory device type")

        machine_backend = memory["machine"].get("backend")
        if machine_backend:
            if machine_backend["type"] is None:
                machine_backend["type"] = "memory-backend-ram"
            dev = qdevices.Memory(machine_backend["type"], machine_backend["props"])
            dev.set_param("id", machine_backend["id"])
            devs.append(dev)

        if memory_machine.get("mem_path"):
            cmd = "-mem-path %s" % memory_machine["mem_path"]
            dev = qdevices.QStringDevice("mem-path", cmdline=cmd)
            devs.append(dev)
    ...
    def _create_cpu_devices(self, cpu):
        def __add_smp():
            ...
            vcpu_maxcpus = cpu_topology.get("max_cpus")
            ...
            enable_dies = vcpu_dies != "INVALID" and Flags.SMP_DIES in self._devices.caps
            enable_clusters = (
                    vcpu_clusters != "INVALID" and Flags.SMP_CLUSTERS in self._devices.caps
            )
            enable_drawers = vcpu_drawers != "INVALID" and Flags.SMP_DRAWERS in self._devices.caps
            enable_books = vcpu_books != "INVALID" and Flags.SMP_BOOKS in self._devices.caps
            ...
            vcpu_prefer = cpu_topology.get("prefer")
            if vcpu_prefer and vcpu_prefer == "socket":
                vcpu_prefer_sockets = True
            else:
                if utils_qemu.get_qemu_version(self._qemu_binary)[
                    0] not in VersionInterval(
            ...
            if vcpu_maxcpus != 0:
                ...
                hotpluggable_cpus = len(cpu.get("devices"))
                if self._params["machine"]["type"].startswith("pseries"):
            ...
            if self._devices.has_option(smp_pattern):
                smp_str += ",maxcpus=%d" % vcpu_maxcpus
            if cpu_topology.get("cores") != 0:
                smp_str += ",cores=%d" % vcpu_cores
            if cpu_topology.get("threads") != 0:
                smp_str += ",threads=%d" % vcpu_threads
            if vcpu_dies != 0:
                smp_str += ",dies=%d" % vcpu_dies
            if vcpu_clusters != 0:
                smp_str += ",clusters=%d" % vcpu_clusters
            if vcpu_drawers != 0:
                smp_str += ",drawers=%d" % vcpu_drawers
            if vcpu_books != 0:
                smp_str += ",books=%d" % vcpu_books
            if cpu_topology.get("sockets") != 0:
                smp_str += ",sockets=%d" % vcpu_sockets
            return smp_str

        def __add_cpu_flags():
            if not self._devices.has_option("cpu"):
                return ""
            ...
            if cpu_info["model"] in ["Penryn", "Nehalem"]:
                recognize_flags = cpu_utils.get_recognized_cpuid_flags()
                if not ("erms" in cpu_info.get("flags") or "erms" in recognize_flags):
                    cmd += ",+erms"
        ...
        cpu_driver = None
        if utils_qemu.has_device_category(self._qemu_binary, "CPU"):
            cpu_model = cpu_info.get("model")
            if cpu_model:
                search_pattern = r"%s-\w+-cpu" % cpu_model
                cpu_driver = utils_qemu.find_supported_devices(
                    self._qemu_binary, search_pattern, "CPU"
                )
                if cpu_driver is None:
                    raise ValueError

        devices = cpu.get("devices")
        if devices:
            for device in devices:
                if not self._devices.has_device(cpu_driver):
                    raise NotImplementedError(self.vmname, cpu_driver)
                dev = qdevices.QCPUDevice(
                    cpu_driver,
                    device.get("vcpu_enable"),
                    params=device["props"],
                    parent_bus={"aobject": device["bus"]},
                )
                devs.append(dev)
            else:
    ...
    def _create_soundcard_device(self, soundcard):
        devs = []
        dev_type = soundcard["type"]
        parent_bus = self._get_pci_parent_bus(soundcard["bus"])
        dev = qdevices.QDevice(dev_type, parent_bus=parent_bus)
        devs.append(dev)
        if dev_type == "intel-hba":
            devs.append(qdevices.QDevice("hda-duplex"))
        return devs

    def _create_monitor_device(self, monitor):

        def __get_monitor_filename():
            return os.path.join(
                data_dir.get_tmp_dir(),
                "monitor-%s-%s" % (monitor_id, self._driver_id)
            )

        def __add_hmp_device(devs):
            if not self._devices.has_option("chardev"):
                cmd = " -monitor unix:'%s',server,nowait" % filename
                dev = qdevices.QStringDevice("HMP-%s" % monitor_id, cmdline=cmd)
                devs.append(dev)
            else:
                chardev_id = "chardev_%s" % monitor_id
                params["id"] = chardev_id
                dev = qdevices.CharDevice(params, chardev_id)
                devs.append(dev)
                cmd = " -mon chardev=%s" % chardev_id
                cmd += ",mode=readline"
                dev = qdevices.QStringDevice("HMP-%s" % monitor_id, cmdline=cmd)
                devs.append(dev)

        def __add_qmp_device(devs):
            if not self._devices.has_option("chardev"):
                cmd = " -qmp unix:'%s',server,nowait" % filename
                dev = qdevices.QStringDevice("QMP-%s" % monitor_id,
                                             cmdline=cmd)
                devs.append(dev)
            else:
                # chardev_id = "qmp_id_%s" % monitor["id"]
                chardev_id = monitor["id"]  # FIXME: use the monitor_id as the chardev_id in order to create the monitor connection
                params["id"] = chardev_id
                dev = qdevices.CharDevice(params, chardev_id)
                devs.append(dev)
                cmd = " -mon chardev=%s" % chardev_id
        ...
        monitor_backend_props = monitor["backend"]["props"]
        filename = __get_monitor_filename()

        # FIXME: convert to Params
        params = Params()
        for k, v in monitor_backend_props.items():
            if k == "port" and isinstance(v, (list, tuple)):
                host = monitor_backend_props.get("host")
                params[k] = str(utils_misc.find_free_ports(v[0], v[1], 1, host)[0])
            params[k] = v

        backend_type = monitor["backend"]["type"]
        if backend_type in [
            "unix_socket",
            "file",
            "pipe",
            ...
            params.update({"path": filename})
            if backend_type == "pipe":
                process.system("mkfifo %s" % filename)

        if not self._devices.has_option("qmp"):
            __add_hmp_device(devs)
        else:
            if monitor_type == "qmp":
                __add_qmp_device(devs)
            else:
                __add_hmp_device(devs)
        return devs

    def _create_panic_device(self, panic):
        if not self._devices.has_device(panic):
            LOG.warn("%s device is not supported", panic["type"])
        if panic["type"] == "pvpanic-pci":
            parent_bus = self._get_pci_parent_bus(panic["bus"])
            dev = qdevices.QDevice(panic["type"], parent_bus=parent_bus)
        else:
            dev = qdevices.QDevice(panic["type"], params=panic["props"])
        return dev
    ...
    def _create_controller_device(self, controller):
        usb_type = controller.get("type")
        if not self._devices.has_device(usb_type):
            raise NotImplementedError(usb_type)
    ...
    def _create_usb_device(self, usb):
        usbdev_type = usb.get("type")
        usb_props = usb.get("props")
        usb_name = usb_props.pop("name")

        if not self._devices.has_device(usbdev_type):
            raise NotImplementedError(usbdev_type)
        if self._devices.has_option("device"):

            usb_controller_type = usb_props.pop("usb_controller")

            dev = qdevices.QDevice(usbdev_type, params=usb_props, aobject=usb_name)
            dev.set_param("id", usb["id"])
            dev.set_param("bus", usb["bus"])

            dev.parent_bus += ({"type": usb_controller_type},)
        else:
            if "tablet" in usbdev_type:
                ...
                dev = qdevices.QStringDevice("missing-usb-%s" % usb_name)
                LOG.error(
    ...
    def _create_disk_devices(self, disk):
        ...
        image_id = disk["id"]
        try:
            image_conf = disk["source"]["spec"]["virt-images"][image_id]
            image_opts = image.qemu.get_image_opts(image_conf)
            LOG.debug(f"==> get_image_opts: {image_opts}")
            filename = image_opts[-1]["file"]["filename"]
        except:
            LOG.error("----->Could not find filename and hard code for it")
            filename = "/home/kvm_autotest_root/images/rhel950-64-virtio-scsi-ovmf.qcow2"

        cmdline = (
            f"-blockdev node-name=file_ovmf_code,driver=file,filename=/usr/share/OVMF/OVMF_CODE.secboot.fd,auto-read-only=on,discard=unmap "
            "-blockdev node-name=drive_ovmf_code,driver=raw,read-only=on,file=file_ovmf_code "
            "-blockdev node-name=file_ovmf_vars,driver=file,filename=/home/filesystem_VARS.raw,auto-read-only=on,discard=unmap "
            "-blockdev node-name=drive_ovmf_vars,driver=raw,read-only=off,file=file_ovmf_vars "
            "-device pcie-root-port,id=pcie-root-port-2,port=2,addr=0x1.0x2,bus=pcie.0,chassis=3 "
            "-device virtio-scsi-pci,id=virtio_scsi_pci0,bus=pcie-root-port-2,addr=0x0 "
            "-blockdev node-name=file_image1,driver=file,auto-read-only=on,discard=unmap,"
            f"aio=threads,filename={filename},cache.direct=on,cache.no-flush=off "
            "-blockdev node-name=drive_image1,driver=qcow2,read-only=off,cache.direct=on,cache.no-flush=off,file=file_image1 "
            "-device scsi-hd,id=image1,drive=drive_image1,write-cache=on")
        dev = qdevices.QStringDevice("disk", cmdline=cmdline)
        return dev
    ...
    def _create_vsock_device(self, vsock):
        machine_type = self._params["machine"]["type"]
        vsock_params = {"id": vsock["id"], "guest-cid": vsock["cid"]}
        if "-mmio:" in machine_type:
            dev = qdevices.QDevice("vhost-vsock-device", vsock_params)
        elif machine_type.startswith("s390"):
            dev = qdevices.QDevice("vhost-vsock-ccw", vsock_params)
        else:
            bus = self._get_pci_parent_bus(vsock["bus"])
            dev = qdevices.QDevice("vhost-vsock-pci", vsock_params, parent_bus=bus)
        return dev
    ...
    def create_devices(self, spec):
        ...
        pci_controllers = self._params.get("pci_controllers")
        if pci_controllers:
            for pci_controller in pci_controllers:
                dev = self._create_pci_controller_device(pci_controller)
                self._devices.insert(dev)
        ...
        if monitors:
            for monitor in monitors:
                dev = self._create_monitor_device(monitor)
                self._devices.insert(dev)
