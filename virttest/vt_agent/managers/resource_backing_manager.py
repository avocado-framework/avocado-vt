...
class ResourceBackingManager(object):
    """
    The resource backing manager manages all the backing objects and the
    resource pool connection objects.
    """
    def __init__(self):
        self._backings = dict()
        self._pool_connections = dict()

        # When the worker node is also the master node, we don't start the
        # agent daemon, the process quits before the job process starts, so
        # we need to read the file to restore the configurations
    ...
    def query_resource_backing(self, resource_id):
        ...
        """
        LOG.info(f"Query the backing uuid of the resource: uuid={resource_id}")
    ...
    def get_resource_info_by_backing(self, backing_id):
        ...
        LOG.info(f"Get the resource information: uuid={backing.resource_uuid}, backing uuid={backing_id}")
    ...
    def destroy_backing_object(self, backing_id):
        ...
        r, o = 0, dict()
        try:
            backing = self.backings.pop(backing_id)
            pool_conn = self.pool_connections[backing.resource_pool_uuid]
            backing.destroy_object(pool_conn)
        except Exception as e:
            r, o["out"] = 1, str(e)

        return r, o

    def clone_resource_by_backing(self, source_backing_id, arguments):
        """
        Clone a new resource by the source resource's backing.
        """
        r, o = 0, dict()
        try:
            backing = self.backings[source_backing_id]
            pool_conn = self.pool_connections[backing.resource_pool_uuid]
            ret = backing.clone_resource(pool_conn, arguments)
            if ret:
                o["out"] = ret
        except Exception as e:
            r, o["out"] = 1, str(e)
