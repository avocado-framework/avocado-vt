...
def _create(virimage_config, arguments):
    def _dd(image_tag):
        qemu_img_cmd = ""
        image_config = virimage_spec["images"][image_tag]
    ...
    def _qemu_img_create(image_tag):
        ...
        if base_tag is not None:
            base_image_config = virimage_spec["images"][base_tag]
            ...
            for tag in image_names:
                if tag == base_tag:
                    break
                config = virimage_spec["images"][tag]
    ...
    virimage_meta = virimage_config["meta"]
    virimage_spec = virimage_config["spec"]

    tag = arguments.pop("target", None)
    image_names = [tag] if tag else list(virimage_meta["topology"].values())[0]
