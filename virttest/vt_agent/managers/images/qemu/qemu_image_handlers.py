...
def _create(virtual_image_config, arguments):
    def _dd(image_tag):
        qemu_img_cmd = ""
        image_config = virtual_image_spec["images"][image_tag]
    ...
    def _qemu_img_create(image_tag):
        ...
        if base_tag is not None:
            base_image_config = virtual_image_spec["images"][base_tag]
            ...
            for tag in image_names:
                if tag == base_tag:
                    break
                config = virtual_image_spec["images"][tag]
    ...
    virtual_image_meta = virtual_image_config["meta"]
    virtual_image_spec = virtual_image_config["spec"]

    tag = arguments.pop("target", None)
    image_names = [tag] if tag else list(virtual_image_meta["topology"].values())[0]
    for image_name in image_names:
        _qemu_img_create(image_name)


def _dd(source_image_config, target_image_config, bs=None, count=None, skip=None):
    """
    ...
    src = copy.deepcopy(source_image_config)
    tgt = copy.deepcopy(target_image_config)
    auth_opts, sec_opts, img_opts = get_image_opts(src)
    ...
    raw_copy = True if sec_opts else False
    if raw_copy:
        cmd_dict["image_format"] = cmd_dict["target_image_format"] = "raw"
        src["spec"]["format"] = tgt["spec"]["format"] = "raw"
    ...
    if objs_repr:
        cmd_dict["secret_object"] = objs_repr

    dd_cmd = (
        qemu_image_binary + " " + cmd_formatter.format(dd_cmd, **cmd_dict)
    )
    process.run(dd_cmd, shell=True, verbose=False, ignore_status=False)


def _backup(virtual_image_config, arguments):
    virtual_image_meta = virtual_image_config["meta"]
    virtual_image_spec = virtual_image_config["spec"]
    image_names = list(virtual_image_meta["topology"].values())[0]

    for image_name in image_names:
        image_config = virtual_image_spec["images"][image_name]
        backup_image_name = image_config["spec"]["backup"]
        backup_image_config = virtual_image_spec["images"][backup_image_name]

        _dd(image_config, backup_image_config)


def _restore(virtual_image_config, arguments):
    virtual_image_meta = virtual_image_config["meta"]
    virtual_image_spec = virtual_image_config["spec"]
    image_names = list(virtual_image_meta["topology"].values())[0]
    ...
    for image_name in image_names:
        ...
        _dd(backup_image_config, image_config)
