...
def _clone(source_logical_image_config, clone_logical_image_config):
    logical_image_meta = source_logical_image_config["meta"]
    logical_image_spec = source_logical_image_config["spec"]

    # Clone each lower-level qemu image
    for image_name in logical_image_meta["topology"]["value"]:
        image_config = logical_image_spec["images"][image_name]
        _copy_data(source_logical_image_config, image_name, clone_logical_image_config, image_name)

    # Construct the backing chain
    clone_logical_image_meta = clone_logical_image_config["meta"]
    if clone_logical_image_meta["topology"]["type"] == "chain":
        image_names = clone_logical_image_meta["topology"]["value"]
...
def _dd(source_logical_image_config, source_image_name,
        target_logical_image_config, target_image_name,
        bs=None, count=None, skip=None, rawcopy=False):
    """
    ...
    base_tag = _get_backing_name(source_image_name, source_logical_image_meta["topology"])
    if base_tag is not None:
...
def _copy_data(source_logical_image_config, source_image_name,
               target_logical_image_config, target_image_name):
    source_image_config  = source_logical_image_config["spec"]["images"][source_image_name]
    target_image_config  = target_logical_image_config["spec"]["images"][target_image_name]
    ...
    if source_volume_type == "file" and target_volume_type == "file":
        ...
        _dd(source_logical_image_config, source_image_name,
            target_logical_image_config, target_image_name, rawcopy=True)
...
def _backup(logical_image_config, arguments):
    ...
    for image_name in image_names:
        ...
        _copy_data(logical_image_config, image_name, logical_image_config, backup_image_name)
...
def _restore(logical_image_config, arguments):
    ...
    for image_name in image_names:
        image_config = logical_image_spec["images"][image_name]
        backup_image_name = image_config["spec"]["backup"]
        _copy_data(logical_image_config, backup_image_name, logical_image_config, image_name)
