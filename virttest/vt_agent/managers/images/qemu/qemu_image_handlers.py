...
def _create(virt_image_config, arguments):
    def _dd(image_tag):
        qemu_img_cmd = ""
        image_config = virt_image_spec["images"][image_tag]
    ...
    def _qemu_img_create(image_tag):
        ...
        if base_tag is not None:
            base_image_config = virt_image_spec["images"][base_tag]
            ...
            for tag in image_names:
                if tag == base_tag:
                    break
                config = virt_image_spec["images"][tag]
    ...
    virt_image_meta = virt_image_config["meta"]
    virt_image_spec = virt_image_config["spec"]

    tag = arguments.pop("target", None)
    image_names = [tag] if tag else virt_image_meta["topology"]["value"]
    for image_name in image_names:
        _qemu_img_create(image_name)


def _dd(virt_image_config, source_image_name, target_image_name, bs=None, count=None, skip=None, rawcopy=False):
    """
    ...
    virt_image_meta = virt_image_config["meta"]
    virt_image_spec = virt_image_config["spec"]
    source_image_config = virt_image_spec["images"][source_image_name]
    target_image_config = virt_image_spec["images"][target_image_name]
    ...
    image_names = virt_image_meta["topology"]["value"]
    if base_tag is not None:
...
def _dump_data(virt_image_config, source_image_name, target_image_name):
    source_image_config = virt_image_config["spec"]["images"][source_image_name]
    target_image_config = virt_image_config["spec"]["images"][target_image_name]
    ...
    if source_volume_config["meta"]["volume-type"] == "file":
        ...
        _dd(virt_image_config, source_image_name, target_image_name)


def _backup(virt_image_config, arguments):
    virt_image_meta = virt_image_config["meta"]
    virt_image_spec = virt_image_config["spec"]
    tag = arguments.pop("target", None)
    image_names = [tag] if tag else virt_image_meta["topology"]["value"]
    ...
    for image_name in image_names:
        image_config = virt_image_spec["images"][image_name]
        volume_config = image_config["spec"]["volume"]
        backup_image_name = image_config["spec"]["backup"]
        _dump_data(virt_image_config, image_name, backup_image_name):


def _restore(virt_image_config, arguments):
    virt_image_meta = virt_image_config["meta"]
    virt_image_spec = virt_image_config["spec"]
    tag = arguments.pop("target", None)
    image_names = [tag] if tag else virt_image_meta["topology"]["value"]
    ...
    for image_name in image_names:
        image_config = virt_image_spec["images"][image_name]
        backup_image_name = image_config["spec"]["backup"]
        _dump_data(virt_image_config, backup_image_name, image_name):
