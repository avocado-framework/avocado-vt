...
class _DirVolumeBacking(_FileVolumeBacking):
    RESOURCE_POOL_TYPE = "filesystem"

    def create_object(self, pool_connection):
        """
        Create the dir volume backing object
        """
        if self._uri:
            if not self._uri.startswith(pool_connection.root_dir):
                raise ValueError(f"Wrong uri {self._uri} specified in path {pool_connection.root_dir}")
        else:
