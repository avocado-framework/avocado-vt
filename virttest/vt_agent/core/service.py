...
from .data_dir import get_services_module_dir
...
class _Services(object):
    ...
    def get_service(self, name):
        ...
        try:
            ...
            raise ServiceError(f"No supported service '{name}'.")
...
def _import_services(services_dir_path):
    ...
    for source_path in service_sources:
        # Construct a unique, dot-separated module name for the spec
        module_spec_name = source_path.replace(os.sep, '.')
        if module_spec_name.endswith('.py'):
            module_spec_name = module_spec_name[:-3]

        # Ensure module_spec_name is cleaned if it starts with a dot (e.g. from relative paths)
        # This is more of a safeguard, as absolute paths are expected here from os.walk
        if module_spec_name.startswith('.'):
            module_spec_name = module_spec_name.lstrip('.')

        module_to_load = None  # To hold module object for cleanup in case of error
        spec_for_module = None  # To hold spec object
        ...
        try:
            ...
            LOG.error("Exception loading service module %s from %s: %s",
                      module_spec_name, source_path, e, exc_info=True)
            if spec_for_module and spec_for_module.name in sys.modules and \
                    sys.modules[spec_for_module.name] is module_to_load:
...
def load_services():
    ...
    services_dir_path = get_services_module_dir()
    ...
    LOG.info("Loading services from directory: %s", services_dir_path)
    ...
    for service_module in imported_service_modules:
        try:
            ...
            relative_path = os.path.relpath(module_file_path,
                                            services_dir_path)
            service_reg_name = os.path.splitext(relative_path)[0].replace(
                os.sep, '.')
            ...
            if not service_reg_name or service_reg_name.startswith('.'):
                LOG.warning(
                    ...
                    service_module.__name__, module_file_path)
                continue
            ...
            LOG.error(
                ...
                service_module.__name__ if hasattr(service_module,
                                                   '__name__') else 'UnknownModule',
                e, exc_info=True)
