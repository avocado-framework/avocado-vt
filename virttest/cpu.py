...
CPU_TYPES_RE = {
    ...
    "EPYC-Milan-v2": "ibrs|amd-ibrs,pcid,ssbd|amd-ssbd,erms,fsrm,invpcid,pku,vaes,vpclmulqdq",
    "EPYC-Milan-v1": "ibrs|amd-ibrs,pcid,ssbd|amd-ssbd,erms,fsrm,invpcid,pku",
    "EPYC-Milan": "ibrs|amd-ibrs,pcid,ssbd|amd-ssbd,erms,fsrm,invpcid,pku",
    "EPYC-Rome-v4": "rdpid,wbnoinvd,stibp|amd-stibp,clwb,umip,ibrs|amd-ibrs",
    "EPYC-Rome-v3": "rdpid,wbnoinvd,stibp|amd-stibp,clwb,umip,xsaves,ibrs|amd-ibrs",
    "EPYC-Rome-v2": "rdpid,wbnoinvd,stibp|amd-stibp,clwb,umip,xsaves,ibrs|amd-ibrs",
    "EPYC-Rome-v1": "rdpid,wbnoinvd,stibp|amd-stibp,clwb,umip,xsaves",
    "EPYC-Rome": "rdpid,wbnoinvd,stibp|amd-stibp,clwb,umip,xsaves",
...
def get_cpu_flags(cpu_info=""):
    ...
    return re.split(r"\s+", cpu_flags.strip())
...
def get_recognized_cpuid_flags(qemu_binary="/usr/libexec/qemu-kvm"):
    ...
    try:
        return list(filter(None, re.split(r"\s", match.group(1))))
...
def get_host_cpu_models():
    ...
    def _cpu_flags_sort(cpu_flags):
        ...
        """
        flag_list = sorted(re.split(r"\s+", cpu_flags.strip()))
...
def extract_qemu_cpu_models(qemu_cpu_help_text):
    ...
    x86_pattern_list = r"x86\s+\[?([a-zA-Z0-9_-]+)\]?.*\n"
    ppc64_pattern_list = r"PowerPC\s+\[?([a-zA-Z0-9_-]+\.?[0-9]?)\]?.*\n"
    s390_pattern_list = r"s390\s+\[?([a-zA-Z0-9_-]+)\]?.*\n"
...
def get_cpu_info_from_virsh_qemu_cli(name):
    ...
    """
    cmd = r"ps aux | grep qemu | grep -e '\s-name\s\+guest=%s\(\,\|\s\)'" % name
    output = process.getoutput(cmd, shell=True)
    cpu_line = re.findall(r"-cpu\s+(\S*)\s+", output)[0]
