...
def delete_linux_bridge_tmux(
    ...
    br_path = "/sys/class/net/%s" % linux_bridge_name
    # Ensure required tools exist across distributions
    pkgs_ok = utils_package.package_install("tmux", session)
    # Ensure DHCP client is available (package name differs)
    need_dhclient = (
        utils_misc.cmd_status_output(
            "command -v dhclient", shell=True, session=session
        )[0]
        != 0
    )
    if need_dhclient:
        installed = False
        for cand in ("dhclient", "isc-dhcp-client", "dhcp-client"):
            if utils_package.package_install(cand, session):
                installed = True
                break
        pkgs_ok = pkgs_ok and installed
    # Ensure ifconfig and process kill are present
    if (
        utils_misc.cmd_status_output(
            "command -v ifconfig", shell=True, session=session
        )[0]
        != 0
    ):
        pkgs_ok = pkgs_ok and utils_package.package_install("net-tools", session)
    if (
        utils_misc.cmd_status_output("command -v pkill", shell=True, session=session)[0]
        != 0
    ):
        pkgs_ok = pkgs_ok and (
            utils_package.package_install("procps", session)
            or utils_package.package_install("procps-ng", session)
        )
    if not pkgs_ok:
        raise exceptions.TestError(
            "Failed to install required tools for bridge deletion."
        )
    if session:
        bridge_exists = session.cmd_status("ip link show %s" % linux_bridge_name) == 0
    else:
