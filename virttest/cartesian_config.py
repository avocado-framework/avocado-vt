...
class OnlyFilter(NoOnlyFilter):
    ...
    def might_pass(self, failed_ctx, failed_ctx_set, ctx, ctx_set, descendant_labels):
        for word in self.filter:
            for block in word:
...
class Lexer(object):
    ...
    def get_until_check(self, lType, end_tokens=None):
        ...
        for token in self.get_until_gen(end_tokens):
            if type(token) in lType:
                ...
                raise ParserError(
                    "Expected %s got %s" % (lType, type(token)),
                    self.line,
                    self.filename,
                    self.linenum,
                )
    ...
    def get_next_check(self, lType):
        ...
        if type(token) in lType:
            ...
            raise ParserError(
                "Expected %s got ['%s']=[%s]"
                % ([x.identifier for x in lType], token.identifier, token),
                self.line,
                self.filename,
                self.linenum,
            )
    ...
    def get_next_check_nw(self, lType):
        ...
        if type(token) in lType:
            ...
            raise ParserError(
                "Expected %s got ['%s']"
                % ([x.identifier for x in lType], token.identifier),
                self.line,
                self.filename,
                self.linenum,
            )
    ...
    while typet not in [LEndL]:
        if typet in [LIdentifier, LLRBracket]:  # join    identifier
            if typet == LLRBracket:  # (xxx=ttt)
                _, ident = lexer.check_token(next_nw(tokens), [LIdentifier])  # (iden
                typet, _ = lexer.check_token(next_nw(tokens), [LSet, LRRBracket])  # =
...
class Parser(object):
    ...
    def __init__(
        ...
        expand_defaults=[],
        debug=False,
        keep_at_variants_in_shortnames=False,
    ):
        self.node = Node()
        self.debug = debug
            ...
            while True:
                ...
                if typet == LIdentifier:
                    ...
                    while True:
                        ...
                        node3.append_to_shortname = (
                            not is_default or self.keep_at_variants_in_shortnames
                        )
...
def postfix_parse(dic):
    ...
    for key in dic:
        ...
        if key.endswith("_max"):
            ...
            if tmp_key not in dic or compare_string(dic[tmp_key], dic[key]) > 0:
