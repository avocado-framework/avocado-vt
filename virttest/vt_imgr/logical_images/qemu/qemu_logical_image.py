...
class QemuLogicalImage(LogicalImage):
    ...
    def add_image_object(self, arguments):
        ...
        self.image_spec["images"][image_name] = self.define_image_config(
    ...
    def clone(self):
        ...
        r, o = node.proxy.image.clone_logical_image(
            self.get_info(verbose=True), obj.get_info(verbose=True)
        )
        if r != 0:
    ...
    def backup(self, arguments):
        ...
        for pool_id in resmgr.pools:
            ...
            if (
                pool_meta["type"] == "filesystem"
                and node_name in pool_meta["access"]["nodes"]
            ):
                break
