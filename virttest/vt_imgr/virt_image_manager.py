...
image_config = imgr.define_virt_image_config(image_name, params)

# Create the upper-level image object
image_id = imgr.create_virt_image_object(image_config)

# Create the upper-level image
imgr.update_virt_image(image_id, {"create":{}})

# Create only one lower-level image
imgr.update_virt_image(image_id, {"create":{"target": "top"}})

# Destroy one lower-level image
imgr.update_virt_image(image_id, {"destroy":{"target": "top"}})

# Get the configuration of the upper-level image
out = imgr.get_virt_image_info(image_id, request=None)
out:
{
   "meta": {
     "uuid": "uuid-sn"
...
from .virt_images import get_virt_image_class

LOG = logging.getLogger("avocado." + __name__)


class _VirtImageManager(object):
    def __init__(self):
        self._images = dict()
    ...
    def define_virt_image_config(self, image_name, params):
        """
        Define the upper-level image configuration by its cartesian params
        ...
        # TODO: virt_image_type is a upper-level image param
        image_type = image_params.get("virt_image_type", "qemu")
        image_class = get_virt_image_class(image_type)

        LOG.debug(f"Define the {image_type} image configuration for {image_name}")
        return image_class.define_config(image_name, params)

    def create_virt_image_object(self, image_config):
        """
        Create an upper-level image object by its configuration without any
        ...
        LOG.debug(f"Create the image object {image.image_id} for {image.image_name}")
        return image.image_id

    def destroy_virt_image_object(self, image_id):
        """
    ...
    def clone_virt_image(self, image_id):
        """
        Clone the image
    ...
    def update_virt_image(self, image_id, config):
        """
    ...
    def get_virt_image_info(self, image_id, request=None, verbose=False):
        """
    ...
    def query_virt_image(self, image_name, vm_name=None):
        """
