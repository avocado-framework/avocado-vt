...
class _QemuImage(_Image):
    ...
    def create_object_from_self(self, pool_id=None):
        if pool_id is None:
            o = resmgr.get_resource_info(self.volume_id, "meta.pool")
            pool_id = o["pool"]
        ...
        config["spec"].update(
            {
                # The upper-level image takes care of the topology
                "backing": None,
                # No need to have the backup information for the new image
                "backup": None,
                "volume": resmgr.create_resource_object_from(self.volume_id, pool_id),
            }
        )
