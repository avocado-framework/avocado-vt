...
class _QemuImage(_Image):
    ...
    def add_virt_image_object(self, arguments):
        ...
        if not set(node_names).issubset(set(self.image_access_nodes)):
            raise ValueError(f"{node_names} should be a subset of {self.image_access_nodes}")

        config = self.define_virt_image_config(
            target, target_image_params
        )
        ...
        if backing_chain:
            ...
            LOG.info("Qemu image changed: name=%s, topology=%s",
                     self.image_meta["name"], self.image_meta["topology"])
    ...
    def remove_virt_image_object(self, arguments):
        ...
        if len(self.virt_images) == 1:
            raise ValueError(f"Cannot remove {target} for a qemu image "
                             "must have at least one lower-level image")

        if target in self.virt_image_names:
            if ("chain" in self.image_meta["topology"] and
                    target != self.virt_image_names[-1]):
                raise ValueError("Only the top virt image in topology(%s) "
                                 "can be removed" % self.virt_image_names)
            elif "none" in self.image_meta["topology"]:
                raise ValueError("Removing %s in topology(%s) can cause an "
        ...
        if target in self.virt_image_names:
            ...
            if len(self.virt_image_names) < 2:
                self.image_meta["topology"]["none"] = self.image_meta["topology"].pop("chain")
    ...
    def qemu_img_destroy(self, arguments):
        ...
        LOG.info("Destroy the qemu image %s, targets: %s",
                 self.image_meta["name"], image_tags)
