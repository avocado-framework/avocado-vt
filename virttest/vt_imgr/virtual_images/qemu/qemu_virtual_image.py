...
class _QemuVirImage(_VirImage):
    ...
    def create_image_object(self, image_name):
    ...
    def create_object(self):
        ...
        for image_name in self.image_names:
            self.images[image_name] = self.create_image_object(image_name)
    ...
    def add_image_object(self, arguments):
        ...
        self.images[target] = self.create_image_object(target)
    ...
    def clone(self):
        LOG.debug(f"Clone the image object from qemu image {self.image_name}")
    ...
    def qemu_img_create(self, arguments):
        ...
        for image_tag in image_tags:
            image = self.images[image_tag]
            image.allocate_volume(arguments)

        r, o = node.proxy.image.update_image(self.get_info(), {"create": arguments})
        if r != 0:
            raise Exception(o["out"])
    ...
    def qemu_img_rebase(self, arguments):
        ...
        node_name = self.image_access_nodes[0]
        node = cluster.get_node(node_name)
        r, o = node.proxy.image.update_image(self.get_info(), {"rebase": arguments})
        if r != 0:
            raise Exception(o["out"])
    ...
    def qemu_img_commit(self, arguments):
        node_name = self.image_access_nodes[0]
        node = cluster.get_node(node_name)
        r, o = node.proxy.image.update_image(self.get_info(), {"commit": arguments})
        if r != 0:
            raise Exception(o["out"])
    ...
    def qemu_img_snapshot(self, arguments):
        node_name = self.image_access_nodes[0]
        node = cluster.get_node(node_name)
        r, o = node.proxy.image.update_image(self.get_info(), {"snapshot": arguments})
        if r != 0:
            raise Exception(o["out"])
    ...
    def qemu_img_info(self, arguments):
        node_name = self.image_access_nodes[0]
        node = cluster.get_node(node_name)
        r, o = node.proxy.image.update_image(self.get_info(), {"info": arguments})
        if r != 0:
            raise Exception(o["out"])
        return o["out"]
    ...
    def qemu_img_check(self, arguments):
        node_name = self.image_access_nodes[0]
        node = cluster.get_node(node_name)
        r, o = node.proxy.image.update_image(self.get_info(), {"check": arguments})
        if r != 0:
            raise Exception(o["out"])
        return o["out"]
