...
class RemoteInstall(object):
    ...
    def close(self):
        ...
        LOG.debug("unattended http server %s successfully created", self.get_url())
...
class UnattendedInstallConfig(object):
    ...
    def get_driver_hardware_id(self, driver, run_cmd=True):
        ...
        if not os.path.ismount(self.cdrom_mount_point):
            process.system(
                "mount %s %s -o loop" % (self.cdrom_virtio, self.cdrom_mount_point),
                timeout=60,
            )
        if not os.path.ismount(self.floppy_mount_point):
            process.system(
                "mount %s %s -o loop" % (self.virtio_floppy, self.floppy_mount_point),
                timeout=60,
            )
        drivers_d = []
        driver_link = None
    ...
    def update_driver_hardware_id(self, driver):
        ...
        if "hwid" in driver:
            ...
            hwid = utils_test.get_driver_hardware_id(
                d_link, mount_point, storage_path, run_cmd=run_cmd
            )
            if hwid:
                driver = driver.replace("hwidcmd", hwid.strip())
            else:
                raise exceptions.TestError(
                    "Can not find hwid from the driver" " inf file"
                )
        return driver
    ...
    def answer_kickstart(self, answer_path):
        ...
        if re.search(dummy_rh_system_stream_id_re, contents):
            ...
            contents = re.sub(
                dummy_rh_system_stream_id_re, rh_system_stream_id, contents
            )
        ...
        if re.search(dummy_rh_guest_compose_meta_re, contents):
            ...
            if cdrom_meta_cd1:
                cdrom_meta_cd1 = os.path.join(data_dir.get_data_dir(), cdrom_meta_cd1)
                with open(cdrom_meta_cd1, "r") as f:
                    rh_guest_compose_meta = f.read()
            contents = re.sub(
                dummy_rh_guest_compose_meta_re, rh_guest_compose_meta, contents
            )
        ...
        if re.search(dummy_repos_re, contents):
            ...
            for index, repo_url in enumerate(repo_list, 1):
                line = (
                    "repo --name=extra_repo%d --baseurl=%s --install "
    ...
    def answer_windows_ini(self, answer_path):
        ...
        if self.cdkey:
            ...
            LOG.error(
                "Param 'cdkey' required but not specified for "
        ...
        if self.install_virtio == "yes":
            parser.set(
                "Unattended", "OemPnPDriversPath", '"%s"' % self.virtio_network_path
            )
        else:
            parser.remove_option("Unattended", "OemPnPDriversPath")

        dummy_re_dirver = {
            "KVM_TEST_VIRTIO_NETWORK_INSTALLER": "virtio_network_installer_path",
            "KVM_TEST_VIRTIO_BALLOON_INSTALLER": "virtio_balloon_installer_path",
            "KVM_TEST_VIRTIO_QXL_INSTALLER": "virtio_qxl_installer_path",
        }
        dummy_re = ""
        ...
        for opt in parser.options("GuiRunOnce"):
            ...
            if re.search(dummy_process_re, check):
                process_check = re.sub(
                    dummy_process_re, "%s" % self.process_check, check
                )
    ...
    def answer_windows_xml(self, answer_path):
        ...
        if self.install_virtio == "yes":
            ...
            values = [
                self.virtio_scsi_path,
                self.virtio_storage_path,
                self.virtio_network_path,
                self.virtio_balloon_path,
                self.virtio_viorng_path,
                self.virtio_vioser_path,
                self.virtio_pvpanic_path,
    ...
    def set_unattended_param_in_kernel(self, unattended_file_url):
        ...
        if "%s=" % self.unattended_file_kernel_param_name in self.kernel_params:
            kernel_params = re.sub(
                "%s=[\w\d:\-\./]+" % (self.unattended_file_kernel_param_name),
                unattended_param,
                self.kernel_params,
            )
        else:
    ...
    def setup_unattended_http_server(self):
        ...
        """
        if self.unattended_file.endswith(".ks") or self.unattended_file.endswith(
            ".preseed"
        ):
        ...
        unattended_file_url = "http://%s:%s/%s" % (
            self.url_auto_content_ip,
            self.unattended_server_port,
            dest_fname,
        )
        kernel_params = self.set_unattended_param_in_kernel(unattended_file_url)
    ...
    def setup_boot_disk(self):
        if self.unattended_file.endswith(".sif"):
            ...
            boot_disk = utils_disk.FloppyDisk(
                self.floppy, self.qemu_img_binary, self.tmpdir, self.vfd_size
            )
            ...
            if self.install_virtio == "yes":
                boot_disk.setup_virtio_win2003(
                    self.virtio_floppy, self.virtio_oemsetup_id
                )
            ...
            if self.params.get("unattended_delivery_method") == "integrated":
                unattended_file_url = "cdrom:/dev/sr0:/isolinux/%s" % (dest_fname)
                kernel_params = self.set_unattended_param_in_kernel(unattended_file_url)
                ...
                if "repo=cdrom" in kernel_params:
                    kernel_params = re.sub(
                        "repo=cdrom[:\w\d\-/]*", "repo=cdrom:/dev/sr0", kernel_params
                    )
                ...
                raise ValueError(
                    "Neither cdrom_unattended nor floppy set "
            ...
            if "autoyast" in self.kernel_params:
                ...
                if (
                    self.cdrom_unattended
                    and self.params.get("unattended_delivery_method") == "cdrom"
                ):
                    boot_disk = utils_disk.CdromDisk(self.cdrom_unattended, self.tmpdir)
                elif self.floppy:
                    ...
                    if "autoyast=" in kernel_params:
                        kernel_params = re.sub(
                            "autoyast=[\w\d\-:\./]+", autoyast_param, kernel_params
                        )
                    else:
                ...
                if self.params.get("unattended_delivery_method") == "cdrom":
                    boot_disk = utils_disk.CdromDisk(self.cdrom_unattended, self.tmpdir)
                    if self.install_virtio == "yes":
                        boot_disk.setup_virtio_win2008(
                            self.virtio_floppy, self.cdrom_virtio
                        )
                    else:
    ...
    def setup_cdrom(self):
        ...
        error_context.context(
            "Copying vmlinuz and initrd.img from install cdrom %s" % self.cdrom_cd1
        )
        ...
        if self.params.get("vm_type") == "libvirt":
            if self.vm.driver_type == "qemu":
                ...
                if base_initrd != "initrd.img":
                    process.run("mv %s initrd.img" % base_initrd, verbose=DEBUG)
                if self.params.get("unattended_delivery_method") != "integrated":
                    i.close()
                ...
                start_auto_content_server_thread(
                    self.url_auto_content_port, self.cdrom_cd1_mount
                )
    ...
    def setup_url(self):
        ...
        if self.vm_type == "qemu":
            error_context.context("downloading vmlinuz/initrd.img from %s" % self.url)
            ...
            sha1sum_initrd_output = process.run(
                sha1sum_initrd_cmd, ignore_status=True, verbose=DEBUG
            ).stdout_text
            try:
            ...
            if not sha1sum_kernel == self.params.get("sha1sum_vmlinuz", None):
                ...
                download.get_file(
                    url_kernel,
                    os.path.join(self.image_path, os.path.basename(self.kernel)),
                )

            if not sha1sum_initrd == self.params.get("sha1sum_initrd", None):
            ...
            if "repo=cdrom" in self.kernel_params:
                # Red Hat
                self.kernel_params = re.sub(
                    "repo=[:\w\d\-/]*", "repo=%s" % self.url, self.kernel_params
                )
            ...
            LOG.info(
                "No action defined/needed for the current virt "
                "type: '%s'" % self.vm_type
            )
    ...
    def setup_nfs(self):
        ...
        """
        error_context.context("copying the vmlinuz and initrd.img from NFS share")
        ...
        if self.unattended_file:
            if self.floppy or self.cdrom_unattended:
                ...
                if self.params.get("store_boot_disk") == "yes":
                    LOG.info(
                        "Storing the boot disk to result directory " "for further debug"
                    )
...
def start_syslog_server_thread(address, port, tcp):
    ...
    syslog_server.set_default_format("[UnattendedSyslog " "(%s.%s)] %s")
...
def copy_file_from_nfs(src, dst, mount_point, image_name):
    LOG.info(
        "Test failed before the install process start."
    ...
    try:
        ...
        for i, console in enumerate(sessions):
            ...
            for path_glob in [
                ...
                try:
                    status, paths = console.cmd_status_output("ls -1 %s" % path_glob)
                    if status:
                        continue
                except Exception as details:
                    failures = True
                ...
                for path in paths.splitlines():
                    ...
                    if path.startswith(os.path.sep):
                        ...
                        rel_path = path
                    dst = os.path.join(test.outputdir, vm.name, str(i), rel_path)
                    ...
                    with open(dst, "w") as fd_dst:
                        try:
                            ...
                            LOG.info(
                                'Attached "%s" log file from guest ' 'at "%s"',
                                path,
                                base_dst_dir,
                            )
                        except Exception as details:
                            ...
                            fd_dst.write(
                                "Unknown exception while getting "
                                "content: %s" % details
                            )
                            failures = True
            for cmd in ["journalctl --no-pager"]:
                dst = os.path.join(
                    test.outputdir, vm.name, str(i), astring.string_to_safe_path(cmd)
                )
                with open(dst, "w") as fd_dst:
                    try:
...
def run(test, params, env):
    ...
    @error_context.context_aware
    def copy_images():
        error_context.base_context("Copy image from NFS after installation failure")
    ...
    install_error_exception_str = (
        "Installation error reported in serial " "console log: %s" % install_error_str
    )
    rh_upgrade_error_str = params.get(
        "rh_upgrade_error_str", "RH system upgrade failed"
    )
    ...
    LOG.info(
        "Waiting for installation to finish. Timeout set to %d s " "(%d min)",
        install_timeout,
        install_timeout // 60,
    )
    ...
    LOG.debug("Monitoring serial console log for completion message: %s", log_file)
    serial_read_fails = 0
    ...
    while (time.time() - start_time) < install_timeout:
        try:
            vm.verify_alive()
            if (
                params.get("send_key_at_install")
                ...
                try:
                    ...
                    if rh_upgrade_error_str_found:
                        raise exceptions.TestFail(
                            "rh system upgrade failed, please " "check serial log"
                        )
        ...
        copy_images()
        raise exceptions.TestFail(
            "Timeout elapsed while waiting for install to " "finish"
        )
    ...
    LOG.info(
        "Guest reported successful installation after %d s (%d min)",
        time_elapsed,
        time_elapsed // 60,
    )

    if params.get("shutdown_cleanly", "yes") == "yes":
        shutdown_cleanly_timeout = int(params.get("shutdown_cleanly_timeout", 120))
        ...
        try:
            ...
            LOG.warning("Guest apparently shut down, but got a " "monitor error: %s", e)
