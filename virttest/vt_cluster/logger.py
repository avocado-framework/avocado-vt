...
class _LoggerStreamHandler(socketserver.StreamRequestHandler):
    ...
    def handle_logger(self, record):
        ...
        _logger.info(format_str.format(asctime=record.asctime, module=record.name,
                                       levelname=record.levelname, msg=record.msg,
                                       client=client, address=self.client_address[0]))
...
class _Server(socketserver.ThreadingTCPServer):
    ...
    def run_server_forever(self):
        _logger.info("Run the logger server.")
        abort = False
        while not abort:
...
class LoggerServer(object):
    ...
    def start(self):
        ...
        self._thread = threading.Thread(
                target=self._server.run_server_forever,
                name='logger_server', args=())
