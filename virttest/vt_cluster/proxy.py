        ...
        try:
            ...
            try:
                if isinstance(kargs, dict):
                    # FIXME: Create the Exception instance on the master node
                    #  again with the error messages are from the worker node.
                    #  Both error instance are not same, there are some protential
                    #  issue.
                    # TODO: Return the error code and error message, add an error
                    #  handler to process those data for the upper lever.
                    if root_mod:
...
class _LocalProxy(object):
    def __init__(self):
        # FIXME: To be compatible with the remote node and local node for
