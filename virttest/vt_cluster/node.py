...
class Node(object):
    ...
    def start_agent_server(self):
        ...
        if not self._server_daemon_pid_file_path:
            raise NodeError(
                "PID file path is not configured for node %s." % self.name
            )
    ...
    def upload_console_log(self, target_path):
        """
        Upload console logs (e.g., from VMs or services) from the remote node.

        :param target_path: The local directory to upload console logs to.
        :type target_path: str
        """
        try:
            console_log_dir = self.proxy.api.get_console_log_dir()
            if console_log_dir:
                remote_path = os.path.join(console_log_dir, "*")
                self._upload_files(remote_path, target_path)
        except Exception as e:
            LOG.warning("Node %s: Failed to upload console log: %s", self.name, e)

    def upload_daemon_log(self, target_path):
        """
        Upload daemon-related logs from the remote node.

        :param target_path: The local directory to upload daemon logs to.
        :type target_path: str
        """
        try:
            daemon_log_dir = self.proxy.api.get_daemon_log_dir()
            if daemon_log_dir:
                remote_path = os.path.join(daemon_log_dir, "*")
                self._upload_files(remote_path, target_path)
        except Exception as e:
            LOG.warning("Node %s: Failed to upload daemon log: %s", self.name, e)

    def upload_ip_sniffer_log(self, target_path):
        """
        Upload IP sniffer logs from the remote node.

        :param target_path: The local directory to upload IP sniffer logs to.
        :type target_path: str
        """
        try:
            ip_sniffer_log_dir = self.proxy.api.get_ip_sniffer_log_dir()
            if ip_sniffer_log_dir:
                remote_path = os.path.join(ip_sniffer_log_dir, "*")
                self._upload_files(remote_path, target_path)
        except Exception as e:
            LOG.warning("Node %s: Failed to upload ip sniffer log: %s", self.name, e)
