...
class Node(object):
    ...
    def __eq__(self, other):
        """
        Check equality between two Node instances based on their address or hostname.
        ...
        if not isinstance(other, Node):
            return False
        if all((self._address, other._address)):
            return self._address == other._address
        elif all((self._hostname, other._hostname)):
    ...
    def _setup_agent_pkgs(self):
        ...
        4. Copying ``aexpect`` source to the agent and installing it in
           develop mode.
        """
        agent_pkg_path = os.path.join(os.path.dirname(avocado_vt.__file__), "vt_agent")
        ...
        try:
            ...
            session.cmd(
                "pip3 install -e %s" % os.path.join(self._agent_dir, "vt_agent")
            )
    ...
    def cleanup_agent_env(self):
        ...
        if self._agent_dir:
            ...
            try:
                agent_session.cmd_output("pip3 uninstall -y avocado_vt.agent")
    ...
    def _start_server_daemon(self, name, host, port):
        ...
        :param name: The module name of the daemon to start (e.g., 'avocado_vt.agent').
        :type name: str
