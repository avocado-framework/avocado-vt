...
AGENT_MOD = "vt_agent"
AGENT_MOD_PATH = os.path.join(os.path.dirname(os.path.dirname(__file__)), AGENT_MOD)
...
def _remote_login(client, host, port, username, password,
                  prompt, auto_close, timeout):
    cmd = ("ssh -o UserKnownHostsFile=/dev/null "
           "-o StrictHostKeyChecking=no -p %s" % port)
    cmd += " -o PreferredAuthentications=password"
    cmd += " %s@%s" % (username, host)

    session = RemoteSession(cmd, linesep="\n", prompt=prompt,
...
class Node(object):
    ...
    def _scp_from_remote(self, source, dest, timeout=600):
        remote.scp_to_remote(self._host, self.shell_port, self.username,
                             self.password, source, dest, timeout=timeout)
    ...
    def _start_server_daemon(self, name, host, port, pidfile):
        ...
        if not utils_misc.wait_for(
                lambda: (end_str in self._session_daemon.get_output()
                         and self.is_server_alive()), timeout):
            err_info = self._session_daemon.get_output()
            LOG.error("Failed to start the server daemon on %s.\n"
                      "The output:\n%s", self.name, err_info)
            return False
    ...
    def start_agent_server(self):
        ...
        if self._is_remote_node:
            pidfile = os.path.join(self.agent_server_dir,
                                   "agent_server_%s.pid" % self.name)
            if not self._start_server_daemon(
                    name=AGENT_MOD, host=self.address,
                    port=self.proxy_port, pidfile=pidfile):
    ...
    def upload_agent_log(self, target_path):
        ...
        if self._is_remote_node:
            ...
            remote.scp_from_remote(self._host, self.shell_port,
    ...
    def create_session(self, auto_close=True, timeout=300):
        ...
        session = _remote_login("ssh", self._host, self.shell_port,
                                self.username, self.password,
                                self.shell_prompt, auto_close, timeout)
        return session
