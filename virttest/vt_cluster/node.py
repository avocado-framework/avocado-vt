...
class Node(object):
    ...
    def _setup_agent_pkgs(self):
        ...
        4. Copying ``aexpect`` source to the agent and installing it in
           develop mode.
        """
        self._scp_to_remote(AGENT_MOD_PATH, self._agent_dir)

        dest_path = os.path.join(self._agent_dir, AGENT_MOD)
    ...
    def _start_server_daemon(self, name, host, port):
        ...
        pythonpath = os.path.join(self._agent_dir, AGENT_MOD)
    ...
    def start_agent_server(self):
        ...
        if not self._start_server_daemon(
            name=AGENT_MOD, host=self._host, port=self._proxy_port
        ):
