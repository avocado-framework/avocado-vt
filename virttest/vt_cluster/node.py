...
from virttest import data_dir, utils_misc, vt_agent
...
class Node(object):
    ...
    def __init__(self, params, name):
        ...
        is_master_node = self._params.get("master_node", "no") == "yes"
        self._uri = None if is_master_node else _uri
        self._agent_server_daemon_pid = None
        self._is_remote_node = not is_master_node
        ...
        if self._server_daemon_pid_file:
            ...
            try:
                ...
                raise NodeError(
                    ...
                    f"with {self._server_daemon_pid_file}: {e}"
                )
            finally:
                _session.close()
        return None
    ...
    def _setup_agent_server_pkgs(self):
        ...
        # FIXME: Copy the entire module file to the agent node
        for pkg in (avocado, virttest):
            pkg_path = os.path.dirname(inspect.getfile(pkg))
            self._scp_from_remote(pkg_path, dest_path)

        session = self._create_session()
        # FIXME: To disable the plugin and skip the "pkg_resources.DistributionNotFound",
        #  just need the avocado.utils module for providing the interfaces.
        target_file = os.path.join(dest_path, "avocado", "__init__.py")
        session.cmd("sed -i 's/initialize_plugins()//g' %s" % target_file)

        # FIXME: Fix the TypeError: expected str, bytes or os.PathLike object, not tuple
        ...
        # FIXME: Install the aexpect module on the agent node directly
    ...
    def setup_agent_env(self):
        """Set up the agent environment of node"""
        if self._is_remote_node:
    ...
    def _copy_qemu_cmdline_format_json(self):
        ...
        if os.path.exists(src_path):
            self._scp_from_remote(src_path, dst_path)
    ...
    def _upload_files(self, remote_path, target_path):
        """
        Upload the files to the master node.
        ...
        if self._is_remote_node:
            try:
                ...
                LOG.warning(f"Failed to upload {remote_path} to {target_path}: {e}")
            finally:
    ...
    def upload_console_log(self, target_path):
        """
        Upload the agent service log to the master node.
        ...
        """
        if self._is_remote_node:
            remote_path = os.path.join(self.proxy.api.get_console_log_dir(), "*")
            self._upload_files(remote_path, target_path)

    def upload_daemon_log(self, target_path):
        """
        Upload the agent service log to the master node.
        ...
        """
        if self._is_remote_node:
            remote_path = os.path.join(self.proxy.api.get_daemon_log_dir(), "*")
            self._upload_files(remote_path, target_path)

    def upload_ip_sniffer_log(self, target_path):
        """
        Upload the agent ip sniffer log to the master node.

        :param target_path: The path of target.
        :type target_path: str
        """
        if self._is_remote_node:
            remote_path = os.path.join(self.proxy.api.get_ip_sniffer_log_dir(), "*")
            self._upload_files(remote_path, target_path)
    ...
    def upload_logs(self, target_path):
        ...
        if self._is_remote_node:
            ...
            self.upload_console_log(target_path)
            self.upload_daemon_log(target_path)
            self.upload_ip_sniffer_log(target_path)
