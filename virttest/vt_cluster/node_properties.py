...
def save_properties():
    ...
    for node in cluster.get_all_nodes():
        ...
        props[node.name][
            "cpu_vendor_id"] = node.proxy.host.cpu.get_cpu_vendor_id()
        props[node.name][
            "cpu_model_name"] = node.proxy.host.cpu.get_cpu_model_name()
...
def load_properties():
    ...
    try:
        ...
        LOG.error(
            f"Failed to parse metadata file {cluster.metadata_file}: {e}")
        return {}
    except Exception as e:
        LOG.error(
            f"Failed to load properties from {cluster.metadata_file}: {e}")
        return {}
...
def remove_properties():
    ...
    try:
        ...
        LOG.warning(
            f"Could not remove metadata file {cluster.metadata_file}: {e}")
