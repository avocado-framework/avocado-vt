...
def rbd_image_exist(
    ...
    namespace = "/%s" % rbd_namespace_name if rbd_namespace_name else ""
    cmd = cmd.format(
        opts=opts, pool=rbd_pool_name, keyring=keyring, namespace=namespace
    )

    output = process.run(cmd, ignore_status=True, verbose=True).stdout_text
...
def get_image_filename(
    ceph_monitor, rbd_pool_name, rbd_image_name, ceph_conf=None, rbd_namespace_name=None
):
    """
    ...
    return uri.format(
        pool=rbd_pool_name, namespace=namespace, image=rbd_image_name, opts=opts
    )
    ...
    if session:
        ...
        if not os.path.exists(mount_point):
            try:
                ...
                LOG.debug(
                    "Creation of the directory:%s failed:%s", mount_point, str(dirError)
                )
            else:
    ...
    if session:
        ...
        if os.path.exists(mount_point):
            try:
                ...
                LOG.debug(
                    "Delete of the directory:%s failed:%s", mount_point, str(dirError)
                )
            else:
