...
class QemuDumpFileManager(GuestDumpFileManager):
    ...
    def run(self, image_path, res_dir):
        ...
        try:
            ...
            self._partition_list = (
                process.run("ls /dev/%s" % self._nbd_dev + "p*").stdout_text.strip().split()
            )
            ...
            for des, platform in _mapping.items():
                _output = process.run(
                    "fdisk -l %s | grep %s" % self._nbd_dev, des
                ).stdout_text
                if _output and _partition in _output:
                    self._guest_type = platform
                    self._nbd_dev_partition_path = _partition
            ...
            utils_misc.mount(
                "/dev/%s" % self._nbd_dev_partition_path,
                self._mount_pt,
                self._fs_support_mapping[self._guest_type](),
            )
            self._process_dump_file_from_guest(res_dir)
        finally:
            try:
                utils_misc.umount(
                    "/dev/%s" % self._nbd_dev_partition_path, self._mount_pt, None
                )
