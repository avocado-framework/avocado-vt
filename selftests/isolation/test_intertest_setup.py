...
from virttest import intertest_setup, utils_params
...
class IntertestSetupTest(Test):
    ...
    def test_update_default(self):
        ...
        DummyTestRun.asserted_tests = [
            ...
            {
                "shortname": "^original.unattended_install.*vm1",
                "vms": "^vm1$",
                "cdrom_cd1": r".*CentOS-8.*\.iso$",
            },
            ...
            {
                "shortname": "^original.unattended_install",
                "vms": "^vm2$",
                "cdrom_cd1": r".*win.*\.iso$",
            },
        ...
        self.assertEqual(
            DummyStateControl.asserted_states["unset"]["customize"][self.shared_pool], 0
        )
        # states after the updated path will be removed (default remove set is the entire graph)
        self.assertEqual(
            DummyStateControl.asserted_states["unset"]["on_customize"][
                self.shared_pool
            ],
            1,
        )
        self.assertEqual(
            DummyStateControl.asserted_states["unset"]["connect"][self.shared_pool], 1
        )
        self.assertEqual(
            DummyStateControl.asserted_states["unset"]["guisetup.clicked"][
                self.shared_pool
            ],
            1,
        )
        self.assertEqual(
            DummyStateControl.asserted_states["unset"]["getsetup.clicked"][
                self.shared_pool
            ],
            1,
        )
    ...
    def test_update_custom(self):
        ...
        self.assertEqual(
            DummyStateControl.asserted_states["unset"]["connect"][self.shared_pool], 0
        )
        # states after the updated path will be removed (default remove set is the entire graph)
    ...
    def test_update_install(self):
        ...
        for unset_state in DummyStateControl.asserted_states["unset"]:
            if unset_state == "install":
                self.assertEqual(
                    DummyStateControl.asserted_states["unset"][unset_state][
                        self.shared_pool
                    ],
                    0,
                )
    ...
    def test_update_remove_set(self):
        ...
        self.assertEqual(
            len(DummyTestRun.asserted_tests),
            0,
            "Some tests were run: %s" % DummyTestRun.asserted_tests,
        )
    ...
    def test_update_restrictions(self):
        ...
        DummyTestRun.asserted_tests = [
            {
                "shortname": "^internal.stateless.noop.vm1.+CentOS",
                "vms": "^vm1$",
                "type": "^shared_configure_install$",
                "nets": "^net1$",
            },
            ...
            {
                "shortname": "^original.unattended_install.*vm1.+CentOS",
                "vms": "^vm1$",
                "cdrom_cd1": r".*CentOS.*\.iso$",
                "nets": "^net1$",
            },
            {
                "shortname": "^internal.automated.customize.vm1.+CentOS",
                "vms": "^vm1$",
                "get_state_images": "^install$",
                "nets": "^net1$",
            },
            {
                "shortname": "^internal.stateless.noop.vm1.+Fedora",
                "vms": "^vm1$",
                "type": "^shared_configure_install$",
                "nets": "^net1$",
            },
            {
                "shortname": "^original.unattended_install.*vm1.+Fedora",
                "vms": "^vm1$",
                "cdrom_cd1": r".*Fedora.*\.iso$",
                "nets": "^net1$",
            },
            {
                "shortname": "^internal.automated.customize.vm1.+Fedora",
                "vms": "^vm1$",
                "get_state_images": "^install$",
                "nets": "^net1$",
            },
        ...
        # states after the updated path will be removed (default remove set is the entire graph)
        self.assertEqual(
            DummyStateControl.asserted_states["unset"]["linux_virtuser"][
                self.shared_pool
            ],
            2 * 2,
        )
        self.assertEqual(
            DummyStateControl.asserted_states["unset"]["windows_virtuser"][
                self.shared_pool
            ],
            1 * 2,
        )
    ...
    def test_multi_vm_manipulation(self):
        ...
        for vm_action in ["boot", "download", "upload", "shutdown"]:
            variant_action = (
                "start"
                if vm_action == "boot"
                else "stop" if vm_action == "shutdown" else vm_action
            )
            with self.subTest(f"Multi-vm {vm_action} ({variant_action})"):
                DummyTestRun.asserted_tests = [
                    ...
                    {
                        "shortname": f"^internal.stateless.manage.{variant_action}.vm2.+.vm3.+Ubuntu",
                        "vms": "^vm2 vm3$",
                        "nets": "^net1$",
        ...
        for state_action in ["check", "pop", "push", "get", "set", "unset"]:
            with self.subTest(f"Manual state {state_action}"):
                DummyStateControl.asserted_states["get"] = {
                    "root": {self.shared_pool: 0}
                }
                DummyTestRun.asserted_tests = [
                    # the order does not diverge (which is desirable here) since similar nodes are not bridged
                    {
                        "shortname": f"^internal.stateful.{state_action}.vm2",
                        "vms": "^vm2$",
                        "nets": "^net1$",
                        "skip_image_processing": "^yes$",
                        "vm_action": "^%s$" % state_action,
                    },
                    # vm2 is incompatible with net5 so skipped
                    {
                        "shortname": f"^internal.stateful.{state_action}.vm3.+Kali",
                        "vms": "^vm3$",
                        "nets": "^net5$",
                        "skip_image_processing": "^yes$",
                        "vm_action": "^%s$" % state_action,
                    ...
                    {
                        "shortname": f"^internal.stateful.{state_action}.vm3.+Ubuntu",
                        "vms": "^vm3$",
                        "nets": "^net5$",
                        "skip_image_processing": "^yes$",
                        "vm_action": "^%s$" % state_action,
        ...
        for state_action in ["collect", "create", "clean"]:
            ...
            with self.subTest(f"Manual state {state_action}"):
                DummyTestRun.asserted_tests = [
                    # the order does not diverge (which is desirable here) since similar nodes are not bridged
                    {
                        "shortname": f"^internal.stateful.{operation}.vm2",
                        "vms": "^vm2$",
                        "nets": "^net1$",
                        "skip_image_processing": "^yes$",
                        "vm_action": "^%s$" % operation,
                    },
                    {
                        "shortname": f"^internal.stateful.{operation}.vm3.+Kali",
                        "vms": "^vm3$",
                        "nets": "^net5$",
                        "skip_image_processing": "^yes$",
                        "vm_action": "^%s$" % operation,
                    ...
                    {
                        "shortname": f"^internal.stateful.{operation}.vm3.+Ubuntu",
                        "vms": "^vm3$",
                        "nets": "^net5$",
                        "skip_image_processing": "^yes$",
                        "vm_action": "^%s$" % operation,
                ...
                for test_dict in DummyTestRun.asserted_tests:
                    ...
                    test_dict[operation + "_mode_images"] = (
                        "^af$" if operation == "set" else "^fa$"
                    )
                    test_dict[operation + "_mode_images"] = (
                        "^ii$"
                        if operation == "get"
                        else test_dict[operation + "_mode_images"]
                    )
                setup_func = getattr(intertest_setup, state_action)
    ...
    def test_permanent_vm_tool(self):
        ...
        DummyTestRun.asserted_tests = [
            {
                "shortname": "^internal.stateless.noop.vm3",
                "vms": "^vm3$",
                "type": "^shared_configure_install$",
            },
            {
                "shortname": "^original.unattended_install.*vm3",
                "vms": "^vm3$",
                "cdrom_cd1": r".*ubuntu-14.04.*\.iso$",
                "set_state_images": "^install$",
            },
            {
                "shortname": "^internal.automated.customize.vm3",
                "vms": "^vm3$",
                "set_state_images": "^customize$",
            },
            {
                "shortname": "^internal.stateless.manage.start.vm3",
                "vms": "^vm3$",
                "set_state_vms": "^ready$",
            },
        ]
