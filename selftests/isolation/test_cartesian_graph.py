...
# this package is not treated as local folder due to its ad-hoc introduction
from unittest_utils import DummyStateControl, DummyTestRun
...
class CartesianWorkerTest(Test):
    ...
    def test_parse_flat_objects_vm(self):
        ...
        self.assertRegex(
            test_objects[1].params["name"], r"vms.vm1\.qemu_kvm_centos.*CentOS.*"
        )
        self.assertEqual(test_objects[1].params["vms"], "vm1")
        self.assertEqual(test_objects[1].params["os_variant"], "centos")
        self.assertRegex(
            test_objects[0].params["name"], r"vms.vm1\.qemu_kvm_fedora.*Fedora.*"
        )
        ...
        self.assertRegex(
            test_object.params["name"], r"vms.vm1\.qemu_kvm_centos.*CentOS.*"
        )

        test_object = TestGraph.parse_flat_objects(
            "vm1", "vms", "no CentOS\nonly qcow2\n", unique=True
        )
        self.assertRegex(
            test_object.params["name"], r"vms.vm1\.qemu_kvm_fedora.*qcow2.*Fedora.*"
        )
    ...
    def test_parse_flat_objects_net(self):
        ...
        test_object = TestGraph.parse_flat_objects(
            "net1", "nets", "localhost", unique=True
        )
        self.assertRegex(test_object.params["name"], r"nets\.localhost\.net1")

        test_object = TestGraph.parse_flat_objects(
            "net1", "nets", "no remotehost\nonly localhost\n", unique=True
        )
        self.assertRegex(test_object.params["name"], r"nets\.localhost\.net1")
    ...
    def test_params(self):
        ...
        for key in test_worker.params.keys():
            self.assertEqual(
                test_worker.net.params[key],
                test_worker.params[key],
    ...
    def test_params_slots(self):
        ...
        test_workers = TestGraph.parse_workers(
            {"nets": "net3 cluster1.net6 net0", "slots": "1 remote.com/2 "}
        )
        self.assertEqual(len(test_workers), 3)
        ...
        self.assertEqual(
            TestSwarm.run_swarms["localhost"].workers,
            [test_workers[0], test_workers[2]],
        )
    ...
    def test_restrs(self):
        ...
        for key in test_worker.restrs.keys():
            self.assertEqual(
                test_worker.net.restrs[key],
                test_worker.restrs[key],
...
class CartesianObjectTest(Test):
    ...
    def test_parse_composite_objects_vm(self):
        ...
        self.assertEqual(len(test_objects), 2)
        self.assertRegex(
            test_objects[1].params["name"], r"vm1\.qemu_kvm_centos.*CentOS.*"
        )
        self.assertEqual(test_objects[1].params["vms"], "vm1")
        self.assertEqual(test_objects[1].params["main_vm"], "vm1")
        self.assertEqual(test_objects[1].params["os_variant"], "centos")
        self.assertEqual(
            test_objects[1].params["cdrom_cd_rip"], "/mnt/local/isos/avocado_rip.iso"
        )
        self.assertRegex(
            test_objects[0].params["name"], r"vm1\.qemu_kvm_fedora.*Fedora.*"
        )
        self.assertEqual(test_objects[0].params["vms"], "vm1")
        self.assertEqual(test_objects[0].params["main_vm"], "vm1")
        self.assertEqual(test_objects[0].params["os_variant"], "fedora")
        self.assertEqual(
            test_objects[0].params["cdrom_cd_rip"], "/mnt/local/isos/avocado_rip.iso"
        )
        self.assertNotIn("only", test_objects[0].params)

        test_object = TestGraph.parse_composite_objects(
            "vm1", "vms", "CentOS", unique=True
        )
    ...
    def test_parse_composite_objects_net(self):
        ...
        test_object = TestGraph.parse_composite_objects(
            "net1", "nets", "", self.config["vm_strs"], unique=True
        )
        self.assertRegex(
            test_object.params["name"],
            r"vm1\.qemu_kvm_centos.*CentOS.*vm2\.qemu_kvm_windows_10.*Win10.*.vm3.qemu_kvm_ubuntu.*Ubuntu.*",
        )
        ...
        self.assertEqual(
            test_object.params["cdrom_cd_rip"], "/mnt/local/isos/avocado_rip.iso"
        )
        ...
        test_object = TestGraph.parse_composite_objects(
            "net1", "nets", "localhost", self.config["vm_strs"], unique=True
        )
        self.assertRegex(test_object.params["name"], r"nets\.localhost\.net1")
        ...
        with self.assertRaises(param.EmptyCartesianProduct):
            TestGraph.parse_composite_objects(
                "net5", "nets", "", {"vm1": "only CentOS\n"}
            )
        with self.assertRaises(param.EmptyCartesianProduct):
            TestGraph.parse_composite_objects(
                "net5", "nets", "", {"vm1": "no Fedora\n"}
            )
        with self.assertRaises(param.EmptyCartesianProduct):
            TestGraph.parse_composite_objects(
                "net5", "nets", "", {"vm2": "only Win7\n"}
            )
        with self.assertRaises(param.EmptyCartesianProduct):
    ...
    def test_parse_composite_objects_net_and_cluster(self):
        ...
        test_object = TestGraph.parse_composite_objects(
            "cluster1.net6", "nets", "", self.config["vm_strs"], unique=True
        )
        self.assertRegex(
            test_object.params["name"],
            r"vm1\.qemu_kvm_centos.*CentOS.*vm2\.qemu_kvm_windows_10.*Win10.*.vm3.qemu_kvm_ubuntu.*Ubuntu.*",
        ...
        test_object = TestGraph.parse_composite_objects(
            "net6", "nets", "cluster1", self.config["vm_strs"], unique=True
        )
        self.assertRegex(test_object.params["name"], r"nets\.cluster1\.net6")
        ...
        with self.assertRaises(param.EmptyCartesianProduct):
            TestGraph.parse_composite_objects(
                "net9", "nets", "cluster2", {"vm1": "only Fedora\n"}
            )
        with self.assertRaises(param.EmptyCartesianProduct):
            TestGraph.parse_composite_objects(
                "net9", "nets", "cluster2", {"vm1": "no CentOS\n"}
            )
        with self.assertRaises(param.EmptyCartesianProduct):
            TestGraph.parse_composite_objects(
                "net9", "nets", "cluster2", {"vm2": "only Win10\n"}
            )
        with self.assertRaises(param.EmptyCartesianProduct):
            TestGraph.parse_composite_objects(
                "net9", "nets", "cluster2", {"vm2": "no Win7\n"}
            )
    ...
    def test_parse_suffix_objects_vms(self):
        ...
        test_objects = TestGraph.parse_suffix_objects(
            "vms", self.config["vm_strs"], self.config["param_dict"]
        )
    ...
    def test_parse_suffix_objects_nets_flat(self):
        ...
        test_objects = TestGraph.parse_suffix_objects(
            "nets", self.config["net_strs"], self.config["param_dict"], flat=True
        )
    ...
    def test_parse_components_for_vm(self):
        ...
        test_objects = TestGraph.parse_components_for_object(
            flat_vm, "vms", unflatten=True
        )
        vms = [o for o in test_objects if o.key == "vms"]
        ...
        self.assertNotEqual(vms_vm1[0].id, vms_vm1[1].id)
        self.assertEqual(
            len([image for image in images if image.long_suffix == "image1_vm1"]), 1
        )
    ...
    def test_parse_components_for_net(self):
        ...
        assertVariant(
            nets[0],
            r"vm1\.qemu_kvm_fedora.*qcow.*Fedora.*vm2\.qemu_kvm_windows_7.*qcow.*Win7.*.vm3.qemu_kvm_ubuntu.*qcow.*Ubuntu.*",
            "fedora",
            "win7",
            "ubuntu",
        )
        assertVariant(
            nets[1],
            r"vm1\.qemu_kvm_fedora.*qcow.*Fedora.*vm2\.qemu_kvm_windows_7.*qcow.*Win7.*.vm3.qemu_kvm_kali.*qcow.*Kali.*",
            "fedora",
            "win7",
            "kl",
        )
        assertVariant(
            nets[2],
            r"vm1\.qemu_kvm_fedora.*qcow.*Fedora.*vm2\.qemu_kvm_windows_10.*qcow.*Win10.*.vm3.qemu_kvm_ubuntu.*qcow.*Ubuntu.*",
            "fedora",
            "win10",
            "ubuntu",
        )
        assertVariant(
            nets[3],
            r"vm1\.qemu_kvm_fedora.*qcow.*Fedora.*vm2\.qemu_kvm_windows_10.*qcow.*Win10.*.vm3.qemu_kvm_kali.*qcow.*Kali.*",
            "fedora",
            "win10",
            "kl",
        )
        assertVariant(
            nets[4],
            r"vm1\.qemu_kvm_centos.*qcow.*CentOS.*vm2\.qemu_kvm_windows_7.*.qcow.*Win7.*vm3.qemu_kvm_ubuntu.*qcow.*Ubuntu.*",
            "centos",
            "win7",
            "ubuntu",
        )
        assertVariant(
            nets[5],
            r"vm1\.qemu_kvm_centos.*qcow.*CentOS.*vm2\.qemu_kvm_windows_7.*qcow.*Win7.*.vm3.qemu_kvm_kali.*qcow.*Kali.*",
            "centos",
            "win7",
            "kl",
        )
        assertVariant(
            nets[6],
            r"vm1\.qemu_kvm_centos.*qcow.*CentOS.*vm2\.qemu_kvm_windows_10.*qcow.*Win10.*.vm3.qemu_kvm_ubuntu.*qcow.*Ubuntu.*",
            "centos",
            "win10",
            "ubuntu",
        )
        assertVariant(
            nets[7],
            r"vm1\.qemu_kvm_centos.*qcow.*CentOS.*vm2\.qemu_kvm_windows_10.*qcow.*Win10.*.vm3.qemu_kvm_kali.*qcow.*Kali.*",
            "centos",
            "win10",
            "kl",
        )
    ...
    def test_parse_components_for_net_restricted(self):
        ...
        flat_net = TestGraph.parse_flat_objects(
            "net1", "nets", params={"only_vm1": "CentOS"}, unique=True
        )
        test_objects = TestGraph.parse_components_for_object(
            flat_net, "nets", restriction="qcow2", unflatten=True
        )
        nets = [o for o in test_objects if o.key == "nets"]
        ...
        assertVariant(
            nets[0],
            r"vm1\.qemu_kvm_centos.*CentOS.*vm2\.qemu_kvm_windows_7.*Win7.*.vm3.qemu_kvm_ubuntu.*Ubuntu.*",
            "centos",
            "win7",
            "ubuntu",
        )
        assertVariant(
            nets[1],
            r"vm1\.qemu_kvm_centos.*CentOS.*vm2\.qemu_kvm_windows_7.*Win7.*.vm3.qemu_kvm_kali.*Kali.*",
            "centos",
            "win7",
            "kl",
        )
        ...
        assertVariant(
            nets[3],
            r"vm1\.qemu_kvm_centos.*CentOS.*vm2\.qemu_kvm_windows_10.*Win10.*.vm3.qemu_kvm_kali.*Kali.*",
            "centos",
            "win10",
            "kl",
        )
    ...
    def test_params(self):
        ...
        regenerated_params = test_object.object_typed_params(
            test_object.recipe.get_params()
        )
        self.assertEqual(
            len(regenerated_params.keys()),
            len(test_object.params.keys()),
            "The parameters of a test object must be the same as its only parser dictionary",
        )
        for key in regenerated_params.keys():
            self.assertEqual(
                regenerated_params[key],
                test_object.params[key],
                "The values of key %s %s=%s must be the same"
                % (key, regenerated_params[key], test_object.params[key]),
            )
    ...
    def test_restrs(self):
        ...
        test_objects += TestGraph.parse_composite_objects(
            "net1", "nets", "", restr_strs, params=restr_params
        )
        ...
        for test_object in test_objects:
            ...
            self.assertIn("vm1", test_object.restrs)
            self.assertEqual(
                test_object.restrs["vm1"], "only CentOS\nonly qcow2\nno qcow1\n"
            )
            self.assertIn("vm2", test_object.restrs)
            self.assertEqual(test_object.restrs["vm2"], "only Win10,Win7\n")
            self.assertIn("vm3", test_object.restrs)
        ...
        self.assertEqual(
            len(TestGraph.parse_flat_objects("net5", "nets", "", params=restr_params)),
            1,
        )
    ...
    def test_sanity_in_graph(self):
        ...
        test_objects = graph.get_objects(
            param_val="vm1", subset=graph.get_objects(param_val="net1")
        )
        for test_object in test_objects:
...
class CartesianNodeTest(Test):
    ...
    def test_prefix_tree_insert(self):
        ...
        self.assertEqual(
            tree.variant_nodes.keys(), set(["aaa", "bbb", "ccc", "eee", "fff"])
        )
        ...
        self.assertNotIn(
            tree.variant_nodes["bbb"][1].children["fff"],
            tree.variant_nodes["bbb"][0].children,
        )
    ...
    def test_parse_flat_nodes(self):
        ...
        test_node = TestGraph.parse_flat_nodes(
    ...
    def test_parse_node_from_object(self):
        ...
        test_objects = TestGraph.parse_components_for_object(
            flat_net, "nets", params=self.config["param_dict"], unflatten=True
        )
        nets = [o for o in test_objects if o.key == "nets"]
        self.assertEqual(len(nets), 1)
        net = nets[0]
        node = TestGraph.parse_node_from_object(
            net, "all..tutorial_get..explicit_noop", params=self.config["param_dict"]
        )
    ...
    def test_parse_node_from_object_invalid_object_type(self):
        ...
        flat_net = TestGraph.parse_net_from_object_restrs(
            "net1", {"vm1": self.config["vm_strs"]["vm1"]}
        )
        test_objects = TestGraph.parse_components_for_object(
            flat_net, "nets", params=self.config["param_dict"], unflatten=True
        )
    ...
    def test_parse_node_from_object_invalid_object_mix(self):
        ...
        with self.assertRaises(param.EmptyCartesianProduct):
            TestGraph.parse_node_from_object(
                net,
                "all..tutorial3.remote.object.control.decorator.util",
                params=self.config["param_dict"],
            )
    ...
    def test_get_and_parse_objects_for_node_and_object_flat(self):
        ...
        get_objects, parse_objects = graph.get_and_parse_objects_for_node_and_object(
            flat_node, flat_object
        )
        ...
        self.assertEqual(
            test_objects[0].components[0].components[0].long_suffix, "image1_vm1"
        )
        self.assertEqual(
            test_objects[0].params["nets_some_key"], flat_object.params["nets_some_key"]
        )
        self.assertEqual(
            test_objects[0].params["cdrom_cd_rip"], "/mnt/local/isos/avocado_rip.iso"
        )
    ...
    def test_get_and_parse_objects_for_node_and_object_full(self):
        ...
        graph = TestGraph()
        flat_node = TestGraph.parse_flat_nodes("normal..tutorial1", unique=True)
        full_object = TestGraph.parse_composite_objects(
            "net1", "nets", "", self.config["vm_strs"], unique=True
        )
        ...
        self.assertEqual(
            test_objects[0].params["nets_some_key"], full_object.params["nets_some_key"]
        )
        self.assertEqual(
            test_objects[0].params["cdrom_cd_rip"], "/mnt/local/isos/avocado_rip.iso"
        )
    ...
    def test_parse_nodes_from_flat_node_and_object(self):
        ...
        test_objects = TestGraph.parse_components_for_object(
            flat_object, "nets", unflatten=True
        )
        nets = [o for o in test_objects if o.key == "nets"]
        self.assertEqual(len(nets), 2)
        graph = TestGraph()
        ...
        for flat_node in flat_nodes:
            ...
            for node in nodes:
                ...
                self.assertEqual(
                    node.params["cdrom_cd_rip"], "/mnt/local/isos/avocado_rip.iso"
                )
    ...
    def test_get_and_parse_nodes_from_flat_node_and_object_unique(self):
        ...
        for flat_node in flat_nodes:
            ...
            get_nodes, parse_nodes = (
                graph.get_and_parse_nodes_from_flat_node_and_object(
                    flat_node, flat_object, params=self.config["param_dict"]
                )
            )
    ...
    def test_parse_composite_nodes(self):
        ...
        nodes = graph.parse_composite_nodes(
            "normal..tutorial1,normal..tutorial2", flat_object
        )
        self.assertEqual(len(nodes), 4)
    ...
    def test_parse_composite_nodes_compatibility_complete(self):
        ...
        self.assertEqual(
            ...
            [
                ...
                if o.key == "nets"
                if "vm1." in o.params["name"] and "vm2." in o.params["name"]
            ],
        )
        ...
        nodes = graph.parse_composite_nodes(
            self.config["tests_str"], flat_object, params=self.config["param_dict"]
        )
    ...
    def test_get_and_parse_composite_nodes(self):
        ...
        get_nodes, parse_nodes = graph.get_and_parse_composite_nodes(
            self.config["tests_str"], flat_object, params=self.config["param_dict"]
        )
    ...
    def test_get_and_parse_nodes_from_composite_node_and_object_unique(self):
        ...
        get_nodes, parse_nodes = (
            graph.get_and_parse_nodes_from_composite_node_and_object(
                full_node, full_net
            )
        )
        self.assertEqual(len(get_nodes), 0)
        ...
        self.assertEqual(
            full_vm.params["cdrom_cd_rip"], "/mnt/local/isos/avocado_rip.iso"
        )
        get_nodes, parse_nodes = (
            graph.get_and_parse_nodes_from_composite_node_and_object(full_node, full_vm)
        )
        self.assertEqual(len(get_nodes), 0)
        self.assertEqual(len(parse_nodes), 1)
        ...
        self.assertEqual(
            new_node.params["cdrom_cd_rip"], "/mnt/local/isos/avocado_rip.iso"
        )
        graph.new_nodes(parse_nodes)
        get_nodes, parse_nodes = (
            graph.get_and_parse_nodes_from_composite_node_and_object(full_node, full_vm)
    ...
    def test_get_and_parse_nodes_from_composite_node_and_object_multiple(self):
        ...
        nodes, objects = TestGraph.parse_object_nodes(
            None,
            "leaves..tutorial_get..implicit_both",
            prefix=self.prefix,
            object_restrs=self.config["vm_strs"],
            params=self.config["param_dict"],
        )
        ...
        full_image = [
            o for o in objects if o.key == "images" and o.long_suffix == "image1_vm2"
        ][0]
        get_nodes, parse_nodes = (
            graph.get_and_parse_nodes_from_composite_node_and_object(
                full_node, full_image
    ...
    def test_get_and_parse_nodes_from_composite_node_and_object_unique_multiobject(
        self,
    ):
        ...
        nodes, objects = TestGraph.parse_object_nodes(
            None,
            "all..tutorial_get..explicit_noop",
            prefix=self.prefix,
            object_restrs=self.config["vm_strs"],
            params=self.config["param_dict"],
        )
        ...
        reused_nodes, _ = TestGraph.parse_object_nodes(
            None,
            "all..tutorial_gui..client_noop",
            prefix=self.prefix,
            object_restrs=self.config["vm_strs"],
            params=self.config["param_dict"],
        )
    ...
    def test_get_and_parse_nodes_from_composite_node_and_object_unique_with_cloning(
        self,
    ):
        ...
        final_node = graph.parse_composite_nodes(
            "leaves..tutorial_finale", full_node.objects[0], unique=True
        )

        # if unique dependency is cloned at some later point preserve default unique node reuse
        get_nodes, parse_nodes = (
            graph.get_and_parse_nodes_from_composite_node_and_object(
                final_node, full_image
            )
        )
        self.assertEqual(len(get_nodes), 0)
        self.assertEqual(len(parse_nodes), 1)
        ...
        _, parse_nodes = graph.get_and_parse_nodes_from_composite_node_and_object(
            full_node, full_image
        )
        clones = graph.parse_cloned_branches_for_node_and_object(
            full_node, full_image, parse_nodes
        )
    ...
    def test_get_and_parse_nodes_from_composite_node_and_object_with_leaves(self):
        ...
        nodes, objects = TestGraph.parse_object_nodes(
            None,
            "all..tutorial_get.explicit_clicked",
            prefix=self.prefix,
            object_restrs=self.config["vm_strs"],
            params=self.config["param_dict"],
        )
        ...
        full_image1 = [
            o for o in objects if o.key == "images" and "vm1" in o.long_suffix
        ][0]
        get_nodes, parse_nodes = (
            graph.get_and_parse_nodes_from_composite_node_and_object(
                full_node, full_image1
            )
        )
        self.assertEqual(len(get_nodes), 0)
        self.assertEqual(len(parse_nodes), 1)
        ...
        full_image2 = [
            o for o in objects if o.key == "images" and "vm2" in o.long_suffix
        ][0]
        get_nodes, parse_nodes = (
            graph.get_and_parse_nodes_from_composite_node_and_object(
                full_node, full_image2
            )
        )
        self.assertEqual(len(get_nodes), 0)
        self.assertEqual(len(parse_nodes), 1)
        # we are adding a leaf node that should be reused as the setup of this node
        leaf_nodes = graph.parse_composite_nodes(
            "leaves..tutorial_gui.client_clicked", full_net
        )
        ...
        full_image3 = [
            o for o in objects if o.key == "images" and "vm3" in o.long_suffix
        ][0]
        get_nodes, parse_nodes = (
            graph.get_and_parse_nodes_from_composite_node_and_object(
                full_node, full_image3
            )
        )
        self.assertEqual(len(get_nodes), 0)
        self.assertEqual(len(parse_nodes), 0)
    ...
    def test_params(self):
        ...
        graph = TestGraph()
        test_node = graph.parse_composite_nodes(
            "normal..tutorial1", flat_net, unique=True
        )
    ...
    def test_restrs(self):
        ...
        restr_strs.update({"vm2": "only Win10,Win7\n", "vm3": ""})
        flat_net = TestGraph.parse_flat_objects(
            "net1", "nets", params=restr_params, unique=True
        )
        graph = TestGraph()

        test_nodes = TestGraph.parse_flat_nodes(
            "all..explicit_clicked", params=restr_params
        )
        ...
        for test_node in test_nodes:
            ...
            if test_nodes.index(test_node) == 0:
                ...
                self.assertEqual(
                    test_node.restrs["vm1"], "only CentOS\nonly qcow2\nno qcow1\n"
                )
                self.assertIn("vm2", test_node.restrs)
                self.assertEqual(test_node.restrs["vm2"], "only Win10,Win7\n")
                self.assertIn("vm3", test_node.restrs)
    ...
    def test_shared_workers(self):
        ...
        flat_net1 = TestGraph.parse_flat_objects(
            "net1", "nets", params={"only_vm1": "CentOS"}, unique=True
        )
        flat_net2 = TestGraph.parse_flat_objects(
            "net2", "nets", params={"only_vm1": "CentOS"}, unique=True
        )
        graph = TestGraph()
        test_node1 = graph.parse_composite_nodes(
            "normal..tutorial1", flat_net1, unique=True
        )
        test_node2 = graph.parse_composite_nodes(
            "normal..tutorial1", flat_net2, unique=True
        )
        ...
        self.assertEqual(
            test_node1.shared_finished_workers, test_node2.shared_finished_workers
        )
        ...
        self.assertEqual(
            test_node3.shared_finished_workers, {test_node3.finished_worker}
        )
    ...
    def test_shared_involved_workers(self):
        ...
        graph = TestGraph()
        test_node1 = graph.parse_composite_nodes(
            "leaves..explicit_noop", flat_net1, unique=True
        )
        test_node2 = graph.parse_composite_nodes(
            "leaves..explicit_noop", flat_net2, unique=True
        )
        test_node1.bridge_with_node(test_node2)
    ...
    def test_shared_results_and_worker_ids(self):
        ...
        self.assertEqual(
            test_node2.shared_result_worker_ids, test_node1.shared_result_worker_ids
        )
        ...
        self.assertEqual(test_node2.results, node2_results)
        self.assertEqual(
            test_node1.shared_results,
            node1_results + node1_extra_results + node2_results,
        )
        self.assertEqual(
            test_node2.shared_results,
            node2_results + node1_results + node1_extra_results,
        )
    ...
    def test_shared_results_filter(self):
        ...
        flat_net1 = TestGraph.parse_flat_objects(
            "cluster1.net6", "nets", params={"only_vm1": "CentOS"}, unique=True
        )
        flat_net2 = TestGraph.parse_flat_objects(
            "cluster2.net6", "nets", params={"only_vm1": "CentOS"}, unique=True
        )
        worker1, worker2 = TestWorker(flat_net1), TestWorker(flat_net2)
        swarm1, swarm2 = TestSwarm("localhost", [worker1]), TestSwarm(
            "localhost", [worker2]
        )
        ...
        test_node1 = graph.parse_composite_nodes(
            ...
            flat_net1,
            unique=True,
            params={"pool_scope": "own swarm shared"},
        )
        test_node2 = graph.parse_composite_nodes(
            "normal..tutorial1",
            flat_net2,
        ...
        self.assertEqual(
            test_node1.shared_filtered_results, test_node2.shared_filtered_results
        )
    ...
    def test_setless_form(self):
        ...
        graph = TestGraph()
        nodes = [
            *graph.parse_composite_nodes(
                "leaves..tutorial_get.explicit_clicked", flat_net
            ),
            *graph.parse_composite_nodes(
                "all..tutorial_get.explicit_clicked", flat_net
            ),
        ]
        self.assertEqual(len(nodes), 2)
    ...
    def test_bridged_form(self):
        ...
        flat_net1 = TestGraph.parse_flat_objects(
            "net1", "nets", params=restriction_params, unique=True
        )
        flat_net2 = TestGraph.parse_flat_objects(
            "net2", "nets", params=restriction_params, unique=True
        )

        graph = TestGraph()
        nodes = [
            *graph.parse_composite_nodes(
                "all..tutorial_get.explicit_clicked", flat_net1
            ),
            *graph.parse_composite_nodes(
                "all..tutorial_get.explicit_clicked", flat_net2
            ),
        ]
        self.assertEqual(len(nodes), 2)
        self.assertNotEqual(nodes[0].params["name"], nodes[1].params["name"])
    ...
    def test_sanity_in_graph(self):
        ...
        test_node = graph.get_nodes(
            param_val="tutorial1", subset=graph.get_nodes(param_val="net1"), unique=True
        )
    ...
    def test_is_unrolled(self):
        ...
        flat_node = TestGraph.parse_flat_nodes("normal..tutorial1", unique=True)
        more_composite_nodes = graph.parse_composite_nodes(
            "normal..tutorial2", flat_object
        )
    ...
    def test_is_ready(self):
        ...
        full_components = TestGraph.parse_components_for_object(
            flat_net, "nets", "", unflatten=True
        )
        full_net = full_components[-1]

        node = TestGraph.parse_node_from_object(full_net, "normal..tutorial1")
        node1 = TestGraph.parse_node_from_object(
            full_net, "normal..tutorial1", prefix="1"
        )
        node2 = TestGraph.parse_node_from_object(
            full_net, "normal..tutorial2", prefix="2"
        )
    ...
    def test_is_started_or_finished(self):
        ...
        nets = " ".join(
            param.all_suffixes_by_restriction(
                "only cluster1,cluster2\nno cluster2.net8,net9\n"
            )
        )
        test_workers = TestGraph.parse_workers({"nets": nets, "only_vm1": "CentOS"})
        full_nodes = []
        for worker in test_workers:
            new_node = TestGraph().parse_composite_nodes(
                "normal..tutorial1", worker.net, prefix="1", unique=True
            )
    ...
    def test_is_started_or_finished_filter(self):
        ...
        nets = " ".join(
            param.all_suffixes_by_restriction(
                "only cluster1,cluster2.net6,cluster2.net7\n"
            )
        )
        test_workers = TestGraph.parse_workers({"nets": nets, "only_vm1": "CentOS"})
        full_nodes = []
    ...
    def test_should_parse(self):
        ...
        worker1 = TestWorker(flat_net1)
        flat_net2 = TestGraph.parse_flat_objects(
            "net2", "nets", params={"only_vm1": ""}, unique=True
        )
        worker2 = TestWorker(flat_net2)
        swarm = TestSwarm("localhost", [worker1, worker2])
        TestSwarm.run_swarms = {swarm.id: swarm}
        flat_node._picked_by_setup_nodes.register(
            mock.MagicMock("some_setup", bridged_form="setup"), worker1
        )
        flat_node._picked_by_setup_nodes.register(
            mock.MagicMock("some_setup", bridged_form="setup"), worker2
        )
        ...
        flat_net3 = TestGraph.parse_flat_objects(
            "net3", "nets", params={"only_vm1": ""}, unique=True
        )
    ...
    def test_default_run_decision(self):
        ...
        DummyStateControl.asserted_states["check"] = {
            "install": {self.shared_pool: False}
        }
    ...
    def test_prefix_priority(self):
        ...
        self.assertEqual(
            TestNode.prefix_priority("3b1-net1vm1vm2vm3", "3a1-net1vm1vm2vm3"), 1
        )
        self.assertEqual(
            TestNode.prefix_priority("3c1-net1vm1vm2vm3", "3b1-net1vm1vm2vm3"), 1
        )
        self.assertEqual(
            TestNode.prefix_priority("3d1-net1vm1vm2vm3", "3c1-net1vm1vm2vm3"), 1
        )
        # nested matching
        self.assertEqual(
            TestNode.prefix_priority("3b2-net1vm1vm2vm3", "3b1-net1vm1vm2vm3"), 1
        )
        self.assertEqual(
            TestNode.prefix_priority("3b2a1-net1vm1vm2vm3", "3b2-net1vm1vm2vm3"), 1
        )
        self.assertEqual(
            TestNode.prefix_priority("3b2c1-net1vm1vm2vm3", "3b2a1-net1vm1vm2vm3"), 1
        )
    ...
    def test_pick_priority_bridged(self):
        ...
        node2 = TestGraph.parse_node_from_object(
            full_net, "normal..tutorial1", prefix="2"
        )
        node3 = TestGraph.parse_node_from_object(
            full_net, "normal..tutorial2", prefix="3"
        )
        node.descend_from_node(node1, flat_net)
        node.descend_from_node(node2, flat_net)
        node.descend_from_node(node3, flat_net)
    ...
    def test_pick_and_drop_node(self):
        ...
        node = TestGraph.parse_node_from_object(full_net, "normal..tutorial1")
        child_node1 = TestGraph.parse_node_from_object(
            full_net, "normal..tutorial1", prefix="1"
        )
        child_node2 = TestGraph.parse_node_from_object(
            full_net, "normal..tutorial2", prefix="2"
        )
        ...
        self.assertIn(
            worker.id, picked_parent._picked_by_cleanup_nodes.get_workers(picked_child)
        )
        picked_child.drop_parent(picked_parent, worker)
        self.assertIn(
            worker.id, picked_parent._dropped_cleanup_nodes.get_workers(picked_child)
        )
    ...
    def test_pick_and_drop_bridged(self):
        ...
        node13 = graph.parse_composite_nodes(
            "normal..tutorial2", flat_net1, unique=True
        )
        node23 = graph.parse_composite_nodes(
            "normal..tutorial2", flat_net2, unique=True
        )
        ...
        self.assertEqual(
            node23._picked_by_cleanup_nodes, node13._picked_by_cleanup_nodes
        )
        ...
        self.assertEqual(
            node13._picked_by_cleanup_nodes.get_workers(node1), {worker1.id}
        )
        self.assertEqual(
            node23._picked_by_cleanup_nodes.get_workers(node2), {worker1.id}
        )
    ...
    def test_pull_locations(self):
        ...
        node = TestGraph.parse_node_from_object(
            full_net, "normal..tutorial1", params=self.config["param_dict"]
        )
        parent_node = TestGraph.parse_node_from_object(
            full_net, "normal..tutorial1", params=self.config["param_dict"]
        )
        worker = TestWorker(flat_net)
        ...
        parent_node.results = [
            {"name": "tutorial1.net1", "status": "PASS", "time_elapsed": 3}
        ]
        ...
        self.assertEqual(
            node.params[f"nets_host_{worker.id}"], worker.params[f"nets_host"]
        )

        # an impossible situation with different worker ids must be validated against
        with mock.patch(
            "virttest.cartgraph.TestNode.shared_result_worker_ids",
    ...
    def test_pull_locations_bridged(self):
        ...
        full_components = TestGraph.parse_components_for_object(
            flat_net1, "nets", "", unflatten=True
        )
        full_net1 = full_components[-1]
        flat_net2.update_restrs(self.config["vm_strs"])
        full_components = TestGraph.parse_components_for_object(
            flat_net2, "nets", "", unflatten=True
        )
        full_net2 = full_components[-1]

        node1 = TestGraph.parse_node_from_object(
            full_net1, "normal..tutorial1", params=self.config["param_dict"]
        )
        node2 = TestGraph.parse_node_from_object(
            full_net2, "normal..tutorial1", params=self.config["param_dict"]
        )
        parent_node1 = TestGraph.parse_node_from_object(
            full_net1, "normal..tutorial1", params=self.config["param_dict"]
        )
        parent_node2 = TestGraph.parse_node_from_object(
            full_net2, "normal..tutorial1", params=self.config["param_dict"]
        )
        worker1 = TestWorker(flat_net1)
        worker2 = TestWorker(flat_net2)
        ...
        self.assertEqual(
            get_locations[1], worker1.id + ":" + node1.params["swarm_pool"]
        )
        # parameters to access the worker location should also be provided for the test
        self.assertEqual(
            node1.params[f"nets_host_{worker1.id}"], worker1.params[f"nets_host"]
        )
        ...
        # parameters to access the worker location should also be provided for the test
        self.assertEqual(
            node2.params[f"nets_host_{worker1.id}"], worker1.params[f"nets_host"]
        )
    ...
    def test_validation(self):
        ...
        with self.assertRaisesRegex(
            ValueError, r"^Additional parametric objects .+ not in .+$"
        ):
            test_node.validate()
        ...
        with self.assertRaisesRegex(
            ValueError, r"^Missing parametric objects .+ from .+$"
        ):
            test_node.validate()
...
class CartesianGraphTest(Test):
    ...
    def setUp(self):
        ...
        self.shared_pool = ":" + self.config["param_dict"]["shared_pool"]
        DummyStateControl.asserted_states = {
            "check": {},
            "get": {},
            "set": {},
            "unset": {},
        ...
        DummyStateControl.asserted_states["check"] = {
            "install": {self.shared_pool: False},
            "customize": {self.shared_pool: False},
            "on_customize": {self.shared_pool: False},
            "connect": {self.shared_pool: False},
            "linux_virtuser": {self.shared_pool: False},
            "windows_virtuser": {self.shared_pool: False},
        }
        DummyStateControl.asserted_states["get"] = {
            "install": {self.shared_pool: 0},
            "customize": {self.shared_pool: 0},
            "on_customize": {self.shared_pool: 0},
            "connect": {self.shared_pool: 0},
            "linux_virtuser": {self.shared_pool: 0},
            "windows_virtuser": {self.shared_pool: 0},
        }
    ...
    def _run_traversal(self, graph, params=None):
        ...
        slot_workers = sorted(
            list(graph.workers.values()), key=lambda x: x.params["name"]
        )
        ...
        self.assertEqual(
            len(DummyTestRun.asserted_tests),
            0,
            "Some tests weren't run: %s" % DummyTestRun.asserted_tests,
        )
    ...
    def test_flag_children_all(self):
        ...
        graph.flag_children(
            object_name="image1_vm1", flag_type="run", flag=lambda self, slot: False
        )
        DummyTestRun.asserted_tests = []
        self._run_traversal(graph, self.config["param_dict"])
    ...
    def test_flag_intersection(self):
        ...
        graph.flag_intersection(
            reuse_graph, flag_type="run", flag=lambda self, slot: False
        )
        DummyTestRun.asserted_tests = [
            {"shortname": "^nonleaves.internal.automated.connect.vm1", "vms": "^vm1$"},
        ]
    ...
    def test_get_objects(self):
        ...
        get_objects = graph.get_objects(
            "os_type", param_val="linux", subset=get_objects
        )
        self.assertEqual(len(get_objects), 1)
        get_objects = graph.get_objects(
            param_key="os_variant", param_val="centos", subset=get_objects
        )
        self.assertEqual(len(get_objects), 1)
        get_node = graph.get_nodes(
            param_key="os_type", param_val="linux", subset=get_objects, unique=True
        )
    ...
    def test_get_nodes(self):
        ...
        get_nodes = graph.get_nodes(
            "remote_control_check", param_val="yes", subset=get_nodes
        )
        self.assertEqual(len(get_nodes), 4)
        get_nodes = graph.get_nodes(
            param_key="remote_decorator_check", param_val="yes", subset=get_nodes
        )
        self.assertEqual(len(get_nodes), 2)
        get_node = graph.get_nodes(
            param_key="remote_util_check",
            param_val="yes",
            subset=get_nodes,
            unique=True,
        )
    ...
    def test_object_node_intersection(self):
        ...
        for n in nodes:
            ...
            raise AssertionError(
                "The tutorial1 variant must be present in the object-node intersection"
            )
        for n in nodes:
            if "tutorial_get" in n.params["name"]:
                raise AssertionError(
                    "The tutorial_get variant must be skipped since vm3 is not available"
                )
    ...
    def test_parse_cloned_branches_for_node_and_object(self):
        ...
        graph = TestGraph()
        flat_object = TestGraph.parse_flat_objects(
            "net1",
            "nets",
            params={"only_vm1": "CentOS", "only_vm2": "Win10", "only_vm3": "Ubuntu"},
            unique=True,
        ...
        full_node = graph.parse_composite_nodes(
            "leaves..tutorial_get..implicit_both", flat_object, unique=True
        )
        ...
        clones = graph.parse_cloned_branches_for_node_and_object(
            full_node, flat_object, full_parents
        )
        ...
        grandchildren = graph.parse_composite_nodes(
            "leaves..tutorial_finale", flat_object
        )
        ...
        for child, parent in zip(grandchild.cloned_nodes, clones):
            ...
            self.assertEqual(
                len(set(parent.cleanup_nodes) - set(grandchild.cloned_nodes)), 0
            )
    ...
    def test_parse_branches_for_node_and_object(self):
        ...
        parents, children = graph.parse_branches_for_node_and_object(
            flat_node, flat_object
        )
        ...
        self.assertEqual(len(graph.objects), 6)
        full_nets = sorted(
            [o for o in graph.objects if o.key == "nets"],
            key=lambda x: x.params["name"],
        )
        self.assertEqual(len(full_nets), 2)
        full_vms = sorted(
            [o for o in graph.objects if o.key == "vms"], key=lambda x: x.params["name"]
        )
    ...
    def test_parse_branches_for_node_and_object_with_bridging(self):
        ...
        flat_object1 = TestGraph.parse_flat_objects(
            "net1",
            "nets",
            params={"only_vm1": "CentOS", "only_vm2": "Win10", "only_vm3": "Ubuntu"},
            unique=True,
        )
        flat_object2 = TestGraph.parse_flat_objects(
            "net2",
            "nets",
            params={"only_vm1": "CentOS", "only_vm2": "Win10", "only_vm3": "Ubuntu"},
            unique=True,
    ...
    def test_parse_branches_for_node_and_object_with_cloning(self):
        ...
        flat_node = TestGraph.parse_flat_nodes(
            "leaves..tutorial_get..implicit_both", unique=True
        )
    ...
    def test_parse_branches_for_node_and_object_with_cloning_multivariant(self):
        ...
        self.assertEqual(
            [n.prefix for n in parents], ["1a1", "1a1", "1a2", "1b2a1", "1b2a2"]
        )
        # validate cache is exactly as expected, namely just the newly parsed nodes
        self.assertEqual(len(graph.nodes), len(parents + children))
        self.assertEqual(set(graph.nodes), set(parents + children))
        ...
        self.assertEqual(
            len([c for c in children if "implicit_both" in c.id]), 3 * 2 ** (3 - 1)
        )
        self.assertEqual(
            len(
                [
                    c
                    for c in children
                    if "implicit_both" in c.id and "guisetup.noop" in c.id
                ]
            ),
            2 ** (3 - 1),
        )
        self.assertEqual(
            len(
                [
                    c
                    for c in children
                    if "implicit_both" in c.id and "guisetup.clicked" in c.id
                ]
            ),
            2 ** (3 - 1),
        )
    ...
    def test_parse_branches_for_node_and_object_with_cloning_bridging(self):
        ...
        parents1, children1 = graph.parse_branches_for_node_and_object(
            flat_node, flat_object1
        )
        self.assertEqual(len([p for p in parents1 if "tutorial_gui" in p.id]), 2)
        self.assertEqual(len(children1), 3)
        parents2, children2 = graph.parse_branches_for_node_and_object(
            flat_node, flat_object2
        )
        ...
        # deeper cloning must also restore bridging among grandchildren
        composite_nodes = graph.parse_composite_nodes(
            "leaves..tutorial_get..implicit_both", flat_object1
        )
        composite_nodes += graph.parse_composite_nodes(
            "leaves..tutorial_get..implicit_both", flat_object2
        )
        self.assertEqual(len(composite_nodes), 2)
        graph.new_nodes(composite_nodes)
        grandchildren = graph.parse_composite_nodes(
            "leaves..tutorial_finale", flat_object1
        )
        grandchildren += graph.parse_composite_nodes(
            "leaves..tutorial_finale", flat_object2
        )
        graph.new_nodes(grandchildren)
        self.assertEqual(len(grandchildren), 2)
        for i in (0, 1):
            grandchildren[i].descend_from_node(
                composite_nodes[i], flat_object1 if i == 0 else flat_object2
            )
        ...
        self.assertEqual(
            grandchildren[0].cloned_nodes,
            (cloned_grandchildren[0], cloned_grandchildren[1]),
        )
        self.assertEqual(
            grandchildren[1].cloned_nodes,
            (cloned_grandchildren[2], cloned_grandchildren[3]),
        )
        self.assertEqual(
            cloned_grandchildren[0].bridged_nodes, (cloned_grandchildren[2],)
        )
        self.assertEqual(
            cloned_grandchildren[1].bridged_nodes, (cloned_grandchildren[3],)
        )
        ...
        for grandchild in cloned_grandchildren:
            ...
            dependency_object = (
                flat_object1
                if cloned_grandchildren.index(grandchild) in (0, 1)
                else flat_object2
            )
    ...
    def test_parse_paths_to_object_roots_with_cloning_deep(self):
        ...
        self.assertEqual(
            len([p for p in flat_parents if "tutorial_get.implicit_both" in p.id]), 1
        )
        leaf_parents1, leaf_children1, leaf_node1 = next(generator)
        self.assertIn(leaf_node1, flat_children)
        self.assertEqual(len(leaf_children1), 0)
        ...
        for node in graph.nodes:
            if node.is_shared_root():
                if shared_root_node is not None:
                    raise AssertionError(
                        "More than one shared root nodes found in graph"
                    )
    ...
    def test_graph_sanity(self):
        ...
        comparable_objects = [
            o for o in graph.objects if o.suffix not in ["net2", "net3", "net4", "net5"]
        ]
        comparable_nodes = [n for n in graph.nodes if "a" not in n.prefix]

        # check prefixes and suffixes match
        self.assertEqual(
            {o.suffix for o in comparable_objects}, {o.suffix for o in objects}
        )
        self.assertEqual(
            {n.prefix for n in comparable_nodes}, {n.prefix for n in nodes}
        )

        # check all objects and nodes within the graph are unique
        for test_object in graph.objects:
            compare = lambda x, y: (
                x.long_suffix == y.long_suffix
                if x.key == "images"
                else x.params["name"] == y.params["name"]
            )
            self.assertEqual(
                [test_object], [o for o in graph.objects if compare(o, test_object)]
            )
            if test_object in comparable_objects:
                self.assertEqual(
                    [test_object.id],
                    [o.id for o in comparable_objects if compare(o, test_object)],
                )
        for test_node in graph.nodes:
            self.assertEqual(
                ...
                [
                    ...
                    if n.params["name"] == test_node.params["name"]
                ],
            )
    ...
    def test_traverse_one_leaf_parallel(self):
        ...
        graph = self._load_for_parsing(
            "normal..tutorial1", {"nets": " ".join([f"net{i+1}" for i in range(4)])}
        )

        DummyTestRun.asserted_tests = [
            {
                "shortname": "^internal.stateless.noop.vm1",
                "vms": "^vm1$",
            ...
            {
                "shortname": "^internal.automated.customize.vm1",
                "vms": "^vm1$",
                "get_state_images": "^install$",
                "set_state_images": "^customize$",
            },
            {
                "shortname": "^internal.automated.on_customize.vm1",
                "vms": "^vm1$",
                "get_state_images": "^customize$",
                "set_state_vms": "^on_customize$",
            },
            {
                "shortname": "^normal.nongui.quicktest.tutorial1.vm1",
                "vms": "^vm1$",
    ...
    def test_traverse_one_leaf_with_setup(self):
        ...
        graph = self._load_for_parsing(
            "normal..tutorial1", {"nets": " ".join([f"net{i+1}" for i in range(3)])}
        )
    ...
    def test_traverse_one_leaf_with_failed_setup(self):
        ...
        graph = self._load_for_parsing(
            "normal..tutorial1", {"nets": " ".join([f"net{i+1}" for i in range(2)])}
        )
    ...
    def test_traverse_one_leaf_with_retried_setup(self):
        ...
        self._run_traversal(
            graph,
            {"max_concurrent_tries": "1", "max_tries": "3", "stop_status": "pass"},
        )
    ...
    def test_traverse_one_leaf_with_occupation_timeout(self):
        ...
        graph = TestGraph()
        graph.new_nodes(
            TestGraph.parse_flat_nodes("normal..tutorial1,normal..tutorial2")
        )
        # different order and restrictions so that we can emulate stuck net2
        graph.new_workers(
            TestGraph.parse_workers({"nets": "net1 net2 net4", "only_vm1": "CentOS"})
        )
        graph.new_nodes(
            graph.parse_composite_nodes("normal..tutorial1", graph.workers["net2"].net)
        )
        graph.new_nodes(
            graph.parse_composite_nodes("normal..tutorial2", graph.workers["net4"].net)
        )
        graph.parse_shared_root_from_object_roots()
        ...
        DummyTestRun.asserted_tests = [
            ...
            {
                "shortname": "^normal.nongui.quicktest.tutorial2.files.vm1",
                "vms": "^vm1$",
                "nets": "^net1$",
            },
            {
    ...
    def test_traverse_two_objects_without_setup(self):
        ...
        graph = self._load_for_parsing(
            "normal..tutorial3", {"nets": " ".join([f"net{i+1}" for i in range(4)])}
        )

        DummyStateControl.asserted_states["check"]["install"][self.shared_pool] = False
        DummyStateControl.asserted_states["check"]["customize"][
            self.shared_pool
        ] = False
        DummyTestRun.asserted_tests = [
            {
            ...
            {
                "shortname": "^internal.stateless.noop.vm2",
                "vms": "^vm2$",
                "nets": "^net2$",
            },
            {
            ...
            {
                "shortname": "^original.unattended_install.cdrom.extra_cdrom_ks.default_install.aio_threads.vm2",
                "vms": "^vm2$",
                "nets": "^net2$",
            },
            {
            ...
            {
                "shortname": "^internal.automated.customize.vm2",
                "vms": "^vm2$",
                "nets": "^net2$",
            },
            {
                "shortname": "^internal.automated.connect.vm1",
                "vms": "^vm1$",
                "nets": "^net1$",
            },
            {
                "shortname": "^normal.nongui.tutorial3",
                "vms": "^vm1 vm2$",
                "nets": "^net1$",
            },
        ]
        ...
        self.assertLessEqual(
            DummyStateControl.asserted_states["get"]["install"][self.shared_pool], 2 * 4
        )
        self.assertLessEqual(
            DummyStateControl.asserted_states["get"]["customize"][self.shared_pool],
            2 * 4,
        )
        # recreated setup is taken from the worker that created it (node reversal)
        self.assertLessEqual(
            DummyStateControl.asserted_states["get"]["connect"][self.shared_pool], 1 * 4
        )
    ...
    def test_traverse_two_objects_with_setup(self):
        ...
        # recreated setup is taken from the worker that created it (node reversal)
        self.assertLessEqual(
            DummyStateControl.asserted_states["get"]["connect"][self.shared_pool], 2 * 4
        )

    def test_traverse_two_objects_with_shared_setup(self):
        """Test a two-object test traversal with shared setup among two nodes."""
        graph = self._load_for_parsing(
            "leaves..tutorial_gui", {"nets": " ".join([f"net{i+1}" for i in range(4)])}
        )

        DummyStateControl.asserted_states["check"]["install"][self.shared_pool] = True
        DummyStateControl.asserted_states["check"]["customize"][self.shared_pool] = True
        DummyStateControl.asserted_states["check"]["guisetup.noop"] = {
            self.shared_pool: False
        }
        DummyStateControl.asserted_states["check"]["guisetup.clicked"] = {
            self.shared_pool: False
        }
        DummyStateControl.asserted_states["get"]["guisetup.noop"] = {
            self.shared_pool: 0
        }
        DummyStateControl.asserted_states["get"]["guisetup.clicked"] = {
            self.shared_pool: 0
        }
        DummyStateControl.asserted_states["unset"] = {
            "guisetup.noop": {self.shared_pool: 0}
        }
        DummyTestRun.asserted_tests = [
            {
                "shortname": "^internal.automated.connect.vm1",
        ...
        DummyTestRun.asserted_tests = [
            {
                "shortname": "^internal.automated.linux_virtuser.vm1",
                "vms": "^vm1$",
                "nets": "^net1$",
            },
            {
                "shortname": "^internal.automated.windows_virtuser.vm2",
                "vms": "^vm2$",
                "nets": "^net2$",
            },
            # net3 waits for net1 and net4 waits for net2, net1 then waits for net2 and net2 moves on
            {
                "shortname": "^leaves.tutorial_gui.client_clicked",
                "vms": "^vm1 vm2$",
                "nets": "^net2$",
            },
            {
                "shortname": "^leaves.tutorial_gui.client_noop",
                "vms": "^vm1 vm2$",
                "nets": "^net3$",
            },
        ]
        ...
        for action in ["get"]:
            for state in ["install", "customize"]:
                ...
                self.assertLessEqual(
                    DummyStateControl.asserted_states[action][state][self.shared_pool],
                    2 * 4,
                )
        for action in ["set"]:
        ...
        self.assertEqual(
            DummyStateControl.asserted_states["unset"]["guisetup.noop"][
                self.shared_pool
            ],
            1,
        )
    ...
    def test_traverse_motif_with_flat_reuse(self):
        ...
        graph = self._load_for_parsing(
            "leaves..client_clicked,leaves..explicit_clicked",
        ...
        DummyStateControl.asserted_states["check"]["getsetup.clicked"] = {
            self.shared_pool: False
        }
        DummyStateControl.asserted_states["get"]["guisetup.clicked"] = {
            self.shared_pool: 0
        }
        DummyStateControl.asserted_states["get"]["getsetup.clicked"] = {
            self.shared_pool: 0
        }
        DummyStateControl.asserted_states["unset"] = {
            "guisetup.clicked": {self.shared_pool: 0}
        }
        DummyStateControl.asserted_states["unset"] = {
            "getsetup.clicked": {self.shared_pool: 0}
        }
        DummyTestRun.asserted_tests = [
            {
                "shortname": "^internal.automated.linux_virtuser.vm1",
            ...
            {
                "shortname": "^leaves.tutorial_get.explicit_clicked.vm1",
                "vms": "^vm1 vm2 vm3$",
                "nets": "^net1$",
            },
        ]
    ...
    def test_traverse_motif_all(self):
        ...
        graph = self._load_for_parsing(
            "leaves", {"nets": " ".join([f"net{i+1}" for i in range(2)])}
        )
        ...
        DummyStateControl.asserted_states["check"].update(
            {
                "getsetup": {self.shared_pool: False},
                "guisetup.noop": {self.shared_pool: False},
                "guisetup.clicked": {self.shared_pool: False},
                "getsetup.noop": {self.shared_pool: False},
                "getsetup.clicked": {self.shared_pool: False},
                "getsetup.guisetup.noop": {self.shared_pool: False},
                "getsetup.guisetup.clicked": {self.shared_pool: False},
            }
        )
        DummyStateControl.asserted_states["get"].update(
            {
                "getsetup": {self.shared_pool: 0},
                "guisetup.noop": {self.shared_pool: 0},
                "guisetup.clicked": {self.shared_pool: 0},
    ...
    def test_overwrite_params_and_restrs(self):
        ...
        default_object_param = graph.get_nodes(
            param_val="explicit_clicked", unique=True
        ).params["images"]
        default_node_param = graph.get_nodes(
            param_val="explicit_clicked", unique=True
        ).params["shared_pool"]
        ...
        self.assertNotEqual(
            test_node.params["shared_pool"],
            default_node_param,
            "The default %s of %s wasn't overwritten"
            % (default_node_param, test_node.prefix),
        )
        self.assertEqual(
            test_node.params["shared_pool"],
            custom_node_param,
            "The new %s of %s must be %s"
            % (default_node_param, test_node.prefix, custom_node_param),
        )
        self.assertEqual(
            test_node.params["new_key"],
            "123",
            "A new parameter=%s of %s must be 123"
            % (test_node.params["new_key"], test_node.prefix),
        )
        # TODO: the current suffix operators don't allow overwriting the suffix-free parameter default
        self.assertEqual(
            test_node.params["images"],
            default_object_param,
            "The object-general default %s of %s must be overwritten"
            % (default_object_param, test_node.prefix),
        )
        self.assertNotEqual(
            test_node.params["images_vm1"],
            default_object_param,
            "The default %s of %s wasn't overwritten"
            % (default_object_param, test_node.prefix),
        )
        self.assertEqual(
            test_node.params["images_vm1"],
            custom_object_param1,
            "The new %s of %s must be %s"
            % (default_object_param, test_node.prefix, custom_object_param2),
        )
        self.assertNotEqual(
            test_node.params["images_vm2"],
            default_object_param,
            "The default %s of %s wasn't overwritten"
            % (default_object_param, test_node.prefix),
        )
        ...
        self.assertEqual(
            test_object_params1["new_key"],
            "123",
            "A new parameter=%s of %s must be 123"
            % (test_object_params1["new_key"], test_object1.suffix),
        )
        self.assertNotEqual(
            test_object_params1["images"],
            default_object_param,
            "The default %s of %s wasn't overwritten"
            % (default_object_param, test_object1.suffix),
        )
        self.assertEqual(
            test_object_params1["images"],
            custom_object_param1,
            "The new %s of %s must be %s"
            % (default_object_param, test_object1.suffix, custom_object_param1),
        )

        test_objects = graph.get_objects(param_val="vm2.*Win10")
        vm_objects = [o for o in test_objects if o.key == "vms"]
        self.assertEqual(
            len(vm_objects), 1, "There must be exactly one vm object for tutorial1"
        )
        ...
        self.assertNotEqual(
            test_object_params2["images"],
            default_object_param,
            "The default %s of %s wasn't overwritten"
            % (default_object_param, test_object2.suffix),
        )
        self.assertEqual(
            test_object_params2["images"],
            custom_object_param2,
            "The new %s of %s must be %s"
            % (default_object_param, test_object2.suffix, custom_object_param2),
        )
    ...
    def test_run_warn_duration(self):
        ...
        net = test_objects[-1]
        test_node = TestGraph.parse_node_from_object(
            net, "normal..tutorial1", params=self.config["param_dict"].copy()
        )
    ...
    def test_run_exit_code(self):
        ...
        test_node = TestGraph.parse_node_from_object(
            net, "normal..tutorial2.files", params=self.config["param_dict"].copy()
        )
        test_node.started_worker = "some-worker-since-only-traversal-allowed"
    ...
    def test_rerun_max_times_serial(self):
        ...
        DummyTestRun.asserted_tests = [
            ...
            {
                "shortname": r"^original.unattended_install.cdrom.extra_cdrom_ks.default_install.aio_threads.vm1",
                "vms": r"^vm1$",
            },
            {"shortname": r"^internal.stateless.noop.vm1", "vms": r"^vm1$"},
            {
            ...
            {
                "shortname": r"^internal.automated.customize.vm1",
                "vms": r"^vm1$",
                "_long_prefix": r"^[a\d]+r1-vm1$",
            },
            {
                "shortname": r"^internal.automated.customize.vm1",
                "vms": r"^vm1$",
                "_long_prefix": r"^[a\d]+r2-vm1$",
            },
            {"shortname": r"^internal.automated.on_customize.vm1", "vms": r"^vm1$"},
            {
                "shortname": r"^internal.automated.on_customize.vm1",
                "vms": r"^vm1$",
                "_long_prefix": r"^[a\d]+r1-vm1$",
            },
            {
            ...
            {
                "shortname": r"^normal.nongui.quicktest.tutorial1.vm1",
                "vms": r"^vm1$",
                "_long_prefix": r"^[a\d]+r1-vm1$",
            },
            {
    ...
    def test_rerun_max_times_parallel(self):
        ...
        DummyStateControl.asserted_states["check"] = {
            "root": {self.shared_pool: True},
            "install": {self.shared_pool: True},
            "customize": {self.shared_pool: True},
            "on_customize": {self.shared_pool: True},
        }
        DummyTestRun.asserted_tests = [
            {
                "shortname": r"^normal.nongui.quicktest.tutorial1.vm1",
    ...
    def test_rerun_status_times(self):
        ...
        for status in [
            ...
            with self.subTest(f"Test rerun on status {status}"):
                ...
                self.assertEqual(
                    len(graph.get_nodes(param_val="tutorial1", unique=True).results), 4
                )
                # also assert the correct results were registered
                self.assertEqual(
                    [x["status"] for x in self.runner.job.result.tests], [status] * 4
                )
                registered_results = []
                ...
                self.assertEqual(
                    registered_results,
                    graph.get_nodes(param_val="tutorial1", unique=True).results,
                )
    ...
    def test_rerun_status_stop(self):
        ...
        for stop_status in ["pass", "warn", "fail", "error"]:
            with self.subTest(f"Test rerun stop on status {stop_status}"):
                ...
                self.assertEqual(
                    len(graph.get_nodes(param_val="tutorial1", unique=True).results), 1
                )
                # also assert the correct results were registered
                self.assertEqual(
                    [x["status"] for x in self.runner.job.result.tests], [status]
                )
                registered_results = []
                for result in self.runner.job.result.tests:
                    result["name"] = result["name"].name
    ...
    def test_rerun_status_rerun(self):
        ...
        for stop_status in ["pass", "warn", "fail", "error"]:
            with self.subTest(f"Test rerun stop on status {stop_status}"):
                ...
                self.assertEqual(
                    len(graph.get_nodes(param_val="tutorial1", unique=True).results), 2
                )
                # also assert the correct results were registered
                self.assertEqual(
                    [x["status"] for x in self.runner.job.result.tests],
                    [status, "INTERRUPTED"],
                )
                registered_results = []
                for result in self.runner.job.result.tests:
    ...
    def test_rerun_previous_job_default(self):
        ...
        on_customize_tests = [
            n.params["name"] for n in graph.get_nodes(param_val="on_customize")
        ]
        self.assertEqual(len(on_customize_tests), 2)
        self.runner.previous_results += [
            {"name": on_customize_tests[0], "status": "PASS", "time_elapsed": 1}
        ]
        self.runner.previous_results += [
            {"name": on_customize_tests[1], "status": "PASS", "time_elapsed": 1}
        ]
        self.runner.previous_results += [
            {"name": node11.params["name"], "status": "PASS", "time_elapsed": 1}
        ]
        self.runner.previous_results += [
            {"name": node12.params["name"], "status": "FAIL", "time_elapsed": 0.2}
        ]
        self.runner.previous_results += [
            {
                "name": node21.params["name"].replace("leaves", "all"),
                "status": "FAIL",
                "time_elapsed": 0.2,
            }
        ]
        self.runner.previous_results += [
            {
                "name": node22.params["name"].replace("leaves", "all"),
                "status": "PASS",
                "time_elapsed": 1,
            }
        ]
        ...
        DummyTestRun.asserted_tests = [
            ...
            {
                "shortname": "^leaves.quicktest.tutorial2.names.vm1.+CentOS",
                "vms": "^vm1$",
            },
            # skip the previously passed test since it doesn't have a rerun status (here fail by default) and rerun second test
            {
                "shortname": "^leaves.quicktest.tutorial2.files.vm1.+Fedora",
                "vms": "^vm1$",
            },
        ]
        ...
        # also assert the correct results were registered
        self.assertEqual(
            [x["status"] for x in self.runner.job.result.tests], ["PASS", "PASS"]
        )
    ...
    def test_rerun_previous_job_status(self):
        ...
        self.runner.previous_results += [
            {
                "name": graph.get_nodes(param_val="on_customize", unique=True).params[
                    "name"
                ],
                "status": "PASS",
                "time_elapsed": 1,
        ...
        self.runner.previous_results += [
            {"name": node1.params["name"], "status": "PASS", "time_elapsed": 1}
        ]
        self.runner.previous_results += [
            {"name": node2.params["name"], "status": "FAIL", "time_elapsed": 0.2}
        ]
        ...
        graph.parse_shared_root_from_object_roots()
        graph.new_workers(
            TestGraph.parse_workers({"nets": "net1", "only_vm1": "CentOS"})
        )
        DummyStateControl.asserted_states["check"] = {
            "root": {self.shared_pool: True},
            "install": {self.shared_pool: True},
        ...
        DummyTestRun.asserted_tests = [
            # previously run setup tests with rerun status are still rerun (scan is overridden)
            {
                "shortname": "^internal.automated.on_customize.vm1",
                "vms": "^vm1$",
                "get_state_images": "^customize$",
    ...
    def test_rerun_invalid(self):
        ...
        with mock.patch.dict(
            self.config["param_dict"], {"max_tries": "3", "stop_status": "invalid"}
        ):
            graph = TestGraph.parse_object_trees(
                None,
                self.config["tests_str"],
            ...
            with self.assertRaisesRegex(
                ValueError, r"^Value of stop status must be a valid test status"
            ):
                self._run_traversal(graph)
        ...
        with mock.patch.dict(
            self.config["param_dict"], {"max_tries": "-32", "stop_status": ""}
        ):
            graph = TestGraph.parse_object_trees(
                None,
                self.config["tests_str"],
            ...
            with self.assertRaisesRegex(
                ValueError, r"^Number of max_tries cannot be less than zero$"
            ):
                self._run_traversal(graph)
        ...
        with mock.patch.dict(
            self.config["param_dict"], {"max_tries": "3.5", "stop_status": ""}
        ):
            graph = TestGraph.parse_object_trees(
                None,
                self.config["tests_str"],
        ...
        with mock.patch.dict(
            self.config["param_dict"], {"max_tries": "hey", "stop_status": ""}
        ):
            graph = TestGraph.parse_object_trees(
                None,
                self.config["tests_str"],
    ...
    def test_dry_run(self):
        ...
        graph = self._load_for_parsing(
            "leaves", {"nets": " ".join([f"net{i+1}" for i in range(3)])}
        )
    ...
    def test_aborted_run(self):
        ...
        graph = self._load_for_parsing(
            "tutorial1", {"nets": " ".join([f"net{i+1}" for i in range(3)])}
        )
        ...
        with self.assertRaisesRegex(
            exceptions.TestSkipError, r"^God wanted this test to abort$"
        ):
    ...
    def test_parsing_on_demand(self):
        ...
        self.assertEqual(
            graph.parse_paths_to_object_roots.call_count, (len(flat_nodes) - 1) * 2
        )
        for node in flat_nodes:
            parse_calls = [
                mock.call(node, w.net, mock.ANY) for w in graph.workers.values()
            ]
            for i in range(2):
                ...
                if node == root:
                    self.assertNotIn(
                        mock.call(node, worker.net, mock.ANY),
                    ...
                    self.assertTrue(
                        parse_calls[0]
                        in graph.parse_paths_to_object_roots.call_args_list
                        or parse_calls[1]
                        in graph.parse_paths_to_object_roots.call_args_list
                    )
    ...
    def test_traversing_in_isolation(self):
        ...
        for node in graph.nodes:
            ...
            self.assertEqual(
                picked_by_setup.get_counters(),
                sum(
                    picked_by_setup.get_counters(worker=w)
                    for w in graph.workers.values()
                ),
            )
            self.assertEqual(
                picked_by_cleanup.get_counters(),
                sum(
                    picked_by_cleanup.get_counters(worker=w)
                    for w in graph.workers.values()
                ),
            )
            self.assertEqual(
                dropped_setup.get_counters(),
                sum(
                    dropped_setup.get_counters(worker=w) for w in graph.workers.values()
                ),
            )
            self.assertEqual(
                dropped_cleanup.get_counters(),
                sum(
                    dropped_cleanup.get_counters(worker=w)
                    for w in graph.workers.values()
                ),
            )
            for worker in graph.workers.values():
                ...
                worker_setup = [
                    n
                    for n in node.setup_nodes
                    if n.is_flat() or worker.id in n.params["name"]
                ]
                worker_cleanup = [
                    n
                    for n in node.cleanup_nodes
                    if n.is_flat() or worker.id in n.params["name"]
                ]
                self.assertEqual(
                    picked_by_setup.get_counters(worker=worker),
                    sum(
                        picked_by_setup.get_counters(n, worker=worker)
                        for n in worker_setup
                    ),
                )
                self.assertEqual(
                    picked_by_cleanup.get_counters(worker=worker),
                    sum(
                        picked_by_cleanup.get_counters(n, worker=worker)
                        for n in worker_cleanup
                    ),
                )
                self.assertEqual(
                    dropped_setup.get_counters(worker=worker),
                    sum(
                        dropped_setup.get_counters(n, worker=worker)
                        for n in worker_setup
                    ),
                )
                self.assertEqual(
                    dropped_cleanup.get_counters(worker=worker),
                    sum(
                        dropped_cleanup.get_counters(n, worker=worker)
                        for n in worker_cleanup
                    ),
                )

                picked_counter = {
                    s: s._picked_by_cleanup_nodes.get_counters(node=node, worker=worker)
                    for s in worker_setup
                }
                average_picked_counter = (
                    sum(picked_counter.values()) / len(picked_counter)
                    if picked_counter
                    else 0.0
                )
                for setup_node in worker_setup:
                    ...
                    self.assertLessEqual(
                        int(abs(picked_counter[setup_node] - average_picked_counter)), 1
                    )
                    # may not be picked by some setup that was needed for it to be setup ready
                    self.assertGreaterEqual(
                        max(
                            picked_counter[setup_node],
                            picked_by_setup.get_counters(
                                node=setup_node, worker=worker
                            ),
                        ),
                        1,
                    )
                    ...
                    self.assertEqual(
                        dropped_setup.get_counters(node=setup_node, worker=worker), 1
                    )
                picked_counter = {
                    c: c._picked_by_setup_nodes.get_counters(node=node, worker=worker)
                    for c in worker_cleanup
                }
                average_picked_counter = (
                    sum(picked_counter.values()) / len(picked_counter)
                    if picked_counter
                ...
                for cleanup_node in worker_cleanup:
                    ...
                    self.assertLessEqual(
                        int(abs(picked_counter[cleanup_node] - average_picked_counter)),
                        1,
                    )
                    # picked at least once from each cleanup node (for one or all workers if flat)
                    self.assertGreaterEqual(
                        picked_by_cleanup.get_counters(
                            node=cleanup_node, worker=worker
                        ),
                        1,
                    )
                    # each cleanup node dropped exactly once per worker
                    self.assertEqual(
                        dropped_cleanup.get_counters(node=cleanup_node, worker=worker),
                        1,
                    )
                ...
                self.assertGreaterEqual(
                    len(
                        [
                            c
                            for c in traverse_calls
                            if c.args[0] == node and c.args[1] == worker
                        ]
                    ),
                    1,
                )
                ...
                self.assertEqual(
                    len(
                        [
                            r
                            for r in reverse_calls
                            if r == mock.call(node, worker, mock.ANY)
                        ]
                    ),
                    1,
                )
                ...
                for setup_node in worker_setup:
                    if setup_node.is_shared_root():
                        continue
                    self.assertGreater(
                        reverse_calls.index(mock.call(setup_node, worker, mock.ANY)),
                        reverse_calls.index(mock.call(node, worker, mock.ANY)),
                    )
                for cleanup_node in worker_cleanup:
                    self.assertLess(
                        reverse_calls.index(mock.call(cleanup_node, worker, mock.ANY)),
                        reverse_calls.index(mock.call(node, worker, mock.ANY)),
                    )
    ...
    def test_trace_work_external(self):
        ...
        graph = self._load_for_parsing(
            "normal..tutorial1,normal..tutorial3",
            {
    ...
    def test_trace_work_swarm(self):
        ...
        graph = self._load_for_parsing(
            "leaves..tutorial2,leaves..tutorial_gui",
            {
                "nets": " ".join([f"net{i+1}" for i in range(4)]),
                "shared_pool": self.config["param_dict"]["shared_pool"],
            },
        ...
        DummyTestRun.asserted_tests = [
            ...
            {
                "shortname": "^leaves.quicktest.tutorial2.files.vm1",
                "vms": "^vm1$",
                "nets": "^net1$",
                "nets_spawner": "lxc",
                "nets_gateway": "^$",
                ...
                "nets_shell_host_net1": "^192.168.254.101$",
                "nets_shell_port_net1": "22",
            },
            ...
            # net4 now moves on to its planned test
            {
                ...
                "nets_gateway": "^$",
                "nets_host": "^c104$",
                "get_location_image1_vm1": r"[\w:/]+ net3:/mnt/local/images/swarm",
                "get_location_image1_vm2": r"[\w:/]+ net4:/mnt/local/images/swarm",
                "nets_shell_host_net3": "^192.168.254.103$",
                "nets_shell_host_net4": "^192.168.254.104$",
                "nets_shell_port_net3": "22",
                "nets_shell_port_net4": "22",
            },
            # net1 continues to the second tutorial2
            {
                "shortname": "^leaves.quicktest.tutorial2.names.vm1",
                "vms": "^vm1$",
                "nets": "^net1$",
                "nets_spawner": "lxc",
                "nets_gateway": "^$",
    ...
    def test_trace_work_remote(self):
        ...
        graph = self._load_for_parsing(
            "leaves..tutorial2,leaves..tutorial_gui",
            {"nets": nets, "shared_pool": self.config["param_dict"]["shared_pool"]},
        )
        ...
        DummyTestRun.asserted_tests = [
            ...
            },
            {
                ...
                "nets_shell_host_cluster1.net6": "^cluster1.net.lan$",
                "nets_shell_port_cluster1.net6": "221",
            },
            {
                "shortname": "^leaves.tutorial_gui.client_clicked",
                "vms": "^vm1 vm2$",
                ...
                "nets_gateway": "^cluster2.net.lan$",
                "nets_host": "^2$",
                "get_location_image1_vm1": r"[\w:/]+ cluster2.net6:/mnt/local/images/swarm",
                "get_location_image1_vm2": r"[\w:/]+ cluster2.net7:/mnt/local/images/swarm",
                "nets_shell_host_cluster2.net6": "^cluster2.net.lan$",
                "nets_shell_host_cluster2.net7": "^cluster2.net.lan$",
                "nets_shell_port_cluster2.net6": "221",
                "nets_shell_port_cluster2.net7": "222",
            },
            {
                "shortname": "^leaves.quicktest.tutorial2.names.vm1",
                "vms": "^vm1$",
    ...
    def test_trace_work_preparsed(self):
        ...
        DummyTestRun.asserted_tests = [
            ...
            # net2 would step back from already occupied linux_virtuser (by net3) and net3 proceeds from most distant path
            {
                ...
                "nets": "^net3$",
                "get_location_image1_vm1": r"[\w:/]+ net3:/mnt/local/images/swarm",
                "get_location_image1_vm2": r"[\w:/]+ net2:/mnt/local/images/swarm",
            },
    ...
    def test_cloning_simple_permanent_object(self):
        ...
        DummyTestRun.asserted_tests = [
            ...
            {
                "shortname": "^leaves.tutorial_get.implicit_both.guisetup.noop.vm1",
                "vms": "^vm1 vm2 vm3$",
                "get_state_images_image1_vm2": "guisetup.noop",
            },
            # second (clicked) duplicated actual test
            {
                "shortname": "^leaves.tutorial_get.implicit_both.guisetup.clicked.vm1",
                "vms": "^vm1 vm2 vm3$",
                "get_state_images_image1_vm2": "guisetup.clicked",
            },
        ]
        ...
        self.assertEqual(
            DummyStateControl.asserted_states["unset"]["getsetup.noop"][
                self.shared_pool
            ],
            1,
        )
        self.assertEqual(
            DummyStateControl.asserted_states["unset"]["ready"][self.shared_pool], 0
        )
    ...
    def test_cloning_simple_cross_object(self):
        ...
        graph = self._load_for_parsing(
            "leaves..tutorial_get,leaves..tutorial_gui", {"nets": "net1"}
        )
        ...
        DummyStateControl.asserted_states["check"].update(
            {
                "guisetup.noop": {self.shared_pool: False},
                "fedora.noop": {self.shared_pool: False},
                "guisetup.clicked": {self.shared_pool: False},
                "getsetup.noop": {self.shared_pool: False},
                "getsetup.clicked": {self.shared_pool: False},
                "getsetup.guisetup.noop": {self.shared_pool: False},
        ...
        DummyStateControl.asserted_states["unset"] = {
            "guisetup.noop": {self.shared_pool: 0},
            "fedora.noop": {self.shared_pool: 0},
            "getsetup.noop": {self.shared_pool: 0},
            "ready": {self.shared_pool: 0},
        }
        DummyTestRun.asserted_tests = [
            ...
            {
                "shortname": "^internal.automated.linux_virtuser.vm1.+CentOS",
                "vms": "^vm1$",
            },
            # automated setup of vm2 of Win10 variant
            {
                "shortname": "^internal.automated.windows_virtuser.vm2.+Win10",
                "vms": "^vm2$",
            },
            # first (noop) parent GUI setup dependency through vm2 of Win10 variant
            {
                "shortname": "^leaves.tutorial_gui.client_noop.vm1.+CentOS.+vm2.+Win10",
                "vms": "^vm1 vm2$",
                "set_state_images_vm2": "guisetup.noop",
            },
            # second (clicked) parent GUI setup dependency through vm2 of Win10 variant
            {
                "shortname": "^leaves.tutorial_gui.client_clicked.vm1.+CentOS.+vm2.+Win10",
                "vms": "^vm1 vm2$",
                "set_state_images_vm2": "guisetup.clicked",
            },
            ...
            {
                "shortname": "^leaves.tutorial_get.explicit_noop.vm1.+CentOS.+vm2.+Win10",
                "vms": "^vm1 vm2 vm3$",
                "get_state_images_vm2": "guisetup.noop",
            },
            # second (clicked) explicit actual test of CentOS+Win10
            {
                "shortname": "^leaves.tutorial_get.explicit_clicked.vm1.+CentOS.+vm2.+Win10",
                "vms": "^vm1 vm2 vm3$",
                "get_state_images_vm2": "guisetup.clicked",
            },
            # first (noop) duplicated actual test of CentOS+Win10
            {
                "shortname": "^leaves.tutorial_get.implicit_both.guisetup.noop.vm1.+CentOS.+vm2.+Win10",
                "vms": "^vm1 vm2 vm3$",
                "get_state_images_image1_vm2": "guisetup.noop",
            },
            ...
            {
                "shortname": "^internal.automated.windows_virtuser.vm2.+Win7",
                "vms": "^vm2$",
            },
            # first (noop) parent GUI setup dependency through vm2 of Win7 variant
            {
                "shortname": "^leaves.tutorial_gui.client_noop.vm1.+CentOS.+vm2.+Win7",
                "vms": "^vm1 vm2$",
                "set_state_images_vm2": "guisetup.noop",
            },
            # first (noop) duplicated actual test of CentOS+Win7
            {
                "shortname": "^leaves.tutorial_get.implicit_both.guisetup.noop.vm1.+CentOS.+vm2.+Win7",
                "vms": "^vm1 vm2 vm3$",
                "get_state_images_image1_vm2": "guisetup.noop",
            },
            # second (clicked) parent GUI setup dependency through vm2 of Win7 variant
            {
                "shortname": "^leaves.tutorial_gui.client_clicked.vm1.+CentOS.+vm2.+Win7",
                "vms": "^vm1 vm2$",
                "set_state_images_vm2": "guisetup.clicked",
            },
            # second (clicked) duplicated actual test of CentOS+Win7
            {
                "shortname": "^leaves.tutorial_get.implicit_both.guisetup.clicked.vm1.+CentOS.+vm2.+Win7",
                "vms": "^vm1 vm2 vm3$",
                "get_state_images_image1_vm2": "guisetup.clicked",
            },
            # TODO: prefix order would change the vm variants on implicit_both flat node
            ...
            {
                "shortname": "^leaves.tutorial_get.implicit_both.guisetup.clicked.vm1.+CentOS.+vm2.+Win10",
                "vms": "^vm1 vm2 vm3$",
                "get_state_images_image1_vm2": "guisetup.clicked",
            },
            # first (noop) explicit actual test of CentOS+Win7
            {
                "shortname": "^leaves.tutorial_get.explicit_noop..+CentOS.+vm2.+Win7",
                "vms": "^vm1 vm2 vm3$",
                "get_state_images_vm2": "guisetup.noop",
            },
            # automated setup of vm1 of Fedora variant, required via extra "tutorial_gui" restriction
            {
                "shortname": "^internal.automated.linux_virtuser.vm1.+Fedora",
                "vms": "^vm1$",
            },
            ...
            {
                "shortname": "^leaves.tutorial_gui.client_noop.vm1.+Fedora.+vm2.+Win10",
                "vms": "^vm1 vm2$",
                "set_state_images_vm2": "fedora.noop",
            },
            ...
            {
                "shortname": "^leaves.tutorial_gui.client_noop.vm1.+Fedora.+vm2.+Win7",
                "vms": "^vm1 vm2$",
                "set_state_images_vm2": "fedora.noop",
            },
            # second (clicked) explicit actual test of CentOS+Win7
            {
                "shortname": "^leaves.tutorial_get.explicit_clicked.vm1.+CentOS.+vm2.+Win7",
                "vms": "^vm1 vm2 vm3$",
                "get_state_images_vm2": "guisetup.clicked",
            },
            ...
            {
                "shortname": "^leaves.tutorial_gui.client_clicked.vm1.+Fedora.+vm2.+Win10",
                "vms": "^vm1 vm2$",
            },
            ...
            {
                "shortname": "^leaves.tutorial_gui.client_clicked.vm1.+Fedora.+vm2.+Win7",
                "vms": "^vm1 vm2$",
            },
        ]
    ...
    def test_cloning_deep(self):
        ...
        DummyTestRun.asserted_tests = [
            ...
            # first (noop) parent GUI setup dependency through vm2
            {
                "shortname": "^tutorial_gui.client_noop",
                "vms": "^vm1 vm2$",
                "set_state_images_vm2": "guisetup.noop",
            },
            # first (noop) duplicated actual test
            {
                "shortname": "^tutorial_get.implicit_both.guisetup.noop",
                "vms": "^vm1 vm2 vm3$",
                "get_state_images_image1_vm2": "guisetup.noop",
                "set_state_images_image1_vm2": "getsetup.guisetup.noop",
            },
            {
                "shortname": "^leaves.tutorial_finale.getsetup.guisetup.noop",
                "vms": "^vm1 vm2 vm3$",
                "get_state_images_image1_vm2": "getsetup.guisetup.noop",
            },
            # second (clicked) parent GUI setup dependency through vm2
            {
                "shortname": "^tutorial_gui.client_clicked",
                "vms": "^vm1 vm2$",
                "set_state_images_vm2": "guisetup.clicked",
            },
            # second (clicked) duplicated actual test
            {
                "shortname": "^tutorial_get.implicit_both.guisetup.clicked",
                "vms": "^vm1 vm2 vm3$",
                "get_state_images_image1_vm2": "guisetup.clicked",
                "set_state_images_image1_vm2": "getsetup.guisetup.clicked",
            },
            {
                "shortname": "^leaves.tutorial_finale.getsetup.guisetup.clicked",
                "vms": "^vm1 vm2 vm3$",
                "get_state_images_image1_vm2": "getsetup.guisetup.clicked",
            },
        ]
    ...
    def test_loader_runner_entries(
        self, mock_load_graph, mock_run_graph, mock_status_server, _mock_status_repo
    ):
        ...
        mock_load_graph.parse_flat_nodes.return_value = [
            TestNode(prefix="1", recipe=None),
            TestNode(prefix="2", recipe=None),
        ]

        self.loader.config = self.config
        resolutions = [self.loader.resolve(reference)]
        mock_load_graph.parse_flat_nodes.assert_called_with(
            self.config["tests_str"], self.config["param_dict"]
        )
        runnables = resolutions_to_runnables(resolutions, self.config)
        test_suite = TestSuite(
            "suite", config=self.config, tests=runnables, resolutions=resolutions
        )
        ...
        run_graph_workers = run_graph_instance.workers.values.return_value = [
            mock.MagicMock()
        ]
