...
class LibvirtXMLTestBase(unittest.TestCase):
    ...
    def _define(file_path, **dargs):
        ...
        xml_path = os.path.join(LibvirtXMLTestBase.__doms_dir__, "%s.xml" % dom_name)
    ...
    def _dumpxml(name, to_file="", **dargs):
        ...
        file_path = os.path.join(LibvirtXMLTestBase.__doms_dir__, "%s.xml" % name)
        if os.path.exists(file_path):
            xml_file = open(file_path, "r")
            domain_xml = xml_file.read()
        else:
            xml_file = open(file_path, "w")
            domain_xml = LibvirtXMLTestBase.__domain_xml__ % (
                name,
    ...
    def _nodedev_dumpxml(name, options="", to_file=None, **dargs):
        ...
        if options != "":
            raise ValueError(
                "Dummy virsh for testing does not support options" " parameter"
            )
...
class Baz(base.LibvirtXMLBase):
    ...
    def __init__(self, parent, virsh_instance):
        accessors.XMLElementText("foobar", self, ["set", "del"], "/", "baz")
        ...
        parent.assertRaises(
            xcepts.LibvirtXMLForbiddenError, self.__setattr__, "foobar", None
        )
        parent.assertRaises(
            xcepts.LibvirtXMLForbiddenError, self.__setitem__, "foobar", None
        )
        parent.assertRaises(xcepts.LibvirtXMLForbiddenError, self.__delattr__, "foobar")
        parent.assertRaises(xcepts.LibvirtXMLForbiddenError, self.__delitem__, "foobar")
...
class Bar(base.LibvirtXMLBase):
    ...
    def __init__(self, parent, virsh_instance):
        subclass_dargs = {"parent": parent, "virsh_instance": virsh_instance}
        accessors.XMLElementNest("baz", self, None, "/", "baz", Baz, subclass_dargs)
        super(Bar, self).__init__(virsh_instance=virsh_instance)
        # must setup some basic XML inside for this element
        self.xml = """<bar></bar>"""
        parent.assertTrue(isinstance(parent, AccessorsTest))
        parent.assertRaises(xcepts.LibvirtXMLNotFoundError, self.__getattr__, "baz")
        parent.assertRaises(xcepts.LibvirtXMLNotFoundError, self.__getitem__, "baz")
        # Built-in type-checking
        parent.assertRaises(
            ValueError, self.__setattr__, "baz", True
        )  # bool() is not a Bar()
        parent.assertRaises(ValueError, self.__setitem__, "baz", None)
...
class AccessorsTest(LibvirtXMLTestBase):
    ...
    def test_accessor_base(self):
        ...
        self.assertRaises(ValueError, accessors.AccessorBase, "foobar", lvx, lvx)
    ...
    def test_XMLElementInt(self):
        class FooBar(base.LibvirtXMLBase):
            __slots__ = ("auto_test", "bin_test", "oct_test", "dec_test", "hex_test")
        ...
        for name, radix in list(name_radix.items()):
            accessors.XMLElementInt(
                name + "_test", lvx, parent_xpath="/", tag_name=name, radix=radix
            )
            self.assertEqual(lvx[name + "_test"], radix)

        self.assertRaises(ValueError, lvx.__setitem__, "bin_test", "text")
    ...
    def test_AllForbidden(self):
        ...
        self.assertRaises(xcepts.LibvirtXMLForbiddenError, lvx.__getitem__, "test")
        self.assertRaises(
            xcepts.LibvirtXMLForbiddenError, lvx.__setitem__, "test", "foobar"
        )
        self.assertRaises(xcepts.LibvirtXMLForbiddenError, lvx.__delitem__, "test")
    ...
    def test_not_enuf_dargs(self):
        ...
        self.assertRaises(ValueError, accessors.XMLElementText, "test", foobar, "/")
        self.assertRaises(TypeError, accessors.XMLElementText)
        self.assertRaises(TypeError, accessors.XMLElementText, "test")
    ...
    def test_too_many_dargs(self):
        ...
        foobar = FooBar(self.dummy_virsh)
        self.assertRaises(
            ValueError, accessors.XMLElementText, "test", foobar, "/", "foobar"
        )
        self.assertRaises(
            ValueError, accessors.XMLElementText, "test", None, None, None, None
        )
        ...
        class Foo(base.LibvirtXMLBase):
            ...
            def __init__(self, parent, virsh_instance):
                ...
                accessors.XMLElementNest(
                    "bar", self, None, "/", "bar", Bar, subclass_dargs
                )
                # pylint: disable=E1003
                ...
                parent.assertRaises(
                    ValueError, self.bar.__setattr__, "baz", Bar
                )  # Baz is not a Bar()
    ...
    def test_XMLElementBool_simple(self):
        class Foo(base.LibvirtXMLBase):
            ...
            def __init__(self, virsh_instance):
                accessors.XMLElementBool("bar", self, parent_xpath="/", tag_name="bar")
                accessors.XMLElementBool("baz", self, parent_xpath="/", tag_name="bar")
                # pylint: disable=E1003
                super(Foo, self).__init__(virsh_instance=virsh_instance)
                self.xml = "<foo/>"
        ...
        class Foo(base.LibvirtXMLBase):
            ...
            def __init__(self, virsh_instance):
                accessors.XMLElementBool(
                    "foo", self, parent_xpath="/l1", tag_name="foo"
                )
                accessors.XMLElementBool(
                    "bar", self, parent_xpath="/l1/l2/l3", tag_name="bar"
                )
                accessors.XMLElementBool(
                    "baz", self, parent_xpath="/l1/l2", tag_name="baz"
                )
                # pylint: disable=E1003
                super(Foo, self).__init__(virsh_instance=virsh_instance)
                self.xml = "<root/>"
        ...
        class Whatchamacallit(object):
            ...
            def from_it(item, index, lvxml):
                if not isinstance(item, Whatchamacallit):
                    raise ValueError
                return ("whatchamacallit", {"secret_sauce": str(item.secret_sauce)})
    ...
    def test_XMLElementList_Text(self):
        class Whatchamacallit(base.LibvirtXMLBase):
            ...
            def __init__(self, virsh_instance, text=None):
                accessors.XMLElementText(
                    "text", self, parent_xpath="/", tag_name="whatchamacallit"
                )
                # pylint: disable=E1003
        ...
        class Foo(base.LibvirtXMLBase):
            ...
            def from_it(item, index, lvxml):
                if not isinstance(item, Whatchamacallit):
                    raise ValueError
                return ("whatchamacallit", {}, item.text)
...
class TestLibvirtXML(LibvirtXMLTestBase):
    ...
    def test_uuid(self):
        ...
        self.assertEqual(test_uuid, UUID)
        self.assertRaises(
            xcepts.LibvirtXMLForbiddenError, lvxml.__setattr__, "uuid", "foobar"
        )
        self.assertRaises(xcepts.LibvirtXMLForbiddenError, lvxml.__delitem__, "uuid")
    ...
    def test_guest_capabilities(self):
        ...
        self.assertEqual(test_guest_capa, expected)
        self.assertRaises(
            xcepts.LibvirtXMLForbiddenError,
            lvxml.__setattr__,
            "guest_capabilities",
            "foobar",
        )
        self.assertRaises(
            xcepts.LibvirtXMLForbiddenError, lvxml.__delitem__, "guest_capabilities"
        )
...
class TestVMXML(LibvirtXMLTestBase):
    ...
    def test_seclabel(self):
        ...
        self.assertRaises(xcepts.LibvirtXMLError, getattr, vmxml, "seclabel")
...
class testNetworkXML(LibvirtXMLTestBase):
    def _from_scratch(self):
        netxml = network_xml.NetworkXML(
            network_name="test0", virsh_instance=self.dummy_virsh
        )
...
class testSerialXML(LibvirtXMLTestBase):
    ...
    XML = "<serial type='pty'><source path='/dev/null'/>\
    ...
    def test_vm_get_modify(self):
        ...
        self.assertEqual(
            source_connect, {"mode": "connect", "host": "1.2.3.4", "service": "2445"}
        )
...
class testVMCPUTuneXML(LibvirtXMLTestBase):
    def test_get_set_del(self):
        ...
        del cputune.shares
        self.assertRaises(xcepts.LibvirtXMLNotFoundError, cputune.__getitem__, "shares")
        del cputune.emulatorpin
        self.assertRaises(
            xcepts.LibvirtXMLNotFoundError, cputune.__getitem__, "emulatorpin"
        )
        ...
        self.assertEqual(cputune.vcpupins[4], {"vcpu": "4", "cpuset": "2-4,^3"})
...
class testDiskXML(LibvirtXMLTestBase):
    def _check_disk(self, disk):
        ...
        for propname in ("rawio", "sgio", "snapshot", "iotune"):
            self.assertRaises(
                xcepts.LibvirtXMLNotFoundError, disk.__getitem__, propname
            )
            self.assertRaises(
                xcepts.LibvirtXMLNotFoundError, disk.__getattr__, propname
            )
            self.assertRaises(
                xcepts.LibvirtXMLNotFoundError, getattr(disk, "get_" + propname)
            )
...
class testAddressXML(LibvirtXMLTestBase):
    def test_required(self):
        ...
        self.assertRaises(
            xcepts.LibvirtXMLError, address.new_from_dict, {}, self.dummy_virsh
        )
        # no type_name attribute
        element = xml_utils.ElementTree.Element("address", {"foo": "bar"})
        self.assertRaises(
            xcepts.LibvirtXMLError, address.new_from_element, element, self.dummy_virsh
        )
...
class testCAPXML(LibvirtXMLTestBase):
    def test_capxmlbase(self):
        ...
        self.assertRaises(NotImplementedError, capxmlbase.get_sysfs_sub_path)
        self.assertRaises(NotImplementedError, capxmlbase.get_key2filename_dict)
        self.assertRaises(NotImplementedError, capxmlbase.get_key2value_dict)
...
class testNodedevXML(LibvirtXMLTestBase):
    def test_new_from_dumpxml(self):
        ...
        nodedevxml = NodedevXML.new_from_dumpxml(
            "pci_0000_00_00_0", virsh_instance=self.dummy_virsh
        )
        self.assertTrue(isinstance(nodedevxml, NodedevXML))

    def test_get_key2value_dict(self):
        NodedevXML = nodedev_xml.NodedevXML
        xml = NodedevXML.new_from_dumpxml(
            "pci_0000_00_00_0", virsh_instance=self.dummy_virsh
        )
