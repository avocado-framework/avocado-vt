...
class QtreeContainerTest(unittest.TestCase):
    ...
    def test_qtree(self):
        ...
        for i in xrange(len(nodes)):
            self.assertTrue(
                isinstance(nodes[i], reference_nodes[i]),
                (
        ...
        self.assertTrue(
            isinstance(tree.str_qtree(), six.string_types),
            "qtree.str_qtree() returns nonstring output.",
        )

        self.assertTrue(
            isinstance(str(tree), six.string_types),
...
class QtreeDiskContainerTest(unittest.TestCase):
    ...
    def test_check_params(self):
        ...
        self.assertEqual(disks.check_guests_proc_scsi(guest_proc_scsi), (0, 0, 1, 0))
        # Check the full disk output (including params)
        for disk in disks.disks:
            self.assertTrue(
                isinstance(str(disk), six.string_types),
    ...
    def test_check_params_bad(self):
        ...
        _guest_proc_scsi = guest_proc_scsi.replace("Channel: 00", "Channel: 01")
...
class KvmQtreeClassTest(unittest.TestCase):
    ...
    def test_qtree_bus_bus(self):
        ...
        self.assertRaises(
            qemu_qtree.IncompatibleTypeError, test.add_child, qemu_qtree.QtreeBus()
        )
    ...
    def test_qtree_dev_dev(self):
        ...
        self.assertRaises(
            qemu_qtree.IncompatibleTypeError, test.add_child, qemu_qtree.QtreeDev()
        )
