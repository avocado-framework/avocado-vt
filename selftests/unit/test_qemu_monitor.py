...
sd0: [not inserted]
    Removable device: not locked, tray closed"""
        ...
        exp = {
            "sd0": {"tray-open": 0, "locked": 0, "not-inserted": 1, "removable": 1},
            "ide0-hd0": {
                "bps_rd": 0,
                "backing_file_depth": 2,
                "removable": 0,
            ...
            "floppy0": {"tray-open": 0, "locked": 0, "not-inserted": 1, "removable": 1},
            "ide1-cd0": {
                "tray-open": 0,
                "locked": 0,
                "not-inserted": 1,
        ...
        exp = {
            ...
            "scsi0-hd1": {"encrypted": 1, "drv": "qcow2", "file": "enc.qcow2"},
        }
        assert (
            out2 == exp
        ), "Info block of qemu 1.5 is parsed incorrectly\n%s" "\n%s" % (out2, exp)
        ...
        for name, params in six.iteritems(out2):
            ...
            for key, value in six.iteritems(params):
                assert (
                    out1[name].get(key, 0) == value
                ), "value of disk %s " "mismatch in info-1.5 %s=%s (%s)\n%s\n%s" % (
                    name,
                    key,
                    value,
                    out1[name].get(key, 0),
        ...
        assert (
            out3 == exp
        ), "QMP query-block of qemu is parsed incorrectly\n" "%s\n%s" % (out3, exp)
        ...
        for name, params in six.iteritems(out1):
            ...
            for key, value in six.iteritems(params):
                assert (
                    out3[name].get(key, 0) == value
                ), "value of disk %s " "mismatch in QMP version %s=%s (%s)\n%s\n%s" % (
                    name,
                    key,
                    value,
                    out3[name].get(key, 0),
