...
"""
        self.assertEqual(
            qdevice.str_long(),
            out,
            "Device output doesn't match" "\n%s\n\n%s" % (qdevice.str_long(), out),
        )

    def test_q_string_device(self):
        """QStringDevice tests"""
        qdevice = qdevices.QStringDevice(
            "MyType", {"addr": "0x7"}, cmdline="-qdevice ahci,addr=%(addr)s"
        )
        self.assertEqual(
            qdevice.cmdline(),
            "-qdevice ahci,addr=0x7",
            "Cmdline"
    ...
    def test_q_device(self):
        ...
        self.assertEqual(
            str(qdevice),
            "a'ahci'",
            "Alternative name error %s " "!= %s" % (str(qdevice), "a'ahci'"),
        )
        ...
        self.assertEqual(
            str(qdevice),
            "q'ahci1'",
            "Id name error %s " "!= %s" % (str(qdevice), "q'ahci1'"),
        )
...
class Buses(unittest.TestCase):
    ...
    def test_q_sparse_bus(self):
        ...
        # Compare short repr
        exp = (
            "my_bus(bus_type): {0-0-0:a'dev1',0-0-1:a'dev6',0-0-2:a'dev7',"
        ...
        self.assertEqual(
            out,
            exp,
            "Short representation corrupted:\n%s\n%s"
...
"""
        ...
        self.assertEqual(
            out,
            exp,
            "Got device while expecting None:\n%s\n%s"
            "\n\n%s" % (out, exp, bus.str_long()),
        )
...
"""
    ...
    def test_usb_bus(self):
        ...
        dev = qdevices.QDevice(
            "usb-kbd", {"port": "2.4.3.1"}, parent_bus={"type": "uhci"}
        )
        ...
        out = hub2.insert(
            qdevices.QDevice(
                "usb-kbd", {"port": "2.4.3.3"}, parent_bus={"type": "uhci"}
            )
        )
        assert out == "BusId"
        ...
        out = hub2.insert(
            qdevices.QDevice("usb-kbd", {"port": "2.4"}, parent_bus={"type": "uhci"})
        )
        assert out == "BusId"
        ...
        # but different port
        out = hub2.insert(
            qdevices.QDevice("usb-kbd", {"port": "2.3.4"}, parent_bus={"type": "uhci"})
        )
        assert out == "BusId"

        # Force insert device with port which belongs to other hub
        dev = qdevices.QDevice(
            "usb-hub", {"port": "2.4.3.4"}, parent_bus={"type": "uhci"}
        )

        # Check the overall buses correctness
        self.assertEqual("usb1.0(uhci): {1:a'usb-kbd',2:a'usb-hub'}", usbc1.str_short())
...
class Container(unittest.TestCase):
    ...
    def test_qdev_functional(self):
        ...
        out = qdev.insert(qdev.machine_by_params(ParamsDict({"machine_type": "pc"})))
...
fdc
  ...
  params:"""
        ...
        self.assertNotEqual(
            re.findall(exp, out),
            None,
            "Long representation is" "corrupted:\n%s\n%s" % (out, exp),
        )
        ...
        bus = qdevices.QSparseBus("bus", [["addr"], [6]], "hba1.0", "hba", "a_hba")
        dev = qdevice(
            "HBA",
            {"id": "hba1", "addr": 10},
            parent_bus={"aobject": "pci.0"},
        ...
        exp = (
            "Devices of vm1: [t'machine',t'i440FX',t'PIIX4_PM',t'PIIX3',"
        ...
        self.assertNotEqual(
            qdev2, qdev, "Other qdev matches this one:\n%s\n%s" % (qdev, qdev2)
        )
        # cmdline
        exp = (
        ...
        self.assertEqual(
            len(out),
            1,
            "Incorrect number of devices by qid "
        ...
        self.assertEqual(
            out,
            None,
            "Failed to remove device:\n%s\nRepr:\n%s" % ("hba1__0", qdev.str_long()),
        )

        # Remove device which contains other devices (without recursive)
        self.assertRaises(qcontainer.DeviceRemoveError, qdev.remove, "hba1", False)
    ...
    def test_qdev_hotplug(self):
        ...
        devs = qdev.images_define_by_variables(
            "disk", "/tmp/a", {"aobject": "pci.0"}, fmt="virtio"
        )
        ...
        assert ver_out is True, "Return value of hotplug" " is not True (%s)" % ver_out
        out = qdev.get_state()
    ...
    def test_qdev_low_level(self):
        ...
        self.assertEqual(
            len(out),
            3,
            "get_buses should return 3 buses but "
            "returned %s instead:\n%s" % (len(out), out),
        )
        ...
        self.assertEqual(
            len(out),
            2,
            "Number of missing named buses is "
            "incorrect: %s != %s\n%s" % (len(out), 2, out),
        )
        ...
        self.assertEqual(
            out, 3, "Incorrect idx of next named bus: %s !=" " %s" % (out, 3)
        )
        ...
        out = bus.get_device()
        assert out == dev, (
            "bus.get_device() returned different device "
    ...
    def test_qdev_equal(self):
        ...
        assert qdev3 != qdev1, (
            "Similar hotplugged qdevs match even thought "
    ...
    def test_pci(self):
        ...
        devs = qdev.pcic_by_params("root.1", {"pci_bus": "pci.0", "type": "ioh3420"})
        qdev.insert(devs)
        devs = qdev.pcic_by_params("pci_switch", {"pci_bus": "root.1", "type": "x3130"})

        qdev.insert(devs)
        devs = qdev.pcic_by_params(
            "pci_bridge", {"pci_bus": "root.1", "type": "pci-bridge"}
        )
        qdev.insert(devs)
        ...
        qdev.insert(
            qdevices.QDevice(
                "ahci",
                {"id": "in_bridge"},
                parent_bus={"type": ("PCI", "PCIE"), "aobject": "pci_bridge"},
            )
        )
        ...
        qdev.insert(
            qdevices.QDevice(
                "ahci",
                {"id": "in_switch1"},
                parent_bus={"type": ("PCI", "PCIE"), "aobject": "pci_switch"},
            )
        )
        qdev.insert(
            qdevices.QDevice(
        ...
        qdev.insert(
            qdevices.QDevice(
                "ahci",
                {"id": "in_root1"},
                parent_bus={"type": ("PCI", "PCIE"), "aobject": "root.1"},
            )
        )
        ...
        qdev.insert(
            qdevices.QDevice(
                "ahci",
                {"id": "in_pci.0"},
                parent_bus={"type": ("PCI", "PCIE"), "aobject": "pci.0"},
            )
        )

        exp = (
            "-machine pc -device ioh3420,id=root.1,bus=pci.0,addr=0x2 "
