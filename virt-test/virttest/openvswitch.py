...
class OpenVSwitchControl(object):
    ...
    def get_version(cls):
        ...
        version = None
        try:
            result = process.run(path.find_command("ovs-vswitchd"),
                                 args=["--version"])
...
class OpenVSwitchControlCli_140(OpenVSwitchControl):
    ...
    def ovs_vsctl(self, parmas, ignore_status=False):
        return process.run(path.find_command("ovs-vsctl"), timeout=10,
...
class OpenVSwitch(OpenVSwitchSystem):
    ...
    def init_db(self):
        process.run(path.find_command("ovsdb-tool"), timeout=10,
                    args=["create", self.db_path, self.dbschema])
        process.run(path.find_command("ovsdb-server"), timeout=10,
    ...
    def start_ovs_vswitchd(self):
        process.run(path.find_command("ovs-vswitchd"), timeout=10,
                    args=["--detach",
